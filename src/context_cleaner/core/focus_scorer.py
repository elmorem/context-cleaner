...
class FocusMetrics:
    ...
    def get_focus_summary(self) -> str:
        ...
        return (f"Focus: {self.focus_score}% | Priority Alignment: {self.priority_alignment_score}% | "
...
class FocusScorer:
    ...
    CURRENT_WORK_KEYWORDS = [
        ...
        'current', 'currently', 'working on', 'in progress', 'now', 'today',
        'implementing', 'debugging', 'fixing', 'building', 'developing',
        'active', 'ongoing', 'next', 'immediate', 'priority', 'urgent',
        
        # Task action words
        'todo', 'task', 'action', 'need to', 'should', 'must', 'required',
        'objective', 'goal', 'target', 'milestone', 'deadline',
        
        # Problem-solving indicators
        'issue', 'problem', 'bug', 'error', 'fix', 'resolve', 'solve',
        'investigate', 'analyze', 'review', 'check', 'test', 'verify'
    ]
    
    DISTRACTION_KEYWORDS = [
        # Old/completed work
        'completed', 'done', 'finished', 'archived', 'old', 'previous',
        'legacy', 'deprecated', 'obsolete', 'historical', 'past',
        
        # Low priority items  
        'nice to have', 'maybe', 'consider', 'someday', 'eventually',
        'low priority', 'not urgent', 'future', 'later',
        
        # Noise indicators
        'random', 'tangent', 'aside', 'off topic', 'unrelated',
        'distraction', 'interruption', 'sidebar'
    ]
    
    HIGH_PRIORITY_INDICATORS = [
        'urgent', 'critical', 'important', 'priority', 'asap', 'immediate',
        'blocker', 'blocking', 'high priority', 'must have', 'required',
        'deadline', 'due', 'needed', 'essential', 'crucial'
    ]
    ...
    def __init__(self):
        ...
        self.distraction_regex = re.compile('|'.join(self.DISTRACTION_KEYWORDS), re.IGNORECASE)
        self.high_priority_regex = re.compile('|'.join(self.HIGH_PRIORITY_INDICATORS), re.IGNORECASE)
        self.task_action_patterns = [re.compile(pattern, re.IGNORECASE) for pattern in self.TASK_ACTION_PATTERNS]
        logger.debug("FocusScorer initialized")
    
    def _extract_content_with_positions(self, context_data: Dict[str, Any]) -> List[Tuple[int, str, Any]]:
    ...
    def _analyze_content_focus(self, content: str) -> Dict[str, Any]:
        ...
        if sum([analysis['is_current_work'], analysis['is_high_priority'], analysis['is_actionable']]) >= 2:
            focus_points += 15
    ...
    def _calculate_priority_alignment(self, items_with_analysis: List[Tuple[int, str, Any, Dict]]) -> int:
        ...
        high_priority_items = [item for item in items_with_analysis if item[3]['is_high_priority']]
        high_priority_in_top = sum(1 for item in high_priority_items if item[0] <= top_quarter_threshold)
    ...
    def _calculate_attention_clarity(self, items_with_analysis: List[Tuple[int, str, Any, Dict]]) -> int:
        """Calculate how clear the next steps are vs contextual noise."""
        if not items_with_analysis:
            return 50
        
        actionable_items = sum(1 for item in items_with_analysis if item[3]['is_actionable'])
        current_work_items = sum(1 for item in items_with_analysis if item[3]['is_current_work'])
        distraction_items = sum(1 for item in items_with_analysis if item[3]['is_distraction'])
        ...
        clarity_score = ((actionable_ratio + current_work_ratio) * 50) - (distraction_ratio * 30)
        
        return int(min(100, max(0, clarity_score)))
    
    def _calculate_context_coherence(self, items_with_analysis: List[Tuple[int, str, Any, Dict]]) -> int:
    ...
    def _calculate_overall_focus_score(self, metrics: Dict[str, Any]) -> int:
        ...
        focus_components = [
            ...
            (metrics['priority_alignment_score'], 0.25),      # 25% weight for priority alignment  
            (metrics['attention_clarity_score'], 0.25),       # 25% weight for attention clarity
    ...
    async def calculate_focus_metrics(self, context_data: Dict[str, Any]) -> FocusMetrics:
        """
        ...
        try:
            ...
            total_items = len(items_with_analysis)
            work_related_items = sum(1 for item in items_with_analysis if item[3]['is_current_work'])
            high_priority_items = sum(1 for item in items_with_analysis if item[3]['is_high_priority'])
            active_task_items = sum(1 for item in items_with_analysis if item[3]['is_actionable'])
            noise_items = sum(1 for item in items_with_analysis if item[3]['is_distraction'])
            
            # Calculate ratios and scores
            work_related_ratio = work_related_items / total_items if total_items > 0 else 0
            priority_alignment_score = self._calculate_priority_alignment(items_with_analysis)
            attention_clarity_score = self._calculate_attention_clarity(items_with_analysis)
            context_coherence_score = self._calculate_context_coherence(items_with_analysis)
            ...
            important_items_in_top_quarter = sum(1 for item in items_with_analysis 
                                               if item[0] <= top_quarter_threshold and item[3]['is_high_priority'])
            current_work_in_top_half = sum(1 for item in items_with_analysis 
                                         if item[0] <= top_half_threshold and item[3]['is_current_work'])
            noise_in_bottom_half = sum(1 for item in items_with_analysis 
                                     if item[0] >= bottom_half_threshold and item[3]['is_distraction'])
            
            # Task and goal clarity
            task_clarity_score = min(100, (active_task_items / max(1, total_items)) * 200)
            goal_alignment_score = min(100, ((work_related_items + high_priority_items) / max(1, total_items)) * 100)
            ...
            analysis_method_breakdown = {
                'keyword_based': sum(1 for item in items_with_analysis if item[3]['focus_keywords']),
            ...
            metrics = FocusMetrics(
                ...
                focus_keywords_found=list(set(focus_keywords_found))[:10],  # Limit to top 10
            ...
            logger.info(f"Focus analysis completed: {overall_focus_score}% focus score, "
