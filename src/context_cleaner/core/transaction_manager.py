...
from .manipulation_engine import ManipulationOperation, ManipulationPlan, ManipulationResult
from .manipulation_validator import ManipulationValidator, ValidationResult, RiskAssessment
...
class TransactionManager:
    ...
    def __init__(self, 
        ...
        self.default_isolation = TransactionIsolation(
            self.config.get('default_isolation', TransactionIsolation.READ_COMMITTED.value)
        )
        self.enable_nested_transactions = self.config.get('enable_nested_transactions', True)
        self.max_transaction_time = self.config.get('max_transaction_time', 300)  # 5 minutes
    ...
    def _complete_transaction(self, transaction: 'Transaction', result: TransactionResult) -> None:
        ...
        if self.enable_transaction_logging:
            logger.info(f"Transaction {transaction.metadata.transaction_id} completed: {result.state.value}")
    ...
    def get_transaction_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            rollbacks = sum(1 for tx in self.transaction_history if tx.rollback_performed)
            
            avg_execution_time = sum(tx.execution_time for tx in self.transaction_history) / total_transactions
            ...
            return {
                ...
                'rollback_rate': rollbacks / total_transactions if total_transactions > 0 else 0,
...
class Transaction:
    ...
    def begin(self, context_data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            logger.debug(f"Transaction {self.metadata.transaction_id} begun with backup {self.transaction_backup_id}")
    ...
    def add_operation(self, operation: ManipulationOperation) -> str:
        ...
        if self.metadata.state != TransactionState.STARTED:
            raise ValueError(f"Cannot add operations to transaction in state {self.metadata.state}")
        ...
        logger.debug(f"Added operation {operation_id} to transaction {self.metadata.transaction_id}")
    ...
    def create_savepoint(self, savepoint_name: str) -> str:
        ...
        if self.metadata.state != TransactionState.STARTED:
            raise ValueError(f"Cannot create savepoint in transaction state {self.metadata.state}")
    ...
    def rollback_to_savepoint(self, savepoint_name: str) -> bool:
        ...
        try:
            ...
            restore_result = self.backup_manager.restore_backup(target_savepoint.context_backup_id)
            if not restore_result.success:
                logger.error(f"Failed to restore from savepoint backup: {restore_result.error_messages}")
                return False
            
            # Restore context data by getting backup entry
            backup_entry = self.backup_manager.get_backup(target_savepoint.context_backup_id)
            if backup_entry:
            ...
            operations_to_remove = len(self.operations) - target_savepoint.operation_count
            if operations_to_remove > 0:
                self.operations = self.operations[:target_savepoint.operation_count]
                logger.info(f"Removed {operations_to_remove} operations after savepoint")
            
            # Remove newer savepoints
            newer_savepoints = [sp for sp in self.savepoints if sp.created_at > target_savepoint.created_at]
    ...
    def execute_operations(self, 
        ...
        if self.metadata.state != TransactionState.STARTED:
            raise ValueError(f"Cannot execute operations in transaction state {self.metadata.state}")
            ...
            for i, tx_operation in enumerate(self.operations):
                try:
                    ...
                    execution_result = {
                        'operation_id': tx_operation.operation_id,
                        'success': True,
                        'tokens_modified': abs(tx_operation.operation.estimated_token_impact),
                    ...
                    logger.debug(f"Executed operation {tx_operation.operation_id} successfully")
                    
                except Exception as e:
                    error_msg = f"Operation {tx_operation.operation_id} execution failed: {e}"
                    tx_operation.execution_error = error_msg
    ...
    def commit(self) -> TransactionResult:
        ...
        if self.metadata.state != TransactionState.STARTED:
            raise ValueError(f"Cannot commit transaction in state {self.metadata.state}")
        ...
        try:
            ...
            completed_ops = sum(1 for op in self.operations if op.execution_result is not None)
            failed_ops = sum(1 for op in self.operations if op.execution_error is not None)
            ...
            if all_succeeded:
                ...
                result = TransactionResult(
                    ...
                    modified_context=deepcopy(self.current_context) if self.current_context else None
                )
                
                logger.info(f"Transaction {self.metadata.transaction_id} committed successfully")
    ...
    def rollback(self) -> TransactionResult:
        ...
        try:
            ...
            if self.transaction_backup_id:
                restore_result = self.backup_manager.restore_backup(self.transaction_backup_id)
                if restore_result.success:
                    # Get the original context back
                    backup_entry = self.backup_manager.get_backup(self.transaction_backup_id)
                    if backup_entry:
                        self.current_context = deepcopy(backup_entry.data)
                    logger.info(f"Successfully restored context from transaction backup")
                else:
                    restore_success = False
                    logger.error(f"Failed to restore from transaction backup: {restore_result.error_messages}")
            ...
            error_messages = [op.execution_error for op in self.operations if op.execution_error]
