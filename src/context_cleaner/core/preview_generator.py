...
from .manipulation_validator import ManipulationValidator, ValidationResult, RiskAssessment, RiskLevel
...
class PreviewGenerator:
    ...
    def __init__(self, validator: Optional[ManipulationValidator] = None, config: Optional[Dict[str, Any]] = None):
        """Initialize preview generator."""
        self.validator = validator or ManipulationValidator()
        self.config = config or {}
        
        # Configuration
        self.max_preview_size = self.config.get('max_preview_size', 10000)  # Max chars to preview
    ...
    def _calculate_change_confidence(self, operation: ManipulationOperation, key: str, context_data: Dict[str, Any]) -> float:
        ...
        try:
            ...
            if key in context_data:
                ...
                if operation.operation_type == "remove":
                    if any(word in content.lower() for word in ['duplicate', 'redundant', 'obsolete']):
    ...
    def _simulate_operation_execution(self, operation: ManipulationOperation, context_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            if operation.operation_type == "remove":
                ...
                if len(operation.target_keys) > 1:
                    ...
                    for key in operation.target_keys:
                        if key in simulated_context:
                            consolidated_content.append(f"{key}: {simulated_context[key]}")
                            del simulated_context[key]
                ...
                for key in operation.target_keys:
                    if key in simulated_context:
                        ...
                        if len(original) > 200:
                            ...
                            summary = original[:100] + "...[summarized]..." + original[-50:]
    ...
    def _generate_change_details(self, 
                                original_context: Dict[str, Any], 
                                modified_context: Dict[str, Any],
                                operation: ManipulationOperation) -> List[ChangeDetail]:
        ...
        for key in all_keys:
            ...
            confidence = self._calculate_change_confidence(operation, key, original_context)
    ...
    def preview_operation(self, 
                         ...
                         include_validation: Optional[bool] = None) -> OperationPreview:
        """Generate preview for a single operation."""
        preview_start = datetime.now()
        
        try:
            # Simulate operation execution
            modified_context = self._simulate_operation_execution(operation, context_data)
            
            # Generate change details
            changes = self._generate_change_details(context_data, modified_context, operation)
            
            # Perform validation if requested
            validation_result = None
            risk_assessment = None
            
            if include_validation or (include_validation is None and self.include_validation):
                if hasattr(self.validator, 'validate_operation_enhanced'):
                    validation_result, risk_assessment = self.validator.validate_operation_enhanced(
                        operation, context_data, enable_risk_assessment=True
                    )
                else:
                    validation_result = self.validator.validate_operation(operation, context_data)
            
            # Calculate impact metrics
            total_size_change = sum(change.size_change for change in changes)
            affected_keys = [change.key for change in changes if change.change_type != ChangeType.UNCHANGED]
            
            estimated_impact = {
                'affected_keys': len(affected_keys),
                'total_size_change': total_size_change,
                'size_reduction_percentage': (abs(total_size_change) / sum(len(str(v)) for v in context_data.values())) * 100 if context_data else 0,
                'risk_score': sum(1 for change in changes if change.risk_level in [RiskLevel.HIGH, RiskLevel.CRITICAL]) / len(changes) if changes else 0
            }
            
            # Generate warnings
            warnings = []
            if total_size_change < 0 and abs(total_size_change) > 1000:
                warnings.append(f"Large content reduction: {abs(total_size_change)} characters")
            
            high_risk_changes = [c for c in changes if c.risk_level in [RiskLevel.HIGH, RiskLevel.CRITICAL]]
    ...
    def preview_plan(self, 
                    plan: ManipulationPlan, 
                    context_data: Dict[str, Any],
                    include_validation: Optional[bool] = None) -> PlanPreview:
        ...
        try:
            ...
            for operation in plan.operations:
                ...
                # Update metrics
                total_changes += len([c for c in op_preview.changes if c.change_type != ChangeType.UNCHANGED])
                total_size_reduction += sum(c.size_change for c in op_preview.changes if c.size_change < 0)
                ...
                cumulative_context = self._simulate_operation_execution(operation, cumulative_context)
            ...
            reduction_percentage = (actual_size_reduction / original_size * 100) if original_size > 0 else 0
            
            summary = {
                'total_operations': len(plan.operations),
                'total_keys_affected': len(set(key for op_preview in operation_previews for change in op_preview.changes for key in [change.key] if change.change_type != ChangeType.UNCHANGED)),
                'original_size': original_size,
                'final_size': final_size,
                'size_reduction': actual_size_reduction,
                'reduction_percentage': reduction_percentage,
                'estimated_execution_time': plan.estimated_execution_time,
                'confidence_score': sum(op.confidence_score for op in plan.operations) / len(plan.operations) if plan.operations else 0,
                'risk_distribution': {
                    'low': sum(1 for op_preview in operation_previews for change in op_preview.changes if change.risk_level == RiskLevel.LOW),
                    'medium': sum(1 for op_preview in operation_previews for change in op_preview.changes if change.risk_level == RiskLevel.MEDIUM),
                    'high': sum(1 for op_preview in operation_previews for change in op_preview.changes if change.risk_level == RiskLevel.HIGH),
                    'critical': sum(1 for op_preview in operation_previews for change in op_preview.changes if change.risk_level == RiskLevel.CRITICAL)
                }
            }
            
            # Determine if confirmation is required
            requires_confirmation = (
                plan.requires_user_approval or
                max_risk_level in [RiskLevel.HIGH, RiskLevel.CRITICAL] or
                reduction_percentage > 50 or
                any("error" in warning.lower() for warning in all_warnings)
            )
    ...
    def generate_diff(self, 
                     original_context: Dict[str, Any], 
                     modified_context: Dict[str, Any],
                     format_type: PreviewFormat = PreviewFormat.TEXT) -> str:
        ...
        try:
            ...
            all_keys = sorted(set(original_context.keys()) | set(modified_context.keys()))
            ...
            if format_type == PreviewFormat.HTML:
                ...
                return html_diff.make_file(original_lines, modified_lines, 
                                         "Original Context", "Modified Context")
    ...
    def _format_text_preview(self, 
        ...
        if isinstance(preview, PlanPreview):
            ...
            lines.append(f"Requires Confirmation: {'YES' if preview.requires_confirmation else 'NO'}")
            lines.append("")
            ...
            if include_details:
                ...
                for i, op_preview in enumerate(preview.operation_previews, 1):
                    lines.append(f"{i}. {op_preview.operation.operation_type.upper()} - {op_preview.operation.operation_id}")
                    lines.append(f"   Target Keys: {', '.join(op_preview.operation.target_keys)}")
    ...
    def _format_markdown_preview(self, 
                               preview: Union[OperationPreview, PlanPreview], 
                               include_details: bool) -> str:
        ...
        if isinstance(preview, PlanPreview):
            ...
            if include_details:
                lines.append("## Operations")
                for i, op_preview in enumerate(preview.operation_previews, 1):
                    lines.append(f"### {i}. {op_preview.operation.operation_type.upper()}")
                    lines.append(f"- **Operation ID:** {op_preview.operation.operation_id}")
                    lines.append(f"- **Target Keys:** {', '.join(op_preview.operation.target_keys)}")
                    lines.append(f"- **Changes:** {len(op_preview.changes)}")
                    if op_preview.warnings:
                        lines.append("- **Warnings:**")
    ...
    def _format_html_preview(self, 
        ...
        if isinstance(preview, PlanPreview):
            ...
            html.append(f"<p><strong>Operations:</strong> {len(preview.operation_previews)}</p>")
            
            risk_class = f"risk-{preview.overall_risk.value}"
            html.append(f"<p><strong>Overall Risk:</strong> <span class='{risk_class}'>{preview.overall_risk.value.upper()}</span></p>")
            
            confirmation_class = "confirmation-required" if preview.requires_confirmation else "no-confirmation"
            html.append(f"<p><strong>Requires Confirmation:</strong> <span class='{confirmation_class}'>{'YES' if preview.requires_confirmation else 'NO'}</span></p>")
            ...
            html.append(f"<p><strong>Operation ID:</strong> {preview.operation.operation_id}</p>")
            html.append(f"<p><strong>Type:</strong> {preview.operation.operation_type}</p>")
            html.append(f"<p><strong>Target Keys:</strong> {', '.join(preview.operation.target_keys)}</p>")
...
if __name__ == "__main__":
    ...
    # Generate diff
    modified_context = generator._simulate_operation_execution(test_operation, test_context)
    diff_output = generator.generate_diff(test_context, modified_context, PreviewFormat.TEXT)
    print("\n--- DIFF OUTPUT ---")
