...
from typing import Dict, Any, List, Tuple, Set, Optional, NamedTuple
...
class PriorityReport:
    ...
    def get_priority_summary(self) -> str:
        ...
        return (f"Priority Alignment: {self.priority_alignment_score}% | "
...
class PriorityAnalyzer:
    ...
    CRITICAL_URGENCY_KEYWORDS = [
        'critical', 'urgent', 'emergency', 'blocker', 'blocking', 'asap', 
        'immediately', 'now', 'crisis', 'broken', 'down', 'failing',
        'deadline today', 'due now', 'overdue'
    ]
    
    HIGH_PRIORITY_KEYWORDS = [
        'high priority', 'important', 'priority', 'needed', 'required',
        'must have', 'essential', 'crucial', 'key', 'main', 'primary',
        'deadline', 'due', 'target', 'milestone', 'goal'
    ]
    
    CURRENT_WORK_KEYWORDS = [
        'current', 'currently', 'working on', 'in progress', 'active',
        'implementing', 'developing', 'building', 'debugging', 'fixing',
        'today', 'this', 'now', 'immediate', 'next'
    ]
    
    LOW_PRIORITY_KEYWORDS = [
        'nice to have', 'optional', 'maybe', 'consider', 'someday',
        'eventually', 'future', 'later', 'low priority', 'if time',
        'bonus', 'enhancement', 'improvement'
    ]
    
    NOISE_KEYWORDS = [
        'random', 'tangent', 'unrelated', 'off topic', 'aside',
        'obsolete', 'deprecated', 'old', 'archived', 'completed',
        'done', 'finished', 'resolved', 'closed'
    ]
    ...
    def __init__(self):
        ...
        self.critical_regex = re.compile('|'.join(self.CRITICAL_URGENCY_KEYWORDS), re.IGNORECASE)
        self.high_priority_regex = re.compile('|'.join(self.HIGH_PRIORITY_KEYWORDS), re.IGNORECASE)
        self.current_work_regex = re.compile('|'.join(self.CURRENT_WORK_KEYWORDS), re.IGNORECASE)
        self.low_priority_regex = re.compile('|'.join(self.LOW_PRIORITY_KEYWORDS), re.IGNORECASE)
        self.noise_regex = re.compile('|'.join(self.NOISE_KEYWORDS), re.IGNORECASE)
        
        self.deadline_patterns = [re.compile(pattern) for pattern in self.DEADLINE_PATTERNS]
        self.dependency_patterns = [re.compile(pattern) for pattern in self.DEPENDENCY_PATTERNS]
    ...
    def _calculate_priority_score(self, content: str, context_position: int, total_items: int) -> Tuple[int, str, str]:
    ...
    def _categorize_by_priority(self, priority_score: int, urgency_level: str, 
                               content: str) -> str:
    ...
    def _analyze_priority_conflicts(self, priority_items: List[PriorityItem]) -> List[Dict[str, Any]]:
    ...
    def _generate_reorder_recommendations(self, priority_items: List[PriorityItem]) -> List[Dict[str, Any]]:
    ...
    def _generate_focus_improvements(self, priority_items: List[PriorityItem], 
                                   priority_alignment_score: int) -> List[str]:
        ...
        if category_counts['noise'] > total_items * 0.2:
            improvements.append(f"Remove {category_counts['noise']} obsolete/noise items to improve focus")
        
        if category_counts['critical'] == 0:
            improvements.append("Consider marking current urgent items as critical priority")
        
        if category_counts['current_work'] < total_items * 0.3:
            improvements.append("Increase focus on current work items - only 30% of context relates to active tasks")
        
        if priority_alignment_score < 60:
            improvements.append("Reorder context to move high-priority items to the top")
        
        blocking_items = [item for item in priority_items if item.category == 'blocking']
        if blocking_items:
            improvements.append(f"Address {len(blocking_items)} blocking items to unblock progress")
        
        return improvements
    
    def _extract_content_items_with_positions(self, context_data: Dict[str, Any]) -> List[Tuple[int, str, str]]:
    ...
    async def analyze_priorities(self, context_data: Dict[str, Any]) -> PriorityReport:
        ...
        try:
            ...
            for position, path, content in content_items:
                ...
                priority_score, urgency_level, impact_level = self._calculate_priority_score(
                ...
                # Categorize the item
                category = self._categorize_by_priority(priority_score, urgency_level, content)
            ...
            critical_items = [item for item in priority_items if item.urgency_level == 'critical']
            high_priority_items = [item for item in priority_items if item.urgency_level == 'high']
            medium_priority_items = [item for item in priority_items if item.urgency_level == 'medium']
            low_priority_items = [item for item in priority_items if item.urgency_level == 'low']
            noise_items = [item for item in priority_items if item.category == 'noise']
            
            # Calculate priority alignment score
            total_items = len(priority_items)
            high_priority_count = len([item for item in priority_items if item.priority_score >= 70])
            top_quarter_count = total_items // 4
            
            # Count high-priority items in top quarter
            sorted_by_position = sorted(priority_items, key=lambda x: int(x.path.split('[')[-1].split(']')[0]) if '[' in x.path and ']' in x.path else 0)
            high_priority_in_top = sum(1 for item in sorted_by_position[:top_quarter_count] if item.priority_score >= 70)
            
            if high_priority_count > 0:
                priority_alignment_score = int((high_priority_in_top / high_priority_count) * 100)
            else:
                priority_alignment_score = 70  # Default if no high priority items
            
            # Calculate other metrics
            current_work_items = [item for item in priority_items if item.category == 'current_work']
            current_work_focus_percentage = (len(current_work_items) / total_items) * 100 if total_items > 0 else 0
            ...
            blocking_items_count = len([item for item in priority_items if item.category == 'blocking'])
            
            # Generate analysis and recommendations
            priority_conflicts = self._analyze_priority_conflicts(priority_items)
            reorder_recommendations = self._generate_reorder_recommendations(priority_items)
            focus_improvement_actions = self._generate_focus_improvements(priority_items, priority_alignment_score)
            
            # Cleanup opportunities
            priority_cleanup_opportunities = []
            if noise_items:
                priority_cleanup_opportunities.append(f"Remove {len(noise_items)} obsolete/noise items")
            
            low_value_items = [item for item in priority_items if item.priority_score < 30]
            if low_value_items:
                priority_cleanup_opportunities.append(f"Consider removing {len(low_value_items)} very low priority items")
            
            items_with_priority_signals = sum(1 for item in priority_items 
            ...
            logger.info(f"Priority analysis completed: {priority_alignment_score}% alignment, "
