...
class CacheInvalidationRequest(BaseModel):
    ...
    pattern: str
    scope: str = Field(
        default="dashboard", pattern="^(dashboard|widgets|telemetry|all)$"
    )
...
def create_error_response(
    ...
    error_data = ErrorDetails(code=error_code, message=message, details=details or {})
    return HTTPException(status_code=status_code, detail=error_data.model_dump())
...
def create_not_found_error(resource: str, identifier: str = None) -> HTTPException:
    ...
    return create_error_response(
        message=message, error_code="RESOURCE_NOT_FOUND", status_code=404
    )
