...
from ..telemetry.jsonl_enhancement.full_content_processor import (
    FullContentBatchProcessor,
)
...
class IncrementalSyncService:
    ...
    def __init__(
        ...
        self.watch_directory = Path(
            watch_directory or Path.home() / ".claude" / "projects"
        )
        ...
        self._executor = ThreadPoolExecutor(
            max_workers=2, thread_name_prefix="jsonl-sync"
        )

        # Metadata for persisted state
        self.state_metadata: Dict[str, Any] = {"context_rot_backfill_complete": False}
        ...
        try:
            if self.bridge_service.clickhouse_client is not None:
                ...
                logger.info(
                    "Context Rot Analyzer initialized for incremental sync service"
                )
        ...
        try:
            ...
            for file_path in jsonl_files:
                ...
                if file_str not in self.file_states:
                    ...
                    if (
                        current_mtime > stored_state.last_modified
                        or current_hash != stored_state.file_hash
                    ):
                        new_files.append(file_str)
    ...
    async def process_file_incremental(self, file_path: str) -> bool:
        ...
        try:
                ...
                for i, line in enumerate(f):
                    if i >= start_line:  # Only process new lines
                        ...
                        try:
                            ...
                            malformed_lines += 1
                            logger.warning(
                                f"Malformed JSON at line {i+1} in {file_path}: {e}"
                            )
                            logger.debug(
                                f"Problematic line content: {line_content[:200]}{'...' if len(line_content) > 200 else ''}"
                            )
                            continue

            if malformed_lines > 0:
                logger.warning(
                    f"Skipped {malformed_lines} malformed JSON lines in {file_path}"
                )
            ...
            for entry in new_lines:
                ...
                if usage_data and isinstance(usage_data, dict):
                    ...
                    cache_creation_tokens = (
                        ...
                        if isinstance(cache_creation_tokens, (int, float))
                        else 0
                    )
                    cache_read_tokens = (
                    ...
                    if actual_tokens > 0:
                        ...
                        logger.debug(
                            f"Extracted {actual_tokens} actual tokens from JSONL entry (input: {input_tokens}, output: {output_tokens}, cache_creation: {cache_creation_tokens}, cache_read: {cache_read_tokens})"
                        )
                    ...
                    logger.debug(
                        f"No usage data found for entry type={entry_type}, role={message_role}"
                    )
            ...
            try:
                processor = FullContentBatchProcessor(
                    self.bridge_service.clickhouse_client
                )
                conversation_stats = await processor.process_jsonl_entries(new_lines)
                conversations_processed = conversation_stats.get(
                    "messages_processed", 0
                )
                logger.info(
                    f"Conversation processing complete: {conversations_processed} messages processed"
                )
            except Exception as e:
            ...
            logger.info(
                f"  Successful token extractions: {successful_extractions}/{len(new_lines)}"
            )
            logger.info(f"  Total tokens extracted: {estimated_tokens}")

            if successful_extractions == 0 and len(new_lines) > 0:
                logger.warning(
                    f"WARNING: No tokens extracted from {len(new_lines)} entries in {file_path}. This may indicate a data structure issue."
                )
    ...
    async def sync_incremental_changes(self) -> int:
        ...
        async with self._sync_lock:
            try:
                ...
                logger.info(
                    f"Incremental sync complete: {total_synced} files processed"
                )
                return total_synced
    ...
    async def _sync_aggregated_data(self):
        ...
        try:
            ...
            analysis = self.bridge_service._convert_dashboard_to_analysis(
                dashboard_data
            )
    ...
    async def start_file_monitoring(self):
        ...
        if self.needs_context_rot_backfill:
            try:
                logger.info(
                    "Running context rot historical backfill before enabling monitoring"
                )
        ...
        if not _WATCHDOG_AVAILABLE:
            logger.warning(
                "Watchdog library not available - using polling mode instead"
            )
    ...
    async def run_scheduled_sync(self, interval_minutes: int = 15):
        ...
        if self.needs_context_rot_backfill:
            try:
                logger.info(
                    "Running context rot historical backfill prior to scheduled sync loop"
                )
                events_generated = await self.backfill_context_rot()
                logger.info(
                    "Context rot historical backfill complete: %s events generated",
    ...
    async def backfill_context_rot(self) -> int:
        ...
        if not self.context_rot_analyzer:
            logger.info(
                "Context Rot Analyzer unavailable; skipping historical backfill"
            )
                    ...
                    with open(file_path, "r", encoding="utf-8") as handle:
                        for line_number, line in enumerate(handle, 1):
                            ...
                            if len(batch) >= batch_size:
                                total_events += await self._process_context_rot_entries(
                                    batch
                                )
                                batch.clear()
...
async def create_incremental_sync_service(
    bridge_service: TokenAnalysisBridgeService, watch_directory: str = None
) -> IncrementalSyncService:
    """Create and initialize incremental sync service."""

    sync_service = IncrementalSyncService(
        bridge_service=bridge_service, watch_directory=watch_directory
    )
