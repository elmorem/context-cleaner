...
class TokenUsageSummaryRecord:
    ...
    def to_clickhouse_values(
        self,
    ) -> Tuple[str, str, List[str], int, float, float, float, float]:
...
class TokenAnalysisBridgeService:
    ...
    async def execute_historical_backfill(
        ...
        try:
            ...
            logger.info(
                "Retrieving comprehensive token analysis from dashboard integration..."
            )

            try:
                from context_cleaner.analysis.dashboard_integration import (
                ...
                logger.info(
                    f"Retrieved dashboard analysis: {dashboard_analysis['total_tokens']:,} total tokens"
                )
                logger.info(f"Files processed: {dashboard_analysis['files_processed']}")
                logger.info(
                    f"Categories found: {len(dashboard_analysis['categories'])}"
                )
            ...
            logger.info(
                f"Retrieved analysis: {analysis.total_sessions_analyzed} sessions, "
            ...
            records = await self._transform_analysis_to_records(
            ...
            if self.clickhouse_client:
                logger.info(
                    f"Inserting {len(records)} records into ClickHouse in batches of {batch_size}..."
                )
            ...
            logger.info(
                f"Historical backfill complete: {self.stats.total_tokens_transferred:,} tokens "
    ...
    async def _transform_analysis_to_records(
        ...
        for session_id, session in analysis.sessions.items():
            ...
            summary["total_tokens"] += (
                session.calculated_total_tokens or session.total_reported_tokens
            )
            ...
            summary["cache_tokens"] += (
                session.reported_cache_creation_tokens
                + session.reported_cache_read_tokens
            )
    ...
    async def _batch_insert_records(
        self, records: List[TokenUsageSummaryRecord], batch_size: int
    ):
        ...
        for i in range(0, len(records), batch_size):
            ...
            try:
                ...
                error_msg = (
                    f"Batch insert failed for records {i}-{i+len(batch)}: {str(e)}"
                )
                logger.error(error_msg)
                self.stats.errors_encountered.append(error_msg)
                raise

    async def _simulate_database_insertion(
        self, records: List[TokenUsageSummaryRecord]
    ):
        ...
        for service, totals in service_totals.items():
            logger.info(
                f"SIMULATION: {service}: {totals['tokens']:,.0f} tokens, {totals['operations']} operations"
            )
        ...
        async with self._sync_lock:
            try:
                ...
                analysis = EnhancedTokenAnalysis(
                    ...
                    total_lines_processed=len(session_metrics.user_messages)
                    + len(session_metrics.assistant_messages),
                ...
                if records and self.clickhouse_client:
                    ...
                    logger.info(
                        f"Synced real-time tokens for session {session_metrics.session_id}"
                    )
                    return True
    ...
    async def validate_data_flow(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if analysis.total_calculated_tokens > 0:
                ...
                logger.info(
                    f"✅ Token analysis working: {analysis.total_calculated_tokens:,} tokens found"
                )
            else:
            ...
            if (
                validation_results["jsonl_analysis_working"]
                and validation_results["database_accessible"]
            ):
    ...
    def _convert_dashboard_to_analysis(
        ...
        for category in categories_data:
            ...
            session = SessionTokenMetrics(
                ...
                content_categories={category["name"]: category.get("file_count", 1)},
            )
        ...
        if not sessions:
            ...
            session = SessionTokenMetrics(
                ...
                reported_cache_creation_tokens=token_breakdown.get(
        ...
        total_calculated = (
            sum(s.calculated_total_tokens for s in sessions.values()) or total_tokens
        )

        accuracy_ratio = (
            total_calculated / total_reported if total_reported > 0 else 1.0
        )
        undercount_percentage = (
            max(0, (total_calculated - total_reported) / total_calculated * 100)
...
async def create_token_bridge_service(
