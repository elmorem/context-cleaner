...
class RealTimeCostMonitor:
    ...
    async def get_current_burn_rate(self, session_id: str) -> BurnRateData:
        ...
        try:
            ...
            duration_hours = max(
                session_duration.total_seconds() / 3600, 0.01
    ...
    def _calculate_cost_acceleration(
        self, session_id: str, current_burn_rate: float
    ) -> float:
        ...
        session_history = [
            h for h in self._cost_history if h["session_id"] == session_id
        ]
    ...
    def _calculate_efficiency_score(
        self, cost: float, api_calls: int, model_breakdown: Dict[str, float]
    ) -> float:
                ...
                if session_usage >= threshold:
                    ...
                    if self._should_send_alert(alert_key):
                        alert = CostAlert(
                            ...
                            session_id=session_id,
                            recommended_action=self._get_session_recommendation(
                                alert_level, session_usage
                            ),
                        )
                        alerts.append(alert)
                ...
                if daily_usage >= threshold:
                    ...
                    if self._should_send_alert(alert_key):
                        alert = CostAlert(
                            ...
                            session_id=session_id,
                            recommended_action=self._get_daily_recommendation(
                                alert_level, daily_usage
                            ),
                        )
                        alerts.append(alert)
                        self._mark_alert_sent(alert_key)
    ...
    async def get_cost_projection(self, session_id: str) -> CostProjection:
        ...
        try:
            ...
            # Project next hour
            next_hour_cost = (
                burn_rate_data.current_cost + burn_rate_data.burn_rate_per_hour
            )

            # Project end of session (assume 2 hours remaining)
            end_session_cost = burn_rate_data.current_cost + (
                burn_rate_data.burn_rate_per_hour * 2
            )
            ...
            confidence = self._calculate_projection_confidence(
                burn_rate_data.cost_acceleration
            )
            ...
            logger.error(
                f"Error generating cost projection for session {session_id}: {e}"
            )
            return CostProjection(
                next_hour_cost=0.0,
