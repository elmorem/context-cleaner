...
class AgentSelector:
    ...
    def __init__(
        self, telemetry_client: ClickHouseClient, workflow_learner: WorkflowLearner
    ):
    ...
    async def select_optimal_agent(
        self, task_context: TaskContext, strategy: SelectionStrategy = None
        ...
        try:
            ...
            logger.info(
                f"Selecting agent for task: {task_context.description[:50]}... using {strategy.value} strategy"
            )
            ...
            optimized_scores = await self._apply_selection_strategy(
                scored_agents, strategy, task_context
            )

            # Step 4: Generate final recommendation
            recommendation = await self._generate_recommendation(
                optimized_scores, task_context
            )
            ...
            logger.info(
                f"Selected agent: {recommendation.primary_agent.value} (confidence: {recommendation.confidence:.2f})"
            )
    ...
    async def batch_select_agents(
        self, task_contexts: List[TaskContext], strategy: SelectionStrategy = None
    ) -> List[SelectionRecommendation]:
        ...
        try:
            ...
            for task_context in task_contexts:
                ...
                temp_workloads[recommendation.primary_agent] = (
                    temp_workloads.get(recommendation.primary_agent, 0) + 1
                )

                recommendations.append(recommendation)

            # Optimize for workflow coherence if needed
            optimized_recommendations = await self._optimize_batch_coherence(
                recommendations, task_contexts
            )
    ...
    async def _get_candidate_agents(self, task_context: TaskContext) -> List[AgentType]:
        ...
        for agent_type, capability in self.agent_capabilities.items():
            ...
            for domain in capability.primary_domains:
                if any(
                    domain in keyword or keyword in domain for keyword in all_keywords
                ):
                    candidates.add(agent_type)
    ...
    async def _calculate_expertise_score(
        self, agent_type: AgentType, task_context: TaskContext
    ) -> float:
        ...
        secondary_matches = sum(
            1
            for domain in capability.secondary_domains
            if any(domain in keyword or keyword in domain for keyword in all_keywords)
        )
        ...
        if task_context.complexity == TaskComplexity.ENTERPRISE and agent_type in [
            ...
            task_context.complexity == TaskComplexity.SIMPLE
            and agent_type == AgentType.GENERAL_PURPOSE
        ):
            score += 0.1
    ...
    async def _calculate_performance_score(
        ...
        try:
            ...
            agent_insights = await self.learner.get_agent_performance_insights(
                agent_type
            )
            ...
            if "error" not in agent_insights:
                ...
                reliability_factor = min(
                    sample_count / 20, 1.0
                )  # Full reliability at 20+ samples
                score = score * reliability_factor + 0.5 * (
                ...
                return self._estimate_performance_from_capability(
                    agent_type, task_context
                )

        except Exception as e:
            logger.error(
                f"Error calculating performance score for {agent_type.value}: {e}"
            )
            return 0.5
    ...
    async def _calculate_cost_score(
        ...
        if (
            task_context.complexity == TaskComplexity.ENTERPRISE
            and capability.cost_per_hour > 3.0
        ):
            base_score += 0.1  # Bonus for high-end agents on complex tasks
        elif (
            task_context.complexity == TaskComplexity.SIMPLE
            and capability.cost_per_hour < 2.5
        ):
    ...
    async def _calculate_availability_score(
        ...
        availability_ratio = (
            max(0, capability.max_concurrent_tasks - current_workload)
        ...
        try:
            ...
            if "error" not in agent_insights:
                ...
                confidence_factors.append(
                    0.3
                )  # Low confidence without performance data
        except:
    ...
    def _generate_score_reasoning(
        ...
        if expertise > 0.7:
            reasoning.append(
                f"Strong expertise match - specializes in {', '.join(capability.primary_domains)}"
            )
        elif expertise < 0.3:
        ...
        if speed > 0.7:
            ...
            reasoning.append(
                "Slower response time - may not be suitable for urgent tasks"
            )
    ...
    def _estimate_performance_from_capability(
        self, agent_type: AgentType, task_context: TaskContext
    ) -> float:
    ...
    async def _apply_selection_strategy(
        self,
        scored_agents: List[SelectionScore],
        strategy: SelectionStrategy,
        task_context: TaskContext,
    ) -> List[SelectionScore]:
        ...
        if strategy == SelectionStrategy.PERFORMANCE_FIRST:
            ...
            for score in scored_agents:
                score.total_score = (
                    score.performance_score * 0.6 + score.total_score * 0.4
                )
            ...
            for score in scored_agents:
                score.total_score = (
                    score.expertise_score * 0.6 + score.total_score * 0.4
                )

        elif strategy == SelectionStrategy.ADAPTIVE:
            # Learn from past successes to adjust weights
            optimized_scores = await self._apply_adaptive_optimization(
                scored_agents, task_context
            )
    ...
    async def _apply_adaptive_optimization(
        self, scored_agents: List[SelectionScore], task_context: TaskContext
    ) -> List[SelectionScore]:
        ...
        if context_key in self.context_patterns:
            ...
            for score in scored_agents:
                if score.agent_type in successful_agents:
                    score.total_score *= (
                        1.2  # 20% boost for historically successful agents
                    )
                    score.reasoning.append(
                        "Historically successful in similar contexts"
                    )
    ...
    async def _generate_recommendation(
        ...
        if primary.confidence < 0.5:
            reasoning_parts.append(
                "Note: Low confidence due to limited historical data"
            )
        ...
        if len(backups) > 0:
            optimization_suggestions.append(
                f"Consider backup options: {backups[0].value}"
            )
        if primary.performance_score < 0.6:
            optimization_suggestions.append(
                "Monitor performance closely and be ready to switch agents"
            )
    ...
    async def _optimize_batch_coherence(
        self,
    ...
    async def _track_selection(
        self, task_context: TaskContext, recommendation: SelectionRecommendation
    ):
        ...
        self.agent_workloads[recommendation.primary_agent] = (
            self.agent_workloads.get(recommendation.primary_agent, 0) + 1
        )
    ...
    async def get_selection_analytics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "most_selected_agent": (
                max(agent_usage.items(), key=lambda x: x[1])[0] if agent_usage else None
            ),
        }
    ...
    async def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        for agent_type in AgentType:
            ...
            if agent_selections:
                ...
                avg_confidence = statistics.mean(
                    [s["confidence"] for s in agent_selections]
                )

                # Mock some additional performance data (would come from actual execution tracking)
                success_rate = min(
                    100.0, 75.0 + avg_confidence * 25
                )  # Scale confidence to success rate
                avg_duration = (
                    5.0 + (1.0 - avg_confidence) * 10.0
                )  # Lower confidence = longer duration
                cost_efficiency = (
                    0.6 + avg_confidence * 0.3
                )  # Higher confidence = better efficiency
