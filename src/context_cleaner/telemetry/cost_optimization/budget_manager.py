...
class BudgetManager:
    ...
    async def get_current_costs(self, session_id: str) -> Dict[str, float]:
        ...
        try:
            ...
            return {"session": session_cost, "daily": daily_cost, "weekly": weekly_cost}
            ...
            suggestions.append(
                OptimizationSuggestion(
                    ...
                    expected_savings_percent=(
                        60.0 if self.config.auto_switch_haiku else None
                    ),
                )
            )
    ...
    async def get_budget_projections(self, session_id: str) -> Dict[str, float]:
        ...
        try:
            ...
            hourly_burn_rate = current_costs["session"] / max(
            ...
            projections = {
                ...
                "projected_daily_total": current_costs["daily"]
                + (hourly_burn_rate * 8),  # Assume 8 more hours
                "projected_session_total": current_costs["session"]
                + (hourly_burn_rate * 2),  # Assume 2 more hours
                "budget_exhaustion_hours": max(
                    0,
                    (self.config.session_limit - current_costs["session"])
                    / max(hourly_burn_rate, 0.01),
                ),
            }
    ...
    async def send_alert(self, alert_type: str, message: str, session_id: str):
        ...
        logger.warning(
            f"BUDGET ALERT [{alert_type}]: {message} (Session: {session_id})"
        )
    ...
    async def get_cost_efficiency_report(self, session_id: str) -> Dict[str, any]:
        ...
        try:
            ...
            cost_per_api_call = current_costs["session"] / max(
                session_metrics.api_calls, 1
            )
            cost_per_token = current_costs["session"] / max(
                session_metrics.total_input_tokens, 1
            )
            ...
            for model, stats in model_stats.items():
                if stats["request_count"] > 0:
                    model_efficiency[model] = {
                        ...
                        "efficiency_score": (1000 / max(stats["avg_duration_ms"], 1))
                        * (1 / max(stats["cost_per_token"], 0.001)),
                    }
            ...
            return {
                ...
                "budget_status": {
                    "session_budget_used": (
                        current_costs["session"] / self.config.session_limit
                    )
                    * 100,
                    "daily_budget_used": (
                        current_costs["daily"] / self.config.daily_limit
                    )
                    * 100,
                    "remaining_session_budget": max(
                        0, self.config.session_limit - current_costs["session"]
                    ),
                    "remaining_daily_budget": max(
                        0, self.config.daily_limit - current_costs["daily"]
                    ),
                },
                "model_efficiency": model_efficiency,
                "projections": projections,
                "recommendations": await self._generate_efficiency_recommendations(
                    session_metrics, model_stats
                ),
            }
    ...
    async def _generate_efficiency_recommendations(
        self, session_metrics, model_stats
    ) -> List[str]:
        ...
        if session_metrics.total_cost / max(session_metrics.api_calls, 1) > 0.03:
            recommendations.append(
                "Consider using Haiku model for routine tasks to reduce cost per API call"
            )

        # Token efficiency recommendation
        avg_input_tokens = session_metrics.total_input_tokens / max(
            session_metrics.api_calls, 1
        )
        if avg_input_tokens > 2000:
            recommendations.append(
                "Large context sizes detected - consider breaking down requests for better efficiency"
            )
        ...
        if total_requests > 0:
            ...
            if sonnet_percentage > 0.7:  # More than 70% Sonnet usage
                recommendations.append(
                    "High Sonnet usage detected - Haiku could handle many tasks at 60-80% cost savings"
                )
