...
class CostOptimizationEngine:
    ...
    async def should_use_haiku(self, task_description: str, session_id: str) -> bool:
        ...
        try:
            ...
            if (
                ...
                in [TaskComplexity.SIMPLE, TaskComplexity.MODERATE]
                and task_analysis.is_routine
            ):
                return True
            ...
            session_percentage = (
                current_costs["session"] / self.budget_config.session_limit
            )
            ...
            if (
                task_analysis.estimated_tokens > 2000
                and not task_analysis.requires_precision
            ):
                return True
    ...
    async def get_model_recommendation(
        ...
        try:
            ...
            if model == ModelType.HAIKU:
                sonnet_cost = self._estimate_cost(
                    ModelType.SONNET_4, task_analysis.estimated_tokens
                )
    ...
    async def analyze_task_complexity(self, task_description: str) -> TaskAnalysis:
        ...
        requires_precision = any(
            keyword in desc_lower for keyword in precision_keywords
        )
    ...
    def _get_accurate_token_count(self, text: str) -> int:
        ...
        try:
            ...
            try:
                ...
                if not api_key:
                    logger.debug(
                        "No ANTHROPIC_API_KEY found in config, using fallback estimation"
                    )
    ...
    def _generate_haiku_reasoning(
        self, task_analysis: TaskAnalysis, session_id: str
    ) -> str:
    ...
    def _generate_sonnet_reasoning(self, task_analysis: TaskAnalysis) -> str:
        ...
        if task_analysis.complexity in [
            TaskComplexity.COMPLEX,
            TaskComplexity.CREATIVE,
        ]:
            reasons.append(
                f"{task_analysis.complexity.value} task benefits from advanced reasoning"
            )
    ...
    async def get_session_analysis(self, session_id: str) -> CostAnalysis:
        ...
        try:
            ...
            daily_avg = (
                1.50  # Default average, could be calculated from historical data
            )
            session_avg = 1.00

            # Calculate vs averages
            vs_daily_avg = (
                ((current_costs["daily"] - daily_avg) / daily_avg) * 100
                if daily_avg > 0
                else 0
            )
            vs_session_avg = (
                ((current_costs["session"] - session_avg) / session_avg) * 100
                if session_avg > 0
                else 0
            )
            ...
            return CostAnalysis(
                ...
                budget_remaining=max(
                    0, self.budget_config.session_limit - current_costs["session"]
                ),
            )
    ...
    async def get_optimization_suggestions(
        ...
        try:
            ...
            budget_suggestions = await self.budget_manager.check_budget_status(
                session_id
            )
            ...
            if session_metrics:
                ...
                avg_tokens = session_metrics.total_input_tokens / max(
    ...
    async def get_usage_patterns(self, session_id: str) -> UsagePattern:
        ...
        try:
            ...
            for model, stats in model_stats.items():
                ...
                preferred_models[model_type] += stats["request_count"] / max(
                    total_requests, 1
                )
            ...
            if (
                session_metrics
                and session_metrics.start_time
                and session_metrics.end_time
            ):
