...
class SearchResults:
    ...
    def get_top_files(self, limit: int = 5) -> List[str]:
        ...
        sorted_results = sorted(
            self.results, key=lambda r: r.relevance_score, reverse=True
        )
    ...
    def get_files_by_extension(self, extension: str) -> List[str]:
        ...
        return [
            r.file_path for r in self.results if r.file_path.endswith(f".{extension}")
        ]
...
class AdvancedSearchEngine:
    ...
    def __init__(
        self, telemetry_client: ClickHouseClient, workspace_root: Optional[Path] = None
    ):
        ...
        try:
            ...
            if progressive and keyword_results:
                ...
                related_files = await self._find_related_files(
                    [r.file_path for r in keyword_results]
                )
            ...
            if progressive:
                ...
                pattern_results = await self._pattern_search(
                    query, results.get_top_files(10)
                )
                ...
                for result in pattern_results:
                    ...
                    existing = next(
                        (r for r in results.results if r.file_path == result.file_path),
                        None,
                    )
                    if existing:
            ...
            results.suggested_actions = await self._generate_action_suggestions(
                query, results
            )
            ...
            logger.info(
                f"Deep search completed: {results.total_matches} results in {search_duration:.2f}s"
            )
            return results
    ...
    async def _keyword_search(
        self, query: str, context_files: Optional[List[str]] = None
    ) -> List[SearchResult]:
        ...
        try:
            ...
            for result in results:
                ...
                if (
                    ...
                    or result.relevance_score > unique_results[key].relevance_score
                ):
    ...
    def _simulate_file_search(
        self, term: str, context_files: Optional[List[str]] = None
    ) -> List[SearchResult]:
        ...
        for file_path in search_files[:5]:  # Limit for demo
            ...
            result = SearchResult(
                ...
                context_lines=[
                    f"  def function_with_{term}():",
                    f"      return {term}_value",
                ],
    ...
    def _should_refresh_relationships(self) -> bool:
        ...
        return (
            not self._relationship_cache_time
            or datetime.now() - self._relationship_cache_time > self._cache_ttl
        )
    ...
    async def _contextual_search(
        self, query: str, related_files: List[str]
    ) -> List[SearchResult]:
    ...
    async def _pattern_search(
        self, query: str, target_files: List[str]
    ) -> List[SearchResult]:
    ...
    async def _generate_action_suggestions(
        self, query: str, results: SearchResults
    ) -> List[str]:
        ...
        if best_pattern:
            next_tools = self._get_next_tools_in_pattern(
                best_pattern, ["Grep"]
            )  # Assume we just did search
            for tool in next_tools[:3]:
                if tool == "Read":
                    suggestions.append(
                        f"Read top result: {results.get_top_files(1)[0] if results.results else 'file'}"
                    )
                elif tool == "Edit":
    ...
    def _find_best_workflow_pattern(
        self, query: str, results: SearchResults
        ...
        for pattern in self.workflow_patterns:
            ...
            for result in results.results:
                file_ext = (
                    result.file_path.split(".")[-1] if "." in result.file_path else ""
                )
    ...
    def _get_next_tools_in_pattern(
        self, pattern: WorkflowPattern, current_tools: List[str]
    ) -> List[str]:
    ...
    def _calculate_search_confidence(self, results: SearchResults) -> float:
        ...
        avg_relevance = sum(r.relevance_score for r in results.results) / len(
            results.results
        )
    ...
    async def get_search_analytics(self) -> Dict[str, Any]:
        ...
        avg_confidence = (
            sum(s["confidence"] for s in self._search_history) / total_searches
        )
        avg_results = (
            sum(s["result_count"] for s in self._search_history) / total_searches
        )
