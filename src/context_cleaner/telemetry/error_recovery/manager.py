...
from .exceptions import (
    MaxRetriesExceeded,
    NoViableStrategyError,  # Legacy compatibility
    create_max_retries_exceeded_error,
    create_no_viable_strategy_error,  # Modern HTTPException pattern
)
...
class ErrorRecoveryManager:
    ...
    async def handle_api_error(
        self, error_type: str, context: RequestContext
    ) -> RecoveryResult:
        """
        ...
        """
        logger.info(
            f"Handling API error: {error_type} for session {context.session_id}"
        )
        ...
        applicable_strategies = self._get_applicable_strategies(
            error_type, context, error_pattern
        )
        ...
        for attempt, strategy in enumerate(
            applicable_strategies[: self.max_retries], 1
        ):
    ...
    async def _analyze_error_pattern(self, error_type: str) -> ErrorPattern:
        ...
        if (
            error_type in self._error_pattern_cache
    ...
    def _get_applicable_strategies(
        self, error_type: str, context: RequestContext, pattern: ErrorPattern
        ...
        for strategy in self.strategies:
            if strategy.is_applicable(error_type, context):
                ...
                logger.debug(
                    f"Strategy {strategy.name} is applicable for error: {error_type}"
                )

        return applicable

    async def _log_recovery_success(
        self, error_type: str, strategy_name: str, context: RequestContext
    ):
        """Log successful recovery for telemetry."""
        logger.info(
            f"Recovery success: {error_type} -> {strategy_name} (session: {context.session_id})"
        )
        # Could send this to telemetry system for tracking recovery effectiveness

    async def _log_recovery_failure(
        self, error_type: str, strategies_tried: List[str], context: RequestContext
    ):
        """Log recovery failure for telemetry."""
        logger.error(
            f"Recovery failure: {error_type} after trying {strategies_tried} (session: {context.session_id})"
        )
    ...
    async def get_recovery_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "most_common_error": (
                    max(error_types.items(), key=lambda x: x[1]["count"])[0]
                    if error_types
                    else None
                ),
            }
