...
class SecurityConfig:
    ...
    dangerous_content_patterns: List[str] = field(
...
class AnalysisEngineConfig:
    ...
    health_thresholds: Dict[str, int] = field(
        default_factory=lambda: {"excellent": 90, "good": 70, "fair": 50}
    )
...
class ApplicationConfig:
    ...
    data_directory: str = field(
        default_factory=lambda: str(Path.home() / ".context_cleaner" / "data")
    )
    ...
    def _validate_configuration(self):
        ...
        if not 0.0 <= self.ml.sentiment_confidence_threshold <= 1.0:
            raise ValueError(
                "sentiment_confidence_threshold must be between 0.0 and 1.0"
            )

        if not 0.0 <= self.ml.frustration_confidence_threshold <= 1.0:
            raise ValueError(
                "frustration_confidence_threshold must be between 0.0 and 1.0"
            )
        ...
        if (
            self.alerting.warning_alert_threshold
            >= self.alerting.critical_alert_threshold
        ):
            raise ValueError(
                "warning_alert_threshold must be less than critical_alert_threshold"
            )
    ...
    def from_env(cls) -> "ApplicationConfig":
        ...
        env_name = (
            os.getenv("CONTEXT_CLEANER_ENVIRONMENT")
            or os.getenv("CONTEXT_ROT_ENVIRONMENT")
        ...
        log_level = (
            os.getenv("CONTEXT_CLEANER_LOG_LEVEL")
            or os.getenv("CONTEXT_ROT_LOG_LEVEL")
        ...
        config.enable_debug_mode = (
            os.getenv("CONTEXT_CLEANER_DEBUG")
            or os.getenv("CONTEXT_ROT_DEBUG")
            or os.getenv("DEBUG")
            or "false"
        ).lower() == "true"
        ...
        # Database configuration
        config.database.clickhouse_host = os.getenv(
            "CLICKHOUSE_HOST", config.database.clickhouse_host
        )
        config.database.clickhouse_port = int(
            os.getenv("CLICKHOUSE_PORT", str(config.database.clickhouse_port))
        )
        config.database.clickhouse_database = os.getenv(
            "CLICKHOUSE_DATABASE", config.database.clickhouse_database
        )
        config.database.clickhouse_username = os.getenv(
            "CLICKHOUSE_USERNAME", config.database.clickhouse_username
        )
        config.database.clickhouse_password = os.getenv(
            "CLICKHOUSE_PASSWORD", config.database.clickhouse_password
        )

        # API configuration
        config.api.host = os.getenv("CONTEXT_CLEANER_API_HOST", config.api.host)
        config.api.port = int(
            os.getenv("CONTEXT_CLEANER_API_PORT", str(config.api.port))
        )
        config.api.redis_url = os.getenv("REDIS_URL", config.api.redis_url)
        config.api.enable_websockets = (
            os.getenv("CONTEXT_CLEANER_ENABLE_WEBSOCKETS", "true").lower() == "true"
        )

        # Dashboard configuration
        config.dashboard.host = os.getenv("CONTEXT_CLEANER_HOST", config.dashboard.host)
        config.dashboard.port = int(
            os.getenv("CONTEXT_CLEANER_PORT", str(config.dashboard.port))
        )
        config.dashboard.auto_open_browser = (
            os.getenv("CONTEXT_CLEANER_AUTO_BROWSER", "true").lower() == "true"
        )

        # Privacy configuration
        config.privacy.local_only = (
            os.getenv("CONTEXT_CLEANER_LOCAL_ONLY", "true").lower() == "true"
        )
        config.privacy.enable_telemetry = (
            os.getenv("CLAUDE_CODE_ENABLE_TELEMETRY", "0") == "1"
        )

        # Performance configuration
        config.performance.max_analysis_latency_ms = int(
            os.getenv(
                "CONTEXT_ROT_MAX_LATENCY_MS",
                str(config.performance.max_analysis_latency_ms),
            )
        )
        config.performance.max_memory_usage_mb = int(
            os.getenv(
                "CONTEXT_ROT_MAX_MEMORY_MB", str(config.performance.max_memory_usage_mb)
            )
        )

        # Retention configuration
        retention_policy = os.getenv(
            "CONTEXT_ROT_RETENTION_POLICY", config.retention.retention_policy.value
        )
        try:
            config.retention.retention_policy = RetentionPolicy(retention_policy)
        except ValueError:
            logger.warning(
                f"Invalid retention policy '{retention_policy}', using default"
            )

        config.retention.enable_automatic_cleanup = (
            os.getenv("CONTEXT_ROT_AUTO_CLEANUP", "true").lower() == "true"
        )
        config.retention.cleanup_interval_hours = int(
            os.getenv(
                "CONTEXT_ROT_CLEANUP_INTERVAL_HOURS",
                str(config.retention.cleanup_interval_hours),
            )
        )

        # External services configuration
        config.external_services.anthropic_api_key = os.getenv(
            "ANTHROPIC_API_KEY", config.external_services.anthropic_api_key
        )
        config.external_services.openai_api_key = os.getenv(
            "OPENAI_API_KEY", config.external_services.openai_api_key
        )
        config.external_services.anthropic_timeout_seconds = int(
            os.getenv(
                "ANTHROPIC_TIMEOUT_SECONDS",
                str(config.external_services.anthropic_timeout_seconds),
            )
        )
        config.external_services.max_retries = int(
            os.getenv(
                "EXTERNAL_API_MAX_RETRIES", str(config.external_services.max_retries)
            )
        )
        config.external_services.rate_limit_requests_per_minute = int(
            os.getenv(
                "EXTERNAL_API_RATE_LIMIT_RPM",
                str(config.external_services.rate_limit_requests_per_minute),
            )
        )
        ...
        for flag_name in config.feature_flags:
            env_var = (
                f'CONTEXT_CLEANER_ENABLE_{flag_name.upper().replace("ENABLE_", "")}'
            )
            legacy_env_var = (
                f'CONTEXT_ROT_ENABLE_{flag_name.upper().replace("ENABLE_", "")}'
            )
            if env_var in os.environ:
                ...
                config.feature_flags[flag_name] = (
                    os.getenv(legacy_env_var, "false").lower() == "true"
                )
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "ApplicationConfig":
        ...
        if "analysis" in data:
            ...
            if "token_estimation_factor" in analysis_data:
                config.analysis.token_estimation_factor = analysis_data[
                    "token_estimation_factor"
                ]
            if "circuit_breaker_threshold" in analysis_data:
                config.analysis.circuit_breaker_threshold = analysis_data[
        ...
        if "dashboard" in data:
            ...
            if "max_concurrent_users" in dashboard_data:
                config.dashboard.max_concurrent_users = dashboard_data[
        ...
        if "tracking" in data:
            ...
            if "session_timeout_minutes" in tracking_data:
                config.tracking.session_timeout_minutes = tracking_data[
                    "session_timeout_minutes"
                ]
            if "data_retention_days" in tracking_data:
                config.tracking.data_retention_days = tracking_data[
                    "data_retention_days"
                ]
...
class ConfigManager:
    ...
    def load_config(
        ...
        logger.info(
            f"Configuration loaded for {self._config.environment.value} environment"
        )
        return self._config
    ...
    def update_config(self, updates: Dict[str, Any]) -> None:
        ...
        for section, values in updates.items():
            if hasattr(self._config, section):
                ...
                if hasattr(section_obj, "__dataclass_fields__") and isinstance(
                    values, dict
                ):
                    for field_name, field_value in values.items():
                        if hasattr(section_obj, field_name):
