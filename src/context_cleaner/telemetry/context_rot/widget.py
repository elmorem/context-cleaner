...
class ContextRotWidget:
    ...
    def __init__(
        ...
        context_rot_analyzer: ContextRotAnalyzer,
    ):
        """
    ...
    async def get_widget_data(
        self, session_id: Optional[str] = None, time_window_minutes: int = 30
    ):
        """
        ...
        try:
            ...
            if session_id:
                ...
                context_rot_data = await self._get_session_context_rot(
                    session_id, time_window_minutes
                )
                title = f"Context Rot - Session {session_id[:8]}..."
            else:
                # Global analysis across all recent sessions
                context_rot_data = await self._get_global_context_rot(
                    time_window_minutes
                )
            ...
            logger.info(
                f"Context rot widget data generated in {analysis_latency:.1f}ms"
            )
            return widget_data
    ...
    async def _get_session_context_rot(
        self, session_id: str, time_window_minutes: int
    ) -> ContextRotMeterData:
        """Get context rot data for a specific session."""
        try:
            # Get session health analysis
            health_analysis = await self.analyzer.analyze_session_health(
                session_id, time_window_minutes
            )
            ...
            return ContextRotMeterData(
                ...
                analysis_latency_ms=float(
                    system_metrics.get("analysis_latency_ms", 0.0)
                ),
    ...
    async def _get_global_context_rot(
        ...
        try:
            ...
            health_status = (
                "healthy"
                if avg_rot < 0.3
                else "degraded" if avg_rot < 0.7 else "critical"
            )
            ...
            if len(hourly_results) >= 3:
                recent_avg = (
                    sum(float(row["avg_rot"]) for row in hourly_results[-3:]) / 3
                )
                earlier_avg = (
                    sum(float(row["avg_rot"]) for row in hourly_results[:3]) / 3
                )
                trend = (
                    ...
                    else "degrading" if recent_avg > earlier_avg else "stable"
                )
            else:
                trend = "stable"
    ...
    def _generate_global_recommendations(self, data: Dict[str, Any]) -> List[str]:
        ...
        if avg_rot > 0.7:
            recommendations.append(
                "High global context rot detected - consider session management best practices"
            )
        elif avg_rot > 0.5:
            recommendations.append(
                "Moderate global context rot - monitor active sessions closely"
            )

        if max_rot > 0.8:
            recommendations.append(
                f"Some sessions showing critical rot levels (max: {max_rot:.2f})"
            )

        if total_alerts > 10:
            recommendations.append(
                f"Multiple attention alerts ({total_alerts}) - review session patterns"
            )

        if active_sessions > 50:
            recommendations.append(
                f"High session volume ({active_sessions}) may impact analysis accuracy"
            )

        if not recommendations:
            recommendations.append(
                "Global context rot levels are within healthy ranges"
            )
    ...
    def _generate_alerts(self, context_rot_data: ContextRotMeterData) -> List[str]:
        ...
        if context_rot_data.current_rot_score >= 0.8:
            alerts.append(
                f"CRITICAL: High context rot detected ({context_rot_data.current_rot_score:.2f})"
            )
        elif context_rot_data.current_rot_score >= 0.6:
            alerts.append(
                f"WARNING: Elevated context rot ({context_rot_data.current_rot_score:.2f})"
            )

        if context_rot_data.attention_alerts > 5:
            alerts.append(
                f"Multiple attention events ({context_rot_data.attention_alerts})"
            )

        if (
            context_rot_data.confidence_level < 0.3
            and context_rot_data.measurements_count > 5
        ):
    ...
    async def get_widget_status_summary(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "clickhouse_healthy": analyzer_status.get("clickhouse_connection")
                == "healthy",
                "cache_entries": len(
                    [k for k in self._cache.keys() if ":timestamp" not in k]
                ),
