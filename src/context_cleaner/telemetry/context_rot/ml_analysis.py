...
class SentimentPipeline:
    ...
    def _determine_sentiment(
        self, frustration: float, confusion: float, positive: float, text: str
    ) -> Tuple[SentimentScore, float, List[str]]:
        ...
        if positive > max(frustration, confusion) and positive > 0.3:
            ...
            evidence.append(
                f"Frustration indicators detected (score: {frustration:.2f})"
            )
        elif confusion > 0.3:
            ...
            evidence.append(
                f"Negative sentiment detected (frustration: {frustration:.2f}, confusion: {confusion:.2f})"
            )
        else:
        ...
        if confidence < self.confidence_threshold:
            sentiment = SentimentScore.NEUTRAL
            evidence.append(
                f"Low confidence ({confidence:.2f}) - defaulting to neutral"
            )
...
class ConversationFlowAnalyzer:
    ...
    def _generate_flow_evidence(
        ...
        if question_ratio > 0.4:
            evidence.append(
                f"High question ratio ({question_ratio:.2f}) suggests confusion"
            )
        elif question_ratio < 0.1:
            evidence.append(
                f"Low question ratio ({question_ratio:.2f}) might indicate disengagement"
            )
...
class MLFrustrationDetector:
    ...
    async def analyze_user_sentiment(self, messages: List[str]) -> FrustrationAnalysis:
        ...
        flow_analysis = await self.conversation_analyzer.analyze_flow(
            messages_to_process
        )
    ...
    def _aggregate_sentiment_analysis(
        self, sentiment_results: List[SentimentResult], flow_analysis: ConversationFlow
    ) -> FrustrationAnalysis:
        ...
        if flow_analysis.escalation_detected:
            avg_confidence = min(
                1.0, avg_confidence * 1.2
            )  # Higher confidence if escalation detected
    ...
    def _calculate_frustration_level(
        self,
        sentiment_breakdown: Dict[SentimentScore, float],
