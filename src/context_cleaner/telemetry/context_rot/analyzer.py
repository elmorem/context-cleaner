...
try:
    ...
    class MLFrustrationDetector:
        async def analyze_user_sentiment(self, messages):
            return None

    class AdaptiveThresholdManager:
        async def get_personalized_thresholds(self, user_id):
            return None
...
class ContextRotAnalyzer:
    ...
    def __init__(
        self, clickhouse_client: ClickHouseClient, error_manager: "ErrorRecoveryManager"
    ):
        ...
        self.monitor = ProductionReadyContextRotMonitor(
            clickhouse_client, error_manager
        )
        ...
        if self.ml_enabled:
            ...
            self.adaptive_threshold_manager = AdaptiveThresholdManager()
            logger.warning(
                "Context Rot Analyzer initialized with ML capabilities disabled (fallback mode)"
            )
    ...
    async def analyze_session_health(
        ...
        try:
            ...
            params = {"session_id": session_id, "time_window": time_window_minutes}
            ...
            return {
                ...
                "status": (
                    "healthy" if float(data["avg_rot_score"]) < 0.5 else "degraded"
                ),
                "metrics": {
    ...
    def _generate_recommendations(self, metrics_data: Dict[str, Any]) -> List[str]:
        ...
        if avg_rot > 0.7:
            recommendations.append(
                "High context rot detected. Consider starting a fresh session."
            )
        elif avg_rot > 0.5:
            recommendations.append(
                "Moderate context rot detected. Review recent conversation for repetitive patterns."
            )

        if max_rot > 0.8:
            recommendations.append(
                "Context rot spike detected. Check for circular conversation patterns."
            )

        if alerts > 3:
            recommendations.append(
                f"Multiple attention alerts ({alerts}). Session may benefit from refocusing or restart."
            )

        if not recommendations:
            recommendations.append(
                "Session health looks good. Continue with current approach."
            )
        ...
        try:
            ...
            return {
                "status": (
                    ...
                    and system_metrics.get("circuit_breaker", {}).get(
                        "uptime_ok", False
                    )
                    else "degraded"
                ),
                "clickhouse_connection": (
                    "healthy" if clickhouse_healthy else "unavailable"
                ),
                ...
                "components": {
                    ...
                    "ml_frustration_detector": (
                        "active" if self.ml_enabled else "fallback"
                    ),
    ...
    async def analyze_conversation_sentiment(
        ...
        try:
            ...
            frustration_analysis = (
                await self.ml_frustration_detector.analyze_user_sentiment(messages)
            )
            ...
            thresholds = (
                await self.adaptive_threshold_manager.get_personalized_thresholds(
                    user_id
                )
            )
            ...
            return {
                ...
                "analysis": {
                    ...
                    "sentiment_breakdown": {
                        str(k): v
                        for k, v in frustration_analysis.sentiment_breakdown.items()
                    },
                ...
                "personalization": {
                    "warning_threshold": (
                        thresholds.warning_threshold if thresholds else 0.5
                    ),
                    "critical_threshold": (
                        thresholds.critical_threshold if thresholds else 0.7
                    ),
                    "confidence_required": (
                        thresholds.confidence_required if thresholds else 0.7
                    ),
                },
                "alert_level": alert_level,
                "recommendations": self._generate_ml_recommendations(
                    frustration_analysis, alert_level
                ),
                "version": "2.0.0-ml",
            }
    ...
    async def get_personalized_insights(self, user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            baseline = await self.adaptive_threshold_manager.user_baseline_tracker.get_user_baseline(
                user_id
            )
            ...
            if baseline:
                ...
                insights["recommendations"] = (
                    self._generate_personalized_recommendations(baseline, thresholds)
                )
    ...
    async def update_user_feedback(
        self, user_id: str, feedback_type: str, session_context: Dict[str, Any] = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            update_success = (
                await self.adaptive_threshold_manager.update_user_sensitivity(
    ...
    def _determine_alert_level(
        ...
        if (
            frustration_level >= thresholds.critical_threshold
            and confidence >= thresholds.confidence_required
        ):
            return "critical"
        elif (
            frustration_level >= thresholds.warning_threshold
            and confidence >= thresholds.confidence_required
        ):
            return "warning"
        else:
    ...
    def _generate_ml_recommendations(
        self, analysis: FrustrationAnalysis, alert_level: str
    ) -> List[str]:
        ...
        if alert_level == "critical":
            recommendations.append(
                "High frustration detected - consider taking a break or trying a different approach"
            )

            if analysis.conversation_patterns.get("escalation_detected"):
                recommendations.append(
                    "Conversation escalation pattern identified - session restart may help"
                )

            if analysis.conversation_patterns.get("repetition_ratio", 0) > 0.4:
                recommendations.append(
                    "High repetition detected - try rephrasing your questions or approach"
                )

        elif alert_level == "warning":
            recommendations.append(
                "Elevated frustration detected - monitoring situation"
            )

            if analysis.conversation_patterns.get("flow_quality_score", 1.0) < 0.5:
                recommendations.append(
                    "Poor conversation flow - try being more specific with questions"
                )
        ...
        if question_ratio > 0.5:
            recommendations.append(
                "Many questions detected - you might benefit from more direct statements"
            )
        elif question_ratio < 0.1:
            recommendations.append(
                "Few questions asked - asking clarifying questions might help"
            )

        if not recommendations:
            recommendations.append(
                "Conversation quality looks good - continue with current approach"
            )

        return recommendations[:5]  # Limit to top 5 recommendations

    def _generate_personalized_recommendations(
        self, baseline: "UserBaseline", thresholds: ThresholdConfig
    ) -> List[str]:
        ...
        if baseline.normal_level > 0.4:
            recommendations.append(
                "You tend to experience higher frustration - consider shorter sessions"
            )
        elif baseline.normal_level < 0.2:
            recommendations.append(
                "Your frustration levels are typically low - current approach is working well"
            )

        if baseline.variance > 0.3:
            recommendations.append(
                "Your frustration levels vary significantly - adaptive thresholds are especially beneficial for you"
            )

        if baseline.avg_session_length > 60:
            recommendations.append(
                "You prefer longer sessions - consider periodic breaks to maintain effectiveness"
            )
        elif baseline.avg_session_length < 20:
            recommendations.append(
                "You prefer shorter sessions - this focused approach seems to work well for you"
            )

        # Threshold-based recommendations
        if thresholds.warning_threshold < 0.3:
            recommendations.append(
                "You have sensitive frustration detection - this helps catch issues early"
            )
        elif thresholds.warning_threshold > 0.6:
            recommendations.append(
                "You have higher frustration tolerance - alerts focus on significant issues"
            )

        if baseline.session_count < 10:
            recommendations.append(
                f"Building your profile ({baseline.session_count} sessions) - accuracy will improve with more usage"
            )
