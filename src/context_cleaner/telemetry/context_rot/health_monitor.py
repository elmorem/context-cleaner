...
class MetricCollector:
    ...
    def get_metric_summary(
        self, name: str, time_window_minutes: int = 5
    ) -> Dict[str, Any]:
        ...
        return {
            ...
            "total": (
                sum(values)
                if relevant_metrics[0].metric_type == MetricType.COUNTER
                else None
            ),
        }
...
class ComponentHealthMonitor:
    ...
    def heartbeat(self) -> None:
        ...
        self.metric_collector.increment_counter(
            "component_heartbeat", {"component": self.component_name}
        )
    ...
    def record_error(self, error_message: str = "") -> None:
        ...
        self.error_count += 1
        self.metric_collector.increment_counter(
            "component_errors", {"component": self.component_name}
        )
    ...
    def record_warning(self, warning_message: str = "") -> None:
        ...
        self.metric_collector.increment_counter(
            "component_warnings", {"component": self.component_name}
        )
    ...
    def record_operation(
        ...
        self.metric_collector.record_timer(
            f"component_operation_duration", duration_ms, labels
        )
...
class AlertManager:
    ...
    def create_alert(
        ...
        # Record alert metric
        self.metric_collector.increment_counter(
            "alerts_created", {"severity": severity, "component": component}
        )
    ...
    def get_active_alerts(
        self, component: str = None, severity: str = None
    ) -> List[Alert]:
...
class ContextRotHealthMonitor:
    ...
    def get_health_snapshot(self) -> SystemHealthSnapshot:
        ...
        # Performance summary
        performance_summary = {
            "analysis_latency": self.metric_collector.get_metric_summary(
                "component_operation_duration"
            ),
            "error_rate": self.metric_collector.get_metric_summary("component_errors"),
            "operations_per_minute": self.metric_collector.get_metric_summary(
    ...
    def _setup_default_alert_handlers(self) -> None:
        ...
        def log_alert_handler(alert: Alert) -> None:
            ...
            logger.log(
                level,
                f"ALERT [{alert.severity.upper()}] {alert.component}: {alert.title} - {alert.message}",
            )
    ...
    def export_metrics_prometheus(self) -> str:
        ...
        for metric in self.metric_collector.get_recent_metrics(minutes=60):
            ...
            lines.append(
                f"{metric.name}{labels_str} {metric.value} {int(metric.timestamp.timestamp() * 1000)}"
            )
