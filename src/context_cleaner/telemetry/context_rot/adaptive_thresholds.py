...
class UserBaselineTracker:
    ...
    def __init__(self, clickhouse_client: ClickHouseClient):
        ...
        self.min_sessions_for_baseline = (
            5  # Minimum sessions needed for reliable baseline
        )
    ...
    async def _create_initial_baseline(self, user_id: str) -> Optional[UserBaseline]:
        ...
        try:
            ...
            logger.info(
                f"Created initial baseline for user {user_id} with {session_count} sessions"
            )
            return baseline
    ...
    async def _update_user_baseline(self, baseline: UserBaseline) -> UserBaseline:
        ...
        try:
            ...
            results = await self.clickhouse.execute_query(
                query, {"user_id": baseline.user_id, "since_date": since_date}
            )
            ...
            updated_baseline = UserBaseline(
                ...
                normal_level=baseline.normal_level * weight_historical
                + recent_avg * weight_recent,
                variance=baseline.variance * weight_historical
                + recent_variance * weight_recent,
                session_count=baseline.session_count + recent_sessions,
                last_updated=datetime.now(),
                confidence=min(
                    1.0,
                    (baseline.session_count + recent_sessions)
                    / self.min_sessions_for_baseline,
                ),
            ...
            logger.info(
                f"Updated baseline for user {baseline.user_id} with {recent_sessions} new sessions"
            )
    ...
    def _needs_baseline_update(self, baseline: UserBaseline) -> bool:
        ...
        return age.days > self.max_baseline_age_days or (
            baseline.confidence < 0.8 and age.hours > 6
        )  # More frequent updates for low confidence
    ...
    async def _store_user_baseline(self, baseline: UserBaseline):
        ...
        try:
            ...
            await self.clickhouse.execute_query(
                query,
                {
                    "user_id": baseline.user_id,
...
class ThresholdOptimizer:
    ...
    async def optimize_thresholds(
        self, baseline: UserBaseline, historical_performance: Dict[str, Any]
    ...
    def _calculate_improvement_score(
        ...
        personalization_score = (
            baseline.confidence
        )  # More confident baseline = better personalization
...
class AdaptiveThresholdManager:
    ...
    async def get_personalized_thresholds(self, user_id: str) -> ThresholdConfig:
        ...
        try:
            ...
            if not baseline or baseline.confidence < 0.3:
                ...
                logger.info(
                    f"Using default thresholds for user {user_id} (insufficient data)"
                )
                return self.default_thresholds
            ...
            personalized_config = self._apply_contextual_adjustments(
                personalized_config
            )
            ...
            logger.info(
                f"Generated personalized thresholds for user {user_id}: "
    ...
    async def update_user_sensitivity(self, user_id: str, feedback: str) -> bool:
        ...
        try:
            ...
            baseline.sensitivity_factor = max(
                0.5, min(2.0, baseline.sensitivity_factor)
            )
            ...
            logger.info(
                f"Updated sensitivity for user {user_id}: {baseline.sensitivity_factor:.2f}"
            )
            return True
