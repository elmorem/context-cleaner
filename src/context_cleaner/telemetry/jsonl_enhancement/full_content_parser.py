...
class FullContentJsonlParser:
    ...
    def extract_message_content(
        ...
        try:
            ...
            detected_languages = FullContentJsonlParser._detect_languages_in_text(
                full_content
            )
            ...
            return {
                ...
                "timestamp": FullContentJsonlParser._parse_timestamp(
                    jsonl_entry.get("timestamp")
                ),
                ...
                "cache_creation_input_tokens": message.get("usage", {}).get(
                    "cache_creation_input_tokens", 0
                ),
                "cache_read_input_tokens": message.get("usage", {}).get(
                    "cache_read_input_tokens", 0
                ),
                "cache_creation_tokens": message.get("usage", {}).get(
                    "cache_creation_tokens", 0
                ),
                "cache_read_tokens": message.get("usage", {}).get(
                    "cache_read_tokens", 0
                ),
    ...
    def extract_file_content(jsonl_entry: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            programming_language = FullContentJsonlParser._detect_language_from_file(
                file_content, file_path
            )
            file_type = FullContentJsonlParser._classify_file_type(
                file_content, file_path
            )
    ...
    def extract_tool_results(jsonl_entry: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            output_type = FullContentJsonlParser._classify_tool_output(
                tool_output_full, tool_data.get("name")
            )
    ...
    def _detect_language_from_file(content: str, file_path: str) -> str:
        ...
        if "def " in content and ("import " in content or "from " in content):
            return "python"
        elif ("function " in content or "const " in content) and (
            "{" in content and "}" in content
        ):
            return "javascript"
        elif "SELECT " in content.upper() and "FROM " in content.upper():
            return "sql"
        elif content.strip().startswith("#!/bin/bash") or content.strip().startswith(
            "#!/bin/sh"
        ):
            return "bash"
    ...
    def _classify_file_type(content: str, file_path: str) -> str:
        ...
        if any(
            ...
            for pattern in ["config", ".env", "settings", ".ini", ".conf"]
        ):
            return "config"
