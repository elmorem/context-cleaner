...
# Core imports
from ..core.manipulation_engine import ManipulationEngine, ManipulationPlan, ManipulationOperation
from ..core.preview_generator import PreviewGenerator, OperationPreview, PlanPreview, PreviewFormat
from ..core.confirmation_workflows import ConfirmationWorkflowManager, ConfirmationLevel, ConfirmationResult
from ..core.transaction_manager import TransactionManager

# Optimization imports
from .personalized_strategies import StrategyType, PersonalizedOptimizationEngine, PersonalizedStrategy
...
class InteractiveWorkflowManager:
    ...
    def __init__(self,
        ...
        self.confirmation_manager = confirmation_manager or ConfirmationWorkflowManager()
        self.transaction_manager = transaction_manager or TransactionManager()
        self.personalization_engine = personalization_engine or PersonalizedOptimizationEngine()
    ...
    def start_interactive_optimization(self,
                                     context_data: Dict[str, Any],
                                     preferred_strategy: Optional[StrategyType] = None,
                                     session_id: Optional[str] = None) -> InteractiveSession:
        """
        ...
        if self.auto_cleanup_sessions and len(self.active_sessions) >= self.max_active_sessions:
    ...
    def recommend_strategy(self,
                          session_id: str,
                          user_preferences: Optional[Dict[str, Any]] = None) -> StrategyType:
        """
        ...
        personalized_strategy = self.personalization_engine.create_personalized_strategy(
            context_analysis=session.metadata,
            user_profile=user_preferences or {}
        )
    ...
    def generate_optimization_plan(self,
                                 session_id: str,
                                 strategy: Optional[StrategyType] = None) -> ManipulationPlan:
        """
        ...
        selected_strategy = strategy or session.selected_strategy or StrategyType.BALANCED
    ...
    def generate_preview(self,
                        session_id: str,
                        preview_format: PreviewFormat = PreviewFormat.TEXT) -> PlanPreview:
        """
    ...
    def request_user_approval(self,
                            session_id: str,
                            confirmation_level: ConfirmationLevel = ConfirmationLevel.DETAILED) -> ConfirmationResult:
        """
        ...
        if not session.manipulation_plan or not session.preview:
            raise ValueError("Plan and preview must be generated before requesting approval.")
    ...
    def apply_selective_changes(self,
                              session_id: str,
                              selected_operations: List[str]) -> WorkflowResult:
        """
        ...
        selected_ops = [
            op for op in session.manipulation_plan.operations
        ...
        filtered_plan = ManipulationPlan(
            ...
            estimated_total_reduction=sum(op.estimated_token_impact for op in selected_ops),
        ...
        return self._execute_plan_with_transaction(session, filtered_plan, selected_operations)
    ...
    def execute_full_plan(self, session_id: str) -> WorkflowResult:
        ...
        all_operation_ids = [op.operation_id for op in session.manipulation_plan.operations]
        return self._execute_plan_with_transaction(session, session.manipulation_plan, all_operation_ids)
    ...
    def cancel_session(self, session_id: str) -> bool:
        ...
        if session_id in self.active_sessions:
            ...
            result = WorkflowResult(
                ...
                operations_planned=len(session.manipulation_plan.operations) if session.manipulation_plan else 0,
    ...
    def get_session_status(self, session_id: str) -> Dict[str, Any]:
        ...
        return {
            ...
            'strategy': session.selected_strategy.value if session.selected_strategy else None,
            'operations_planned': len(session.manipulation_plan.operations) if session.manipulation_plan else 0,
    ...
    def _execute_plan_with_transaction(self,
                                     session: InteractiveSession,
        ...
        try:
            ...
            with self.transaction_manager.transaction(
                ...
                # Execute operations
                execution_results = tx.execute_operations(manipulation_engine=self.manipulation_engine)
    ...
    def _generate_conservative_plan(self, context_data: Dict[str, Any]) -> ManipulationPlan:
        ...
        if isinstance(context_data, dict):
            for key, value in context_data.items():
                if isinstance(value, str):
                    ...
                    if any(marker in value.lower() for marker in ['✅', 'completed', 'done', 'resolved']):
                        operations.append(ManipulationOperation(
    ...
    def _generate_balanced_plan(self, context_data: Dict[str, Any]) -> ManipulationPlan:
        ...
        analysis_result = self._create_basic_analysis_result(context_data, health_score=70, optimization_potential=0.6)
    ...
    def _generate_aggressive_plan(self, context_data: Dict[str, Any]) -> ManipulationPlan:
        ...
        analysis_result = self._create_basic_analysis_result(context_data, health_score=40, optimization_potential=0.8)
    ...
    def _create_basic_analysis_result(self, context_data: Dict[str, Any], 
                                      health_score: int = 70,
                                      optimization_potential: float = 0.6) -> 'ContextAnalysisResult':
        ...
        return ContextAnalysisResult(
            ...
            context_categories={"files": len([k for k in context_data.keys() if 'file' in k.lower()])},
            analysis_timestamp=datetime.now().isoformat(),
            analysis_duration=0.1,
            performance_metrics={},
            optimization_potential=optimization_potential,
            critical_context_ratio=0.7,
            cleanup_impact_estimate=int(len(str(context_data)) * optimization_potential * 0.1)
        )


# Convenience functions

def start_interactive_optimization(context_data: Dict[str, Any],
                                 preferred_strategy: Optional[StrategyType] = None) -> Tuple[InteractiveWorkflowManager, InteractiveSession]:
    """
...
def quick_optimization_preview(context_data: Dict[str, Any],
                              strategy: StrategyType = StrategyType.BALANCED) -> PlanPreview:
    """
