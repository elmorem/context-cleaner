...
class ResourceMonitor:
    ...
    def is_system_overloaded(self) -> bool:
        ...
        return (
            ...
            or stats["disk_percent"] > self.disk_threshold
        )
...
class AdvancedTaskProcessor:
    ...
    def __init__(
        self, max_concurrent_tasks: int = 50, enable_resource_monitoring: bool = True
    ):
        ...
        while retry_count <= task.max_retries:
            try:
                executor = self._get_executor(
                    task.metadata.get("executor_type", "async")
                )
            ...
            if retry_count <= task.max_retries:
                self.logger.warning(
                    f"Task {task.id} failed, retrying ({retry_count}/{task.max_retries})"
                )
                await asyncio.sleep(
                    task.retry_delay * retry_count
                )  # Exponential backoff
    ...
    async def _process_tasks(self):
        ...
        while self.processing:
            try:
                ...
                if (
                    self.resource_monitor
                    and self.resource_monitor.is_system_overloaded()
                ):
                ...
                # Execute the task
                self.logger.info(
                    f"Starting execution of task {available_task.id} ({available_task.name})"
                )
                ...
                async def run_task():
                    try:
                        ...
                        if result.status == TaskStatus.COMPLETED:
                            ...
                            self.logger.info(
                                f"Task {available_task.id} completed successfully"
                            )
                        else:
                            self.failed_tasks[available_task.id] = result
                            self.stats["tasks_failed"] += 1
                            self.logger.error(
                                f"Task {available_task.id} failed: {result.error}"
                            )
