...
class ChangeApprovalSystem:
    ...
    def categorize_operations(self, operations: List[ManipulationOperation]) -> Dict[ChangeCategory, List[ManipulationOperation]]:
        """
    ...
    def create_approval_session(self,
                              plan: ManipulationPlan,
                              preview: Optional[PlanPreview] = None) -> str:
        """
    ...
    def select_operation(self,
        ...
        result.selections = [s for s in result.selections if s.operation_id != operation_id]
    ...
    def select_by_category(self,
        ...
        """
        count = 0
        batch_reason = reason or f"Batch {decision.value} for {category.value} operations"
    ...
    def apply_user_preferences(self,
                              approval_id: str,
                              operations: List[ManipulationOperation]) -> int:
        """
        ...
        for operation in operations:
            ...
            if operation_category in self.category_settings:
                ...
                if (operation.confidence_score >= settings.get('min_confidence', 0.8) and
                    settings.get('default_action') == ApprovalDecision.APPROVE):
    ...
    def get_approval_summary(self, approval_id: str) -> Dict[str, Any]:
        ...
        approval_rate = len(result.approved_operations) / result.total_operations if result.total_operations > 0 else 0
    ...
    def finalize_approval(self,
                         approval_id: str,
                         user_feedback: Optional[str] = None) -> SelectiveApprovalResult:
        """
    ...
    def import_approval_preferences(self, preferences: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            for category_name, settings in preferences.get('category_settings', {}).items():
                try:
    ...
    def _get_operation_category(self, operation: ManipulationOperation) -> ChangeCategory:
    ...
    def _update_approval_lists(self, result: SelectiveApprovalResult):
        ...
        result.approval_rate = (len(result.approved_operations) / result.total_operations 
    ...
    def _initialize_category_settings(self):
        ...
        self.category_settings[ChangeCategory.REORDERING]['requires_confirmation'] = False
        self.category_settings[ChangeCategory.SAFETY]['default_action'] = ApprovalDecision.APPROVE


# Convenience functions

def create_quick_approval(operations: List[ManipulationOperation],
                         auto_approve_safe: bool = True) -> Tuple[ChangeApprovalSystem, str]:
    """
    ...
    if auto_approve_safe:
        ...
        for operation in operations:
            if operation.confidence_score >= 0.9 and not operation.requires_confirmation:
...
def approve_safe_operations_only(operations: List[ManipulationOperation],
    ...
    for operation in operations:
        if (operation.confidence_score >= min_confidence and
            not operation.requires_confirmation and
            operation.operation_type in ['reorder', 'consolidate']):
