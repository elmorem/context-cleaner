...
class PersonalizedOptimizationEngine:
    ...
    def __init__(self, storage_path: Optional[Path] = None):
        ...
        self.storage_path = storage_path or Path.home() / ".context_cleaner" / "strategies"
    ...
    async def create_personalized_strategy(
        ...
        rules = await self._generate_strategy_rules(
            strategy_type, profile, dashboard_data, cross_session_insights, recommendations
        )
    ...
    async def adapt_strategy(
        ...
        recent_effectiveness = strategy.effectiveness_history[-5:]  # Last 5 applications
        if len(recent_effectiveness) > 1:
            trend = recent_effectiveness[-1] - recent_effectiveness[0]
            strategy.adaptation_speed = max(0.1, min(2.0, strategy.adaptation_speed + trend))
        
        # Update user satisfaction based on feedback
        if user_feedback:
            satisfaction_adjustment = self._parse_user_feedback(user_feedback)
            strategy.user_satisfaction = max(0.0, min(1.0, 
                strategy.user_satisfaction + satisfaction_adjustment))
    ...
    async def recommend_strategy_changes(
        ...
        if len(recent_performance) > 3:
            trend = statistics.mean(recent_performance[-3:]) - statistics.mean(recent_performance[:-3])
        ...
        if strategy.optimization_mode == OptimizationMode.MANUAL and strategy.success_rate > 0.8:
            recommendations.append(StrategyRecommendation(
        ...
        workflow_performance = self._analyze_workflow_performance(strategy, current_context)
        ...
        return sorted(recommendations, key=lambda r: r.confidence * r.personalization_fit, reverse=True)
    ...
    async def apply_strategy(
        ...
        sorted_rules = sorted(strategy.rules, key=lambda r: r.effectiveness_score, reverse=True)
        ...
        optimization_results["estimated_improvements"] = {
            "token_savings": sum([action.get("result", {}).get("token_savings", 0) for action in applied_actions]),
            "efficiency_gain": sum([action.get("result", {}).get("efficiency_gain", 0) for action in applied_actions]),
            "focus_improvement": max([action.get("result", {}).get("focus_improvement", 0) for action in applied_actions])
        }
        
        # Calculate confidence scores
        optimization_results["confidence_scores"] = {
            "overall_confidence": strategy.confidence_score,
            "application_confidence": statistics.mean([action.get("confidence", 0) for action in applied_actions]) if applied_actions else 0,
            "strategy_effectiveness": strategy.success_rate
        }
        
        # Suggest next actions
        optimization_results["next_actions"] = await self._suggest_next_actions(strategy, context, applied_actions)
    ...
    async def _determine_optimal_strategy_type(
        ...
        if health_score < 0.4:  # Poor health - need aggressive action
            return StrategyType.AGGRESSIVE
        elif health_score > 0.8 and adaptation_score > 0.7:  # Good health and adaptation - use learning
            return StrategyType.LEARNING_ADAPTIVE
        elif automation_comfort > 0.7:  # Comfortable with automation - balanced approach
            return StrategyType.BALANCED
        else:  # Default to conservative
            return StrategyType.CONSERVATIVE
    
    def _determine_optimization_mode(self, profile: PersonalizationProfile) -> OptimizationMode:
    ...
    async def _generate_strategy_rules(
        ...
        if dashboard_data.token_analysis:
            token_rules = self._create_token_efficiency_rules(dashboard_data.token_analysis)
    ...
    def _create_workflow_preferences(
        ...
        preferences = {
            ...
            "session_length_preference": profile.typical_session_length.total_seconds() / 3600,  # hours
    ...
    def _get_aggressive_rules(self) -> List[StrategyRule]:
        ...
        return [
            StrategyRule(
                ...
                action="remove_duplicates",
                parameters={"confirmation_required": False, "confidence_threshold": 0.7},
    ...
    def _get_workflow_specific_rules(self, insights: CrossSessionInsights) -> List[StrategyRule]:
        ...
        for template in insights.workflow_templates:
            rule = StrategyRule(
                ...
                parameters={"template_id": template.template_id, "strategy": template.optimization_strategy},
    ...
    def _create_rule_from_recommendation(self, recommendation: IntelligentRecommendation) -> StrategyRule:
    ...
    def _create_token_efficiency_rules(self, token_analysis) -> List[StrategyRule]:
        ...
        if token_analysis.waste_percentage > 20:
            rules.append(StrategyRule(
                ...
                parameters={"waste_patterns": [p.pattern for p in token_analysis.waste_patterns[:3]]},
                effectiveness_score=0.9,
                confidence_level=0.8
            ))
    ...
    async def _should_apply_rule(
        ...
        if rule.last_applied:
            ...
            if time_since_last < timedelta(minutes=30):  # Minimum 30 minutes between applications
                return False
    ...
    async def _apply_optimization_rule(self, rule: StrategyRule, context: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _adjust_strategy_for_poor_performance(self, strategy: PersonalizedStrategy) -> None:
    ...
    def _parse_user_feedback(self, feedback: str) -> float:
        ...
        if any(word in feedback_lower for word in ["great", "excellent", "perfect", "love"]):
            return 0.1
        elif any(word in feedback_lower for word in ["good", "helpful", "nice"]):
            return 0.05
        elif any(word in feedback_lower for word in ["bad", "terrible", "hate", "awful"]):
            return -0.2
    ...
    def _analyze_workflow_performance(
        ...
        for workflow in strategy.workflow_preferences.get("preferred_workflows", []):
            ...
            workflow_performance[workflow] = max(0.1, strategy.success_rate + (hash(workflow) % 100) / 500.0)
