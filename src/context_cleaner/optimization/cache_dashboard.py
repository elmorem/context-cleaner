...
from ..cache import (
    ...
    CrossSessionCorrelationAnalyzer, UsagePatternSummary, TokenAnalysisSummary,
    TemporalInsights, CacheEnhancedAnalysis, CorrelationInsights
)
...
class CacheEnhancedDashboard:
    ...
    async def generate_dashboard(
        ...
        try:
            ...
            correlation_insights = analysis_results[4] if include_cross_session else None
            ...
            health_metrics = self._calculate_usage_health_metrics(
                usage_summary, token_analysis, temporal_insights, 
                enhanced_analysis, correlation_insights
            )
            ...
            return CacheEnhancedDashboardData(
                context_size=enhanced_analysis.weighted_context_size if enhanced_analysis else 0,
                ...
                correlation_insights=correlation_insights or CorrelationInsights(
                    session_clusters=[], cross_session_patterns=[], 
                    long_term_trends=[], total_sessions_analyzed=0, analysis_time_span_days=0,
                    file_usage_correlations={}, tool_usage_correlations={}, temporal_correlations={},
                    session_dependencies=[], workflow_continuations=[],
                    complexity_evolution=[], efficiency_evolution=[], focus_evolution=[],
                    predicted_next_patterns=[], recommended_session_timing=[], optimal_workflow_sequences=[],
                    analysis_confidence=0.5, data_completeness=0.5
                ),
    ...
    async def _discover_cache_locations(self) -> List[Path]:
        ...
        try:
            locations = await asyncio.to_thread(self.cache_discovery.discover_cache_locations)
            return [loc.path for loc in locations if loc.path.exists()]
        except Exception:
            return []
    
    async def _parse_recent_sessions(self, cache_paths: List[Path], max_sessions: int) -> List[Any]:
        ...
        for cache_path in cache_paths:
            try:
                sessions = await asyncio.to_thread(
                    self.session_parser.parse_cache_directory, 
                    cache_path, max_sessions
                )
    ...
    async def _analyze_usage_patterns(self, sessions: List[Any]) -> UsagePatternSummary:
        ...
        return await asyncio.to_thread(self.usage_analyzer.analyze_usage_patterns, sessions)
    
    async def _analyze_token_efficiency(self, sessions: List[Any]) -> TokenAnalysisSummary:
        """Analyze token efficiency from sessions."""
        return await asyncio.to_thread(self.token_analyzer.analyze_token_efficiency, sessions)
    
    async def _analyze_temporal_patterns(self, sessions: List[Any]) -> TemporalInsights:
        """Analyze temporal patterns from sessions."""
        return await asyncio.to_thread(self.temporal_analyzer.analyze_temporal_patterns, sessions)
    
    async def _analyze_enhanced_context(self, sessions: List[Any], context_path: Optional[Path]) -> CacheEnhancedAnalysis:
        """Run enhanced context analysis with cache data."""
        return await asyncio.to_thread(
            self.enhanced_analyzer.analyze_with_cache_data, 
            sessions, context_path
        )
    
    async def _analyze_cross_session_correlation(self, sessions: List[Any]) -> CorrelationInsights:
    ...
    async def _generate_enhanced_health_analysis(
        ...
        if context_path and context_path.exists():
            ...
            if enhanced_analysis:
                ...
                health_report.priority_alignment = enhanced_analysis.priority_alignment_score
            return health_report
        else:
            # Generate synthetic health report from cache data
            return HealthScore(
                overall_score=int((enhanced_analysis.overall_health_score if enhanced_analysis else 0.5) * 100),
                component_scores={
                    "focus": int((enhanced_analysis.usage_weighted_focus_score if enhanced_analysis else 0.5) * 100),
                    "priority_alignment": int((enhanced_analysis.priority_alignment_score if enhanced_analysis else 0.5) * 100),
                    "redundancy": 30,
                    "recency": 70,
    ...
    def _calculate_usage_health_metrics(
        ...
        focus_score = enhanced_analysis.usage_weighted_focus_score if enhanced_analysis else 0.5
        
        # Token efficiency score
        efficiency_score = 1.0 - (token_analysis.waste_percentage / 100.0) if token_analysis else 0.6
        
        # Temporal coherence score
        coherence_score = temporal_insights.coherence_score if temporal_insights else 0.5
        
        # Cross-session consistency
        consistency_score = correlation_insights.correlation_strength if correlation_insights else 0.5
        
        # Optimization potential (inverse of current efficiency)
        optimization_potential = 1.0 - efficiency_score if efficiency_score < 0.8 else 0.2
    ...
    def _generate_usage_insights(
        ...
        if token_analysis and token_analysis.waste_percentage > 20:
            insights.append(UsageInsight(
                ...
                file_patterns=[pattern.pattern for pattern in token_analysis.waste_patterns[:3]],
        ...
        if usage_summary and usage_summary.workflow_efficiency < 0.6:
            insights.append(UsageInsight(
                ...
                file_patterns=[pattern.file_path for pattern in usage_summary.file_patterns[:3]],
    ...
    async def _generate_basic_dashboard(self, context_path: Optional[Path]) -> CacheEnhancedDashboardData:
        ...
        health_metrics = UsageBasedHealthMetrics(
            usage_weighted_focus_score=traditional_health.component_scores.get("focus", 50) / 100.0,
        ...
        return CacheEnhancedDashboardData(
            ...
            optimization_recommendations=[{
                ...
                "actions": ["Ensure Claude Code cache is accessible", "Run more sessions to build analysis data"],
