...
class LRUCache:
    ...
    def _evict_if_needed(self, incoming_size_mb: float):
        ...
        while (self._current_memory_mb + incoming_size_mb > self.max_memory_mb 
        ...
        with self._lock:
            return {
                ...
                "memory_usage_percent": round(
                    (self._current_memory_mb / self.max_memory_mb) * 100, 1
...
class MemoryOptimizer:
        ...
        while not self._stop_monitoring.is_set():
            try:
                ...
                if snapshot.process_mb > self.target_memory_mb:
                    logger.debug(f"Memory usage ({snapshot.process_mb:.1f}MB) above target, optimizing")
                    self._optimize_memory_usage()
                
                # Critical memory handling
                if snapshot.process_mb > self.critical_memory_mb:
                    logger.warning(f"Critical memory usage ({snapshot.process_mb:.1f}MB), aggressive cleanup")
    ...
    def get_memory_report(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "caches": {
                ...
                "total_items": sum(stats["total_items"] for stats in cache_stats.values()),
                "details": cache_stats
            },
            "trends": {
    ...
    def _calculate_memory_health(self, snapshot: MemorySnapshot) -> int:
        ...
        total_cache_memory = sum(cache._current_memory_mb for cache in self._caches.values())
