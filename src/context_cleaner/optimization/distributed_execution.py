...
class LoadBalancer:
    ...
    def get_optimal_node(self, task: Task) -> Optional[NodeInfo]:
        ...
        def score_node(node: NodeInfo) -> float:
            ...
            capacity_score = 1.0 - (
                node.active_tasks / node.max_tasks
            ...
            if hasattr(task, "required_capabilities"):
                ...
                if required_caps:
                    ...
                    capability_score = (
                        matches / len(required_caps) if required_caps else 1.0
                    )
            ...
            return (
                load_score * 0.4 + capacity_score * 0.4 + capability_score * 0.2
            ) * priority_bonus
    ...
    def rebalance_tasks(self) -> List[Tuple[str, str, str]]:
        ...
        underloaded_nodes = [
            ...
            if n.current_load < 0.4 and n.status == NodeStatus.ACTIVE
        ]
            ...
            for underloaded_node in underloaded_nodes:
                if overloaded_node.active_tasks > underloaded_node.active_tasks + 2:
                    ...
                    rebalance_suggestions.append(
                        ("rebalance", overloaded_node.node_id, underloaded_node.node_id)
                    )
                    break
...
class DistributedTaskCoordinator:
    ...
    async def start(
        self, master_host: Optional[str] = None, master_port: Optional[int] = None
    ):
        ...
        self.websocket_server = await ws_serve(
            self._handle_websocket_connection, self.host, self.port
        )
        ...
        if self.is_master:
            ...
            self.logger.info(
                f"Master node {self.node_id} started on {self.host}:{self.port}"
            )
        else:
            ...
            self.logger.info(
                f"Worker node {self.node_id} started on {self.host}:{self.port}"
            )
    ...
    async def _handle_task_assignment(self, message: ClusterMessage):
        ...
        try:
            ...
            asyncio.create_task(
                self._monitor_task_completion(task_id, message.sender_id)
            )
        ...
        try:
            ...
            result_msg = ClusterMessage(
                ...
                payload={
                    ...
                    "result": (
                        pickle.dumps(result.result).hex() if result.result else None
                    ),
    ...
    async def _handle_task_result(self, message: ClusterMessage):
        ...
        if task_id in self.pending_results:
            ...
            result = TaskResult(
                ...
                result=(
                    pickle.loads(bytes.fromhex(payload["result"]))
            ...
            try:
                if timeout:
                    result = await asyncio.wait_for(
                        self.pending_results[task_id], timeout=timeout
                    )
                else:
