...
from .cache_dashboard import CacheEnhancedDashboardData, UsageBasedHealthMetrics, HealthLevel
from .cross_session_analytics import CrossSessionInsights, PatternEvolution, WorkflowTemplate
...
class AdvancedReportingSystem:
    ...
    async def generate_comprehensive_report(
        ...
        sections = await self._generate_report_sections(
            report_type, dashboard_data, cross_session_insights, 
            recommendations, personalization_profile
        )
    ...
    async def _generate_executive_summary(
        ...
        critical_recs = [r for r in recommendations if r.priority.value in ["critical", "high"]]
        
        summary_parts = []
        
        # Health status
        summary_parts.append(f"**Context Health Status**: {health_level.value.title()} "
                           f"(Score: {overall_score:.1%})")
        
        # Analysis scope
        summary_parts.append(f"**Analysis Coverage**: {sessions_analyzed} sessions analyzed "
                           f"over {cross_session_insights.time_span_days} days")
        
        # Key findings
        if health_level in [HealthLevel.POOR, HealthLevel.CRITICAL]:
            summary_parts.append("**Key Finding**: Context health requires immediate attention. "
                                f"{len(critical_recs)} high-priority optimizations identified.")
        elif health_level == HealthLevel.EXCELLENT:
            summary_parts.append("**Key Finding**: Context is well-optimized. Focus on maintaining "
                                "current performance and leveraging automation opportunities.")
        else:
            summary_parts.append(f"**Key Finding**: Context shows {health_level.value} performance "
        ...
        if dashboard_data.token_analysis:
            ...
            summary_parts.append(f"**Token Efficiency**: {100-waste_pct:.1f}% efficient "
        ...
        if dashboard_data.usage_summary:
            ...
            summary_parts.append(f"**Workflow Alignment**: {workflow_eff:.1%} efficiency "
                                "based on actual usage patterns")
        
        # Automation potential
        automation_count = len(cross_session_insights.automation_opportunities)
        if automation_count > 0:
            summary_parts.append(f"**Automation Potential**: {automation_count} processes "
    ...
    async def _generate_report_sections(
        ...
        if report_type == ReportType.EXECUTIVE_SUMMARY:
            ...
            sections.extend(await self._generate_detailed_sections(
                dashboard_data, cross_session_insights, recommendations
            ))
        elif report_type == ReportType.TREND_REPORT:
            sections.extend(await self._generate_trend_sections(
                dashboard_data, cross_session_insights
            ))
        elif report_type == ReportType.OPTIMIZATION_REPORT:
            sections.extend(await self._generate_optimization_sections(
                recommendations, cross_session_insights
            ))
        elif report_type == ReportType.PERSONALIZATION_REPORT:
            sections.extend(await self._generate_personalization_sections(
                personalization_profile, recommendations
            ))
        elif report_type == ReportType.COMPARATIVE_ANALYSIS:
            sections.extend(await self._generate_comparative_sections(
                dashboard_data, cross_session_insights
            ))
    ...
    async def _generate_executive_sections(
        ...
        sections.append(ReportSection(
            ...
            charts=[{"type": "health_gauge", "data": asdict(dashboard_data.health_metrics)}],
            insights=[
        ...
        sections.append(ReportSection(
            ...
            content=kpi_content,
            charts=[{"type": "kpi_dashboard", "data": cross_session_insights.efficiency_trends}],
            insights=[
    ...
    async def _generate_detailed_sections(
        ...
        if dashboard_data.usage_summary:
            ...
            sections.append(ReportSection(
                ...
                charts=[{"type": "usage_heatmap", "data": dashboard_data.usage_trends}],
                insights=[
                ...
                recommendations=[r.title for r in recommendations if r.category.value == "workflow_alignment"],
        ...
        if dashboard_data.token_analysis:
            ...
            sections.append(ReportSection(
                ...
                charts=[{"type": "waste_breakdown", "data": dashboard_data.efficiency_trends}],
                insights=[
                ...
                recommendations=[r.title for r in recommendations if r.category.value == "token_efficiency"],
                metadata={"importance": "high", "technical_level": "detailed"}
            ))
        ...
        sections.append(ReportSection(
            ...
            charts=[{"type": "pattern_evolution", "data": cross_session_insights.pattern_evolution}],
            insights=[
    ...
    async def _generate_trend_sections(
        ...
        # Efficiency Trends
        trend_content = self._format_trend_analysis(cross_session_insights.efficiency_trends)
        sections.append(ReportSection(
            title="Efficiency Trends Over Time",
            content=trend_content,
            charts=[{"type": "line_chart", "data": cross_session_insights.efficiency_trends}],
            insights=[
            ...
            recommendations=["Continue current optimization strategy", "Monitor for trend reversals"],
            metadata={"importance": "medium", "time_series": True}
        ))
        
        # Pattern Evolution
        evolution_content = self._format_pattern_evolution(cross_session_insights.pattern_evolution)
        sections.append(ReportSection(
            title="Pattern Evolution Analysis",
            content=evolution_content,
            charts=[{"type": "evolution_chart", "data": cross_session_insights.pattern_evolution}],
            insights=[
            ...
            recommendations=["Implement automation for stable patterns", "Continue monitoring adaptation"],
    ...
    async def _generate_optimization_sections(
        ...
        high_priority = [r for r in recommendations if r.priority.value in ["critical", "high"]]
        rec_content = self._format_recommendations(high_priority)
        sections.append(ReportSection(
            title="High Priority Optimizations",
            content=rec_content,
            charts=[{"type": "priority_matrix", "data": [asdict(r) for r in high_priority]}],
            insights=[
        ...
        sections.append(ReportSection(
            ...
            charts=[{"type": "automation_potential", "data": cross_session_insights.automation_opportunities}],
            insights=[
            ...
            recommendations=["Implement automated workflows", "Monitor automation effectiveness"],
    ...
    async def _generate_personalization_sections(
        ...
        if personalization_profile:
            ...
            sections.append(ReportSection(
                ...
                charts=[{"type": "profile_radar", "data": asdict(personalization_profile)}],
                insights=[
                ...
                recommendations=["Continue building profile data", "Adjust automation based on comfort level"],
                metadata={"importance": "medium", "personalized": True}
            ))
            
            # Personalized Recommendations
            personalized_recs = [r for r in recommendations if r.user_preference_alignment > 0.7]
            pers_rec_content = self._format_personalized_recommendations(personalized_recs)
            sections.append(ReportSection(
                title="Personalized Optimization Strategy",
                content=pers_rec_content,
                charts=[{"type": "personalization_fit", "data": [asdict(r) for r in personalized_recs]}],
                insights=[
    ...
    async def _generate_comparative_sections(
        ...
        sections.append(ReportSection(
            ...
            charts=[{"type": "comparison_chart", "data": cross_session_insights.efficiency_trends}],
            insights=[
            ...
            recommendations=["Continue successful optimization patterns", "Address declining areas"],
    ...
    def _extract_key_metrics(
        ...
        return {
            ...
            "workflow_efficiency": dashboard_data.usage_summary.workflow_efficiency if dashboard_data.usage_summary else 0,
            "token_waste_percentage": dashboard_data.token_analysis.waste_percentage if dashboard_data.token_analysis else 0,
            "sessions_analyzed": cross_session_insights.sessions_analyzed,
            "user_adaptation_score": cross_session_insights.user_adaptation_score,
            "automation_opportunities": len(cross_session_insights.automation_opportunities),
    ...
    def _calculate_performance_indicators(
        ...
        efficiency_trends = cross_session_insights.efficiency_trends.get("overall_efficiency", [])
        if len(efficiency_trends) > 1:
            indicators["efficiency_improvement"] = efficiency_trends[-1] - efficiency_trends[0]
        ...
        if cross_session_insights.pattern_evolution:
            stability_scores = [p.stability_score for p in cross_session_insights.pattern_evolution]
            indicators["pattern_stability"] = statistics.mean(stability_scores)
        
        # Automation readiness
        automation_count = len(cross_session_insights.automation_opportunities)
        indicators["automation_readiness"] = min(1.0, automation_count / 5.0)  # Normalize to 0-1
    ...
    def _extract_critical_insights(
        ...
        if health_level == HealthLevel.CRITICAL:
            ...
            insights.append("✅ Excellent context health - focus on maintaining performance")
        
        # Token efficiency insights
        if dashboard_data.token_analysis and dashboard_data.token_analysis.waste_percentage > 30:
            insights.append(f"💰 High token waste: {dashboard_data.token_analysis.waste_percentage:.1f}% - significant cost savings possible")
        
        # Usage pattern insights
        if dashboard_data.usage_summary and dashboard_data.usage_summary.workflow_efficiency < 0.5:
            insights.append(f"🔄 Low workflow efficiency: {dashboard_data.usage_summary.workflow_efficiency:.1%} - context misaligned with usage")
        
        # Cross-session insights
        if cross_session_insights.user_adaptation_score > 0.8:
            insights.append("📈 Excellent user adaptation - optimizations are working well")
        elif cross_session_insights.user_adaptation_score < 0.3:
            insights.append("📉 Poor adaptation - current optimizations may not be effective")
        
        # Automation insights
        automation_count = len(cross_session_insights.automation_opportunities)
        if automation_count > 3:
            insights.append(f"🤖 High automation potential: {automation_count} processes identified")
        
        # Critical recommendations
        critical_recs = [r for r in recommendations if r.priority.value == "critical"]
        if critical_recs:
            insights.append(f"⚡ {len(critical_recs)} critical optimizations require immediate action")
    ...
    async def _generate_charts_data(
        ...
        if dashboard_data.token_analysis:
            charts.append({
                ...
                "data": {
                    "Efficient Usage": 100 - dashboard_data.token_analysis.waste_percentage,
            ...
            charts.append({
                ...
                "data": {
                    "patterns": [
                        ...
                        for pattern in dashboard_data.usage_summary.file_patterns[:10]
                    ]
                }
            })
...
"""
    ...
    def _format_recommendations(self, recommendations: List[IntelligentRecommendation]) -> str:
    ...
    def _format_personalized_recommendations(self, recommendations: List[IntelligentRecommendation]) -> str:
    ...
    async def _persist_report(self, report: UsageReport, output_format: ReportFormat) -> None:
        """Persist report to storage."""
        try:
            report_path = self.storage_path / f"{report.report_id}.{output_format.value}"
