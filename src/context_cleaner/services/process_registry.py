...
class ProcessRegistryDatabase:
    ...
    def _setup_database(self):
        ...
        with self._get_connection() as conn:
            ...
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_service_type ON processes(service_type)"
            )
            conn.execute("CREATE INDEX IF NOT EXISTS idx_status ON processes(status)")
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_session_id ON processes(session_id)"
            )
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_registration_source ON processes(registration_source)"
            )
        ...
        try:
            ...
            if conn is not None:
                try:
                    ...
                    logger.warning(
                        f"Failed to close process registry connection: {close_error}"
                    )

    def _ensure_column(
        self, conn: sqlite3.Connection, column: str, definition: str
    ) -> None:
        ...
        try:
            with self._get_connection() as conn:
                ...
                data["start_time"] = (
                    ...
                    else entry.start_time.isoformat()
                )
                data["registration_time"] = (
                    data["registration_time"]
                    if isinstance(data["registration_time"], str)
                    else entry.registration_time.isoformat()
                )

                if entry.last_health_check:
                    data["last_health_check"] = (
                        data["last_health_check"]
                        if isinstance(data["last_health_check"], str)
                        else entry.last_health_check.isoformat()
                    )
                ...
                conn.commit()
                logger.debug(
                    f"Registered process PID {entry.pid} ({entry.service_type})"
                )
                return True
    ...
    def get_all_processes(self) -> List[ProcessEntry]:
        ...
        try:
            with self._get_connection() as conn:
                rows = conn.execute(
                    "SELECT * FROM processes ORDER BY registration_time DESC"
                ).fetchall()
    ...
    def update_process_status(
        self, pid: int, status: str, health_status: Optional[bool] = None
    ) -> bool:
    ...
    def cleanup_stale_entries(self, max_age_hours: int = 24) -> int:
        ...
        try:
            with self._get_connection() as conn:
                # Get all processes
                rows = conn.execute(
                    "SELECT pid, registration_time FROM processes"
                ).fetchall()
                stale_pids = []
                ...
                if stale_pids:
                    ...
                    cursor = conn.execute(
                        f"DELETE FROM processes WHERE pid IN ({placeholders})",
                        stale_pids,
                    )
                    conn.commit()
        ...
        try:
            with self._get_connection() as conn:
                ...
                for row in rows:
                    ...
                    registration_counts[registration_source] = (
                        registration_counts.get(registration_source, 0) + 1
                    )
                    ...
                    if (
                        ...
                        or registration_time < stats["oldest_entry"]
                    ):
                        stats["oldest_entry"] = registration_time
                    if (
                        stats["newest_entry"] is None
                        or registration_time > stats["newest_entry"]
                    ):
    ...
    def prune_processes(self, *, service_type: Optional[str] = None) -> int:
        ...
        try:
            with self._get_connection() as conn:
                ...
                logger.info(
                    "Pruned %s registry entries (service_type=%s)",
                    removed,
                    service_type or "all",
                )
                return removed
    ...
    def _discover_by_command_line(self) -> List[ProcessEntry]:
        ...
        try:
            for process in psutil.process_iter(
                ["pid", "name", "cmdline", "create_time", "cwd"]
            ):
                try:
        ...
        try:
            ...
            for conn in connections:
                try:
                        ...
                        if conn.pid:
                            ...
                            if any(
                                ...
                                entry = ProcessEntry(
                                    ...
                                    host=conn.laddr.ip,
                                    working_directory=(
                                        process.cwd() if hasattr(process, "cwd") else ""
                                    ),
                    ...
                    logger.debug(
                        f"Process access error during port discovery (pid={getattr(conn, 'pid', 'unknown')}): {e}"
                    )
                    continue
                except Exception as e:
                    # Log unexpected errors but don't let them stop the entire discovery process
                    logger.debug(
                        f"Connection error during port discovery (pid={getattr(conn, 'pid', 'unknown')}): {e}"
                    )
                    continue
                    ...
                    for child in parent.children(recursive=True):
                        try:
                            ...
                            entry = ProcessEntry(
                                ...
                                working_directory=(
                                    child.cwd() if hasattr(child, "cwd") else ""
                                ),
