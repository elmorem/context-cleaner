...
class SystemDiagnosticsAndRecovery:
    ...
    async def run_full_system_diagnostic(self) -> List[DiagnosticResult]:
        ...
        self.logger.info(
            f"System diagnostic completed. Found {len([r for r in results if r.level in [DiagnosticLevel.ERROR, DiagnosticLevel.CRITICAL]])} critical issues"
        )
        ...
        for result in diagnostic_results:
            if result.level == DiagnosticLevel.CRITICAL:
                critical_actions.extend(
                    [(action, result.component) for action in result.recovery_actions]
                )
            elif result.level == DiagnosticLevel.ERROR:
                warning_actions.extend(
                    [(action, result.component) for action in result.recovery_actions]
                )
    ...
    async def _should_attempt_recovery(
        self, action: RecoveryAction, component: str
    ) -> bool:
        ...
        recent_attempts = [
            attempt
            for attempt in self.recovery_history[-10:]  # Last 10 attempts
            if attempt.action == action
            and attempt.component == component
    ...
    async def _execute_recovery_action(
        ...
        attempt = RecoveryAttempt(
            action=action, component=component, timestamp=datetime.now(), success=False
        )
    ...
    async def _clean_duplicate_processes(self) -> bool:
        ...
        try:
            result = subprocess.run(
                ["pkill", "-f", "context_cleaner"], capture_output=True, timeout=10
            )
    ...
    async def _reset_database_connections(self) -> bool:
        ...
        try:
            ...
            result = subprocess.run(
                ["docker", "restart", "clickhouse-otel"],
                capture_output=True,
                timeout=30,
            )
    ...
    def get_system_health_summary(self) -> Dict[str, Any]:
        ...
        recent_results = [
            r
            for r in self.diagnostic_history
            if datetime.now() - r.timestamp < timedelta(minutes=30)
        ]
        ...
        # Categorize results
        critical_count = len(
            [r for r in recent_results if r.level == DiagnosticLevel.CRITICAL]
        )
        error_count = len(
            [r for r in recent_results if r.level == DiagnosticLevel.ERROR]
        )
        warning_count = len(
            [r for r in recent_results if r.level == DiagnosticLevel.WARNING]
        )
        ...
        recovery_success_rate = (
            len([r for r in recent_recoveries if r.success])
