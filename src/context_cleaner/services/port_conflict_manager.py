...
class PortConflictManager:
    ...
    async def detect_port_conflicts(
        ...
        for service_name, port in service_ports.items():
            ...
            if not available and self.verbose:
                print(
                    f"⚠️  Port conflict detected: {service_name} port {port} - {error}"
                )
    ...
    async def start_retry_session(
        ...
        if self.verbose:
            print(
                f"🔄 Starting port retry session: {service_name} (original port: {original_port})"
            )
    ...
    async def find_available_port(
        ...
        """
        session = await self.start_retry_session(
            service_name, original_port, strategy, max_attempts
        )
        ...
        fallback_ports = self._generate_fallback_ports(
            original_port, service_name, strategy, max_attempts
        )

        if self.verbose:
            print(
                f"   Trying {len(fallback_ports)} fallback ports: {fallback_ports[:5]}{'...' if len(fallback_ports) > 5 else ''}"
            )
        ...
        for attempt_num, port in enumerate(fallback_ports, 1):
            ...
            if available:
                ...
                if self.verbose:
                    print(
                        f"✅ Found available port {port} for {service_name} (attempt {attempt_num})"
                    )
        ...
        session.error_message = (
            f"No available port found after {len(session.attempts)} attempts"
        )

        if self.verbose:
            print(
                f"❌ Port retry exhausted for {service_name} - no available ports found"
            )

        return None, session

    async def monitor_retry_session(
        self, service_name: str
    ) -> Optional[PortConflictSession]:
        """
    ...
    def get_retry_statistics(self) -> Dict[str, Any]:
        ...
        for session in self.active_sessions.values():
            ...
            if session.status == RetryStatus.SUCCESS:
                ...
                if session.successful_port:
                    stats["port_usage_frequency"][session.successful_port] = (
                        stats["port_usage_frequency"].get(session.successful_port, 0)
                        + 1
                    )
            ...
            if session.status == RetryStatus.SUCCESS:
                strategy_successes[strategy_name] = (
                    strategy_successes.get(strategy_name, 0) + 1
                )
...
class PortRegistry:
        ...
        with self._allocation_lock:
            ...
            if service_name in self.service_ports:
                ...
                if allocation and allocation.is_active:
                    return (
                        existing_port,
                        f"Service {service_name} already allocated port {existing_port}",
                    )
            ...
            if self._is_port_allocatable(target_port, service_type):
                allocated_port = self._perform_allocation(
                    service_name, service_type, target_port
                )
                return (
                    allocated_port,
                    f"Allocated preferred port {allocated_port} to {service_name}",
                )
            ...
            if force_preferred:
                return (
                    None,
                    f"Preferred port {target_port} unavailable and force_preferred=True",
                )
            ...
            for port in range(port_range[0], port_range[1] + 1):
                if self._is_port_allocatable(port, service_type):
                    ...
                    return (
                        allocated_port,
                        f"Allocated alternative port {allocated_port} to {service_name}",
                    )

            # No ports available in range
            return (
                None,
                f"No available ports in range {port_range} for service type {service_type}",
            )
    ...
    def _perform_allocation(
        self, service_name: str, service_type: str, port: int
    ) -> int:
            ...
            for service_name, port in self.service_ports.items():
                ...
                if allocation:
                    allocations[service_name] = {
                        ...
                        "last_health_check": (
                            allocation.last_health_check.isoformat()
                            if allocation.last_health_check
                            else None
                        ),
                    }
            return allocations
    ...
    def get_port_conflicts(self) -> Dict[str, Any]:
        ...
        for service_name, port in self.service_ports.items():
            ...
            if not available:
                conflicts["conflicted_services"].append(
                    {"service_name": service_name, "port": port, "error": error}
                )
        ...
        conflicts["total_conflicts"] = len(conflicts["conflicted_services"]) + len(
            conflicts["system_conflicts"]
        )
        return conflicts
