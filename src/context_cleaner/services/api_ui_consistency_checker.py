...
class APIUIConsistencyChecker:
    ...
    def __init__(
        ...
        self.check_interval = float(
            os.getenv("CONTEXT_CLEANER_CONSISTENCY_INTERVAL", "300")
        )
        self.ui_check_timeout = float(
            os.getenv("CONTEXT_CLEANER_CONSISTENCY_UI_TIMEOUT", "5.0")
        )
        self.startup_delay = float(
            os.getenv("CONTEXT_CLEANER_CONSISTENCY_STARTUP_DELAY", "15.0")
        )
        self.http_session_timeout = float(
            os.getenv("CONTEXT_CLEANER_CONSISTENCY_HTTP_TIMEOUT", "30.0")
        )
        self.http_connector_limit = int(
            os.getenv("CONTEXT_CLEANER_CONSISTENCY_HTTP_LIMIT", "4")
        )
        self.max_endpoints_per_cycle = int(
            os.getenv("CONTEXT_CLEANER_CONSISTENCY_ENDPOINT_BATCH", "5")
        )
        self.ui_min_content_length = int(
            os.getenv("CONTEXT_CLEANER_CONSISTENCY_MIN_HTML", "2000")
        )
        self.fd_logging_enabled = os.getenv(
            "CONTEXT_CLEANER_CONSISTENCY_LOG_FDS", "false"
        ...
        self.max_consecutive_failures = (
            10  # Increased from 3 to be more tolerant of temporary issues
        )
        ...
        self.startup_grace_period = (
            60.0  # seconds - 60 second grace period for system startup
        )
    ...
    def _define_api_endpoints(self) -> List[APIEndpointTest]:
        ...
        return [
            ...
            APIEndpointTest(
                ...
                expected_keys=["last_updated", "range", "summary", "timeline"],
            ),
            APIEndpointTest(
                "/api/code-patterns-analytics",
                expected_keys=["last_updated", "patterns"],
            ),
            APIEndpointTest(
                "/api/content-search",
                method="GET",
                expected_keys=["results", "status", "message"],
            ),
            # Analytics endpoints
            APIEndpointTest(
            ...
            APIEndpointTest(
                ...
                expected_keys=["alerts", "data", "last_updated", "status", "title"],
            ),
            APIEndpointTest(
            ...
            APIEndpointTest(
                ...
                method="GET",
                expected_keys=["status", "message", "example_query"],
                critical=False,
            ),
        ]
    ...
    async def discover_dashboard_port(self) -> Optional[int]:
        ...
        self.logger.info(
            "🔍 PORT_DISCOVERY: Starting dynamic port discovery for dashboard..."
        )
        ...
        try:
            ...
            self.logger.debug(
                "🔍 PORT_DISCOVERY: Scanning running processes for gunicorn..."
            )
            found_processes = 0
            ...
            for proc in psutil.process_iter(["pid", "name", "cmdline"]):
                try:
                    ...
                    if "gunicorn" in cmdline and "context_cleaner" in cmdline:
                        self.logger.info(
                            f"🔍 PORT_DISCOVERY: Found gunicorn process PID {proc.info['pid']}: {cmdline[:100]}..."
                        )
                        ...
                        for i, arg in enumerate(proc.info["cmdline"]):
                            if arg == "--bind" and i + 1 < len(proc.info["cmdline"]):
                                ...
                                self.logger.debug(
                                    f"🔍 PORT_DISCOVERY: Found --bind argument: {bind_arg}"
                                )
                                ...
                                if ":" in bind_arg:
                                    ...
                                    try:
                                        ...
                                        self.logger.info(
                                            f"🔍 PORT_DISCOVERY: ✅ Successfully discovered dashboard port {port} from gunicorn process PID {proc.info['pid']} in {discovery_duration:.2f}s"
                                        )
                                        return port
                                    except ValueError as ve:
                                        self.logger.warning(
                                            f"🔍 PORT_DISCOVERY: Invalid port format '{port_str}': {ve}"
                                        )
                                        continue
            ...
            self.logger.debug(
                f"🔍 PORT_DISCOVERY: Scanned {found_processes} processes, no gunicorn context_cleaner found"
            )
            ...
            self.logger.info(
                f"🔍 PORT_DISCOVERY: Testing {len(common_ports)} ports across multiple ranges for dashboard services..."
            )
            self.logger.debug(
                f"🔍 PORT_DISCOVERY: Port ranges: 8000-8019, 8050-8069, 8080-8129, 8200-8219, 8300-8309, 8400-8409, 8500-8509, 8600-8609, 8700-8709, 8800-8809, 8900-8909, 9000-9019, 9100-9109, 9200-9209"
            )
            ...
            for batch_start in range(0, len(common_ports), batch_size):
                ...
                self.logger.debug(
                    f"🔍 PORT_DISCOVERY: Testing batch {batch_start//batch_size + 1}: ports {batch_ports[0]}-{batch_ports[-1]} ({len(batch_ports)} ports)"
                )
                ...
                for i, port in enumerate(batch_ports):
                    ...
                    try:
                        ...
                        if port in [8110, 8080]:  # Expected or default ports
                            self.logger.info(
                                f"🔍 PORT_DISCOVERY: 🎯 Testing KEY port {port} ({overall_index+1}/{len(common_ports)})..."
                            )
                        elif overall_index % 50 == 0:  # Log progress every 50 ports
                            self.logger.info(
                                f"🔍 PORT_DISCOVERY: Progress: testing port {port} ({overall_index+1}/{len(common_ports)})..."
                            )
                        else:
                            self.logger.debug(
                                f"🔍 PORT_DISCOVERY: Testing port {port} ({overall_index+1}/{len(common_ports)})..."
                            )
                            ...
                            for test_endpoint in test_endpoints:
                                try:
                                    ...
                                    if port in [8110, 8080]:
                                        self.logger.info(
                                            f"🔍 PORT_DISCOVERY: 🎯 Making request to {test_url}"
                                        )

                                    async with session.get(test_url) as response:
                                        port_test_duration = (
                                            time.time() - port_test_start
                                        )

                                        if port in [8110, 8080]:
                                            self.logger.info(
                                                f"🔍 PORT_DISCOVERY: 🎯 Port {port} endpoint {test_endpoint} responded with status {response.status} in {port_test_duration:.2f}s"
                                            )
                                        else:
                                            self.logger.debug(
                                                f"🔍 PORT_DISCOVERY: Port {port} endpoint {test_endpoint} responded with status {response.status} in {port_test_duration:.2f}s"
                                        ...
                                        if response.status == 200:
                                            ...
                                            if test_endpoint in [
                                                ...
                                                self.logger.info(
                                                    f"🔍 PORT_DISCOVERY: ✅ Successfully discovered dashboard port {port} via {test_endpoint} endpoint in {discovery_duration:.2f}s"
                                                )
                                                return port
                                                ...
                                                try:
                                                    ...
                                                    if port in [8110, 8080]:
                                                        self.logger.info(
                                                            f"🔍 PORT_DISCOVERY: 🎯 Port {port} health response: {json.dumps(data, indent=2)}"
                                                        )
                                                    else:
                                                        self.logger.debug(
                                                            f"🔍 PORT_DISCOVERY: Port {port} health response: {json.dumps(data, indent=2)}"
                                                    ...
                                                    if isinstance(data, dict) and (
                                                        ...
                                                        self.logger.info(
                                                            f"🔍 PORT_DISCOVERY: ✅ Successfully discovered dashboard port {port} via health check in {discovery_duration:.2f}s"
                                                        )
                                                        return port
                                                except:
                                                    pass
                                                ...
                                                self.logger.info(
                                                    f"🔍 PORT_DISCOVERY: ✅ Successfully discovered dashboard port {port} via main page in {discovery_duration:.2f}s"
                                                )
                                                return port
                                    ...
                                    if port in [8110, 8080]:
                                        self.logger.debug(
                                            f"🔍 PORT_DISCOVERY: 🎯 Port {port} endpoint {test_endpoint} failed: {str(endpoint_error)[:50]}"
                                        )
                                    continue
                        ...
                        if port in [8110, 8080]:
                            self.logger.warning(
                                f"🔍 PORT_DISCOVERY: 🎯 Port {port} connection FAILED in {port_test_duration:.2f}s: {str(conn_error)}"
                            )
                        else:
                            self.logger.debug(
                                f"🔍 PORT_DISCOVERY: Port {port} connection failed in {port_test_duration:.2f}s: {str(conn_error)[:100]}"
                            )
                        continue

            discovery_duration = time.time() - discovery_start
            self.logger.warning(
                f"🔍 PORT_DISCOVERY: ❌ No dashboard port found after checking {len(common_ports)} ports in {discovery_duration:.2f}s"
            )

        except Exception as e:
            discovery_duration = time.time() - discovery_start
            self.logger.error(
                f"🔍 PORT_DISCOVERY: ❌ Discovery failed after {discovery_duration:.2f}s: {e}"
            )
            import traceback

            self.logger.error(
                f"🔍 PORT_DISCOVERY: Discovery error traceback:\n{traceback.format_exc()}"
            )
    ...
    async def update_dashboard_port_if_needed(self) -> bool:
        ...
        if discovered_port and discovered_port != self.dashboard_port:
            ...
            self.logger.info(
                f"🔄 Updated dashboard port from {old_port} to {discovered_port}"
            )
                ...
                if response.status == 200:
                    try:
                        ...
                        if missing_keys:
                            ...
                            self._record_circuit_breaker_result(
                                endpoint_test.path, "partial"
                            )
                            return result

                        result = ("success", response_time, data_size, None, data)
                        self._record_circuit_breaker_result(
                            endpoint_test.path, "success"
                        )
                        return result
                        ...
                        self._record_circuit_breaker_result(
                            endpoint_test.path, "invalid_json"
                        )
                        return result
    ...
    async def fetch_dashboard_html(
        ...
        try:
            ...
            self.logger.debug(
                f"🌐 DASHBOARD_HTML: Failed to fetch dashboard HTML: {exc}"
            )
    ...
    def _classify_dashboard_html(self, html: str) -> Tuple[str, Optional[str]]:
        ...
        if (
            "data-dashboard-error" in lowered
        ...
        ready_marker = any(
            marker.lower() in lowered for marker in self.ui_ready_markers
        )
        loading_marker = (
    ...
    async def wait_for_dashboard_ready(
        ...
        self.logger.info(
            f"🌐 DASHBOARD_WAIT: Starting dashboard readiness check for {self.base_url}"
        )
        self.logger.info(
            f"🌐 DASHBOARD_WAIT: Max wait: {max_wait}s, check interval: {check_interval}s"
        )
        ...
        while (time.time() - start_time) < max_wait:
            ...
            try:
                self.logger.debug(
                    f"🌐 DASHBOARD_WAIT: Attempt #{attempt_count} - checking {self.base_url}"
                )
                ...
                async with aiohttp.ClientSession(
                    ...
                    async with session.get(self.base_url) as response:
                        ...
                        if response.status == 200:
                            ...
                            self.logger.debug(
                                f"🌐 DASHBOARD_WAIT: Attempt #{attempt_count} - HTTP 200, content length: {last_content_length}"
                            )

                            if (
                                len(html_content) > 1000
                            ):  # Basic check that we got a real dashboard
                                elapsed_time = time.time() - wait_start_time
                                self.logger.info(
                                    f"🌐 DASHBOARD_READY: Dashboard is ready after {elapsed_time:.2f}s, {attempt_count} attempts"
                                )
                                self.logger.info(
                                    f"🌐 DASHBOARD_READY: Final status: HTTP {response.status}, content length: {last_content_length}"
                                )
                                return True
                            else:
                                self.logger.debug(
                                    f"🌐 DASHBOARD_WAIT: Content too short ({last_content_length} bytes), dashboard may not be fully loaded"
                                )
                        else:
                            self.logger.debug(
                                f"🌐 DASHBOARD_WAIT: Attempt #{attempt_count} - HTTP {response.status} (took {attempt_duration:.2f}s)"
                            )

            except asyncio.CancelledError:
                elapsed_time = time.time() - wait_start_time
                self.logger.info(
                    f"🌐 DASHBOARD_WAIT: Dashboard readiness check cancelled after {elapsed_time:.2f}s, {attempt_count} attempts"
                )
                ...
                last_error = str(e)
                self.logger.debug(
                    f"🌐 DASHBOARD_WAIT: Attempt #{attempt_count} failed after {attempt_duration:.2f}s: {e}"
                )
            ...
            if attempt_count % 10 == 0:  # Every 10 attempts
                elapsed_time = time.time() - wait_start_time
                self.logger.info(
                    f"🌐 DASHBOARD_WAIT: Still waiting... {attempt_count} attempts in {elapsed_time:.1f}s"
                )
                self.logger.info(
                    f"🌐 DASHBOARD_WAIT: Last status: {last_status_code}, last error: {last_error}"
                )
        ...
        elapsed_time = time.time() - wait_start_time
        self.logger.warning(
            f"🌐 DASHBOARD_TIMEOUT: Dashboard not ready after {elapsed_time:.2f}s ({attempt_count} attempts)"
        )
        self.logger.warning(
            f"🌐 DASHBOARD_TIMEOUT: Last status: {last_status_code}, last content length: {last_content_length}"
        )
        self.logger.warning(f"🌐 DASHBOARD_TIMEOUT: Last error: {last_error}")
        self.logger.warning(
            f"🌐 DASHBOARD_TIMEOUT: Proceeding with monitoring anyway..."
        )
        return False

    def determine_consistency_status(
        self, api_status: str, ui_status: str, api_data_size: int
    ) -> ConsistencyStatus:
        ...
        if api_status == "success" and ui_status == "loaded" and api_data_size > 0:
            ...
            api_status == "success"
            and api_data_size > 0
            and ui_status in ["loading", "minimal"]
        ):
    ...
    def generate_recommendations(self, result: ConsistencyCheckResult) -> List[str]:
        ...
        if result.api_response_time > 5.0:
            recommendations.append(
                f"API response time is slow ({result.api_response_time:.2f}s)"
            )
    ...
    async def run_consistency_check(
        ...
        if self.fd_logging_enabled:
            ...
            if fd_before is not None:
                self.logger.debug(
                    f"🔍 FD_MONITOR: descriptors before cycle={fd_before}"
                )
        ...
        try:
            ...
            for endpoint_test in endpoints_to_check:
                ...
                api_status, response_time, data_size, api_error, api_data = (
    ...
    def _select_endpoints_for_cycle(self) -> List[APIEndpointTest]:
        ...
        if self.max_endpoints_per_cycle <= 0 or self.max_endpoints_per_cycle >= len(
            self.api_endpoints
        ):
    ...
    def get_summary_report(self) -> Dict[str, Any]:
        ...
        if not self.last_check_results:
            if create_error_response:
                ...
                return {
                    "error": "No consistency check results available"
                }  # Fallback for testing
        ...
        slow_apis = sorted(
            [r for r in self.last_check_results.values()],
            key=lambda x: x.api_response_time,
            reverse=True,
        )[:5]
    ...
    async def start_monitoring(self):
        ...
        self.logger.info(
            "🚀 MONITOR_START: Starting API/UI consistency monitoring with error recovery..."
        )
        self.logger.info(
            f"🚀 MONITOR_START: Check interval: {self.check_interval}s, startup delay: {self.startup_delay}s"
        )

        try:
            # Add startup delay to allow dashboard to fully initialize
            self.logger.info(
                f"🔄 MONITOR_START: Waiting {self.startup_delay}s for dashboard to initialize..."
            )
            ...
            if self.dynamic_port_discovery:
                try:
                    self.logger.info(
                        "🔄 MONITOR_START: Attempting dynamic port discovery..."
                    )
                    port_updated = await self.update_dashboard_port_if_needed()
                    if port_updated:
                        self.logger.info(
                            "🔄 MONITOR_START: Dashboard port updated successfully"
                        )
                    else:
                        self.logger.info(
                            f"🔄 MONITOR_START: Using configured dashboard port {self.dashboard_port}"
                        )
                except Exception as e:
                    self.logger.warning(
                        f"🔄 MONITOR_START: Port discovery failed, continuing with port {self.dashboard_port}: {e}"
                    )
            ...
            try:
                ...
                if not dashboard_ready:
                    self.logger.warning(
                        "🚀 MONITOR_START: Dashboard not ready, attempting port discovery..."
                    )
                    ...
                    if self.dynamic_port_discovery:
                        try:
                            ...
                            if port_updated:
                                self.logger.info(
                                    "🚀 MONITOR_START: Port updated, retrying dashboard ready check..."
                                )
                                dashboard_ready = await self.wait_for_dashboard_ready()
                                if dashboard_ready:
                                    self.logger.info(
                                        "🚀 MONITOR_START: Dashboard now ready after port update!"
                                    )
                                else:
                                    self.logger.warning(
                                        "🚀 MONITOR_START: Dashboard still not ready after port update"
                                    )
                        except Exception as e:
                            self.logger.warning(
                                f"🚀 MONITOR_START: Port discovery failed: {e}"
                            )

                    if not dashboard_ready:
                        self.logger.warning(
                            "🚀 MONITOR_START: Dashboard not ready, but continuing anyway"
                        )
                else:
                    self.logger.info(
                        "🚀 MONITOR_START: Dashboard confirmed ready, beginning monitoring loop"
                    )
            except asyncio.CancelledError:
                startup_duration = time.time() - start_time
                self.logger.info(
                    f"🚀 MONITOR_CANCELLED: Dashboard ready check was cancelled after {startup_duration:.2f}s during startup"
                )
            ...
            while not self.shutdown_event.is_set():
                ...
                try:
                    self.logger.debug(
                        f"🔄 MONITOR_LOOP: Iteration #{loop_iteration} starting"
                    )
                    ...
                    connector = aiohttp.TCPConnector(
                        limit_per_host=self.http_connector_limit
                    )
                    ...
                    async with aiohttp.ClientSession(
                        ...
                        await asyncio.wait_for(
                            ...
                            * 0.8,  # Use 80% of interval as timeout
                        )
                    ...
                    self.logger.debug(
                        f"🔄 MONITOR_LOOP: Iteration #{loop_iteration} completed in {check_duration:.2f}s"
                    )

                    if old_failures > 0:
                        self.logger.info(
                            f"🔄 MONITOR_RECOVERY: Recovered from {old_failures} consecutive failures"
                        )
                    ...
                    if critical_issues:
                        self.logger.warning(
                            f"🔄 MONITOR_ISSUES: Found {len(critical_issues)} critical API/UI consistency issues:"
                        )
                        for issue in critical_issues:
                            self.logger.warning(
                                f"🔄 MONITOR_ISSUES:   {issue.endpoint}: {issue.consistency_status.value}"
                            )
                    ...
                    if sleep_time > 0:
                        self.logger.debug(
                            f"🔄 MONITOR_LOOP: Sleeping for {sleep_time:.2f}s until next check"
                        )
                        await asyncio.wait_for(
                    ...
                    if (
                        loop_duration > self.check_interval * 1.2
                    ):  # If loop took >20% longer than expected
                        self.logger.info(
                            f"🔄 MONITOR_SLOW: Loop iteration took {loop_duration:.2f}s (expected {self.check_interval}s)"
                        )
                    ...
                    self.logger.warning(
                        f"🔄 MONITOR_TIMEOUT: Iteration #{loop_iteration} timed out after {loop_duration:.2f}s"
                    )
                    self.logger.warning(
                        f"🔄 MONITOR_TIMEOUT: Failure {self.consecutive_failures}/{self.max_consecutive_failures}"
                    )

                    # Use exponential backoff for consecutive failures
                    backoff_sleep = min(
                        self.check_interval
                        * (2 ** min(self.consecutive_failures - 1, 3)),
                        300,
                    )
                    self.logger.warning(
                        f"🔄 MONITOR_TIMEOUT: Using exponential backoff: {backoff_sleep:.2f}s"
                    )
                    ...
                    self.consecutive_failures += 1
                    self.logger.error(
                        f"🔄 MONITOR_ERROR: Exception in iteration #{loop_iteration} after {loop_duration:.2f}s"
                    )
                    self.logger.error(
                        f"🔄 MONITOR_ERROR: Failure {self.consecutive_failures}/{self.max_consecutive_failures}: {str(e)}"
                    )

                    import traceback

                    self.logger.error(
                        f"🔄 MONITOR_ERROR: Traceback:\n{traceback.format_exc()}"
                    )

                    if self.consecutive_failures >= self.max_consecutive_failures:
                        self.logger.error(
                            "🔄 MONITOR_RECOVERY: Max consecutive failures reached, entering recovery mode..."
                        )
                        ...
                        if self.dynamic_port_discovery:
                            try:
                                self.logger.info(
                                    "🔄 MONITOR_RECOVERY: Attempting dynamic port discovery..."
                                )
                                port_updated = (
                                    await self.update_dashboard_port_if_needed()
                                )
                                if port_updated:
                                    self.logger.info(
                                        "🔄 MONITOR_RECOVERY: Port updated during recovery, may resolve connection issues"
                                    )
                            except Exception as port_error:
                                self.logger.warning(
                                    f"🔄 MONITOR_RECOVERY: Port discovery during recovery failed: {port_error}"
                                )

                        # Wait longer before retrying, but don't give up
                        recovery_sleep = self.check_interval * 5
                        self.logger.error(
                            f"🔄 MONITOR_RECOVERY: Recovery mode sleep: {recovery_sleep}s"
                        )
                        await asyncio.sleep(recovery_sleep)
                        self.consecutive_failures = 0  # Reset after extended wait
                        self.logger.info(
                            "🔄 MONITOR_RECOVERY: Exiting recovery mode, resetting failure count"
                        )
                    else:
                        ...
                        self.logger.warning(
                            f"🔄 MONITOR_ERROR: Using exponential backoff: {backoff_sleep:.2f}s"
                        )
                        await asyncio.sleep(backoff_sleep)
            ...
            self.logger.info(
                f"🛑 MONITOR_CANCELLED: Consistency monitoring task cancelled after {total_runtime:.2f}s"
            )
            self.logger.info(
                f"🛑 MONITOR_CANCELLED: Completed {loop_iteration} iterations, {self.consecutive_failures} consecutive failures"
            )
            raise
        except Exception as e:
            total_runtime = time.time() - start_time
            self.logger.error(
                f"🛑 MONITOR_FATAL: Fatal error in consistency monitoring after {total_runtime:.2f}s"
            )
            self.logger.error(
                f"🛑 MONITOR_FATAL: Completed {loop_iteration} iterations, {self.consecutive_failures} consecutive failures"
            )
            ...
            self.logger.error(
                f"🛑 MONITOR_FATAL: Fatal traceback:\n{traceback.format_exc()}"
            )
            raise
        finally:
            total_runtime = time.time() - start_time
            self.is_running = False
            self.logger.info(
                f"🛑 MONITOR_STOPPED: Consistency monitoring stopped after {total_runtime:.2f}s"
            )
            self.logger.info(
                f"🛑 MONITOR_STOPPED: Final stats - {loop_iteration} iterations, {self.consecutive_failures} consecutive failures"
            )
    ...
    def is_monitoring_healthy(self) -> bool:
        ...
        if startup_age < self.startup_grace_period:
            self.logger.debug(
                f"💊 HEALTH_INTERNAL: In startup grace period ({startup_age:.1f}s / {self.startup_grace_period}s)"
            )
        ...
        health_info = {
            ...
            "has_monitoring_task": hasattr(self, "monitoring_task")
            and self.monitoring_task is not None,
            "task_done": None,
            "task_cancelled": None,
        ...
        if not self.is_running:
            self.logger.warning(
                f"💊 HEALTH_INTERNAL: ❌ Service NOT RUNNING - consecutive_failures: {self.consecutive_failures}, task_status: {health_info}"
            )
            return False
        ...
        if self.consecutive_failures >= self.max_consecutive_failures:
            ...
            ui_loading_count = sum(
                1 for issue in recent_issues if "ui_loading" in issue.status.value
            )
            total_issues = len(recent_issues)

            if (
                total_issues > 0 and ui_loading_count / total_issues > 0.8
            ):  # More than 80% are UI loading issues
                self.logger.info(
                    f"💊 HEALTH_INTERNAL: 🔄 UI loading states detected ({ui_loading_count}/{total_issues}), not marking as unhealthy"
                )
                self.logger.info(
                    f"💊 HEALTH_INTERNAL: This suggests APIs work but WebSocket/real-time updates may be failing"
                )
                return True  # Don't restart for UI loading issues

            self.logger.error(
                f"💊 HEALTH_INTERNAL: ❌ TOO MANY FAILURES - {self.consecutive_failures}/{self.max_consecutive_failures} consecutive failures"
            )
            self.logger.error(
                f"💊 HEALTH_INTERNAL: Failure context - port: {self.dashboard_port}, base_url: {self.base_url}"
            )
            ...
            self.logger.warning(
                f"💊 HEALTH_INTERNAL: ⚠️  Approaching failure threshold - {self.consecutive_failures}/{self.max_consecutive_failures} failures ({failure_ratio:.1%})"
            )
        ...
        if hasattr(self, "monitoring_task") and self.monitoring_task:
            ...
            if task.done() and not task.cancelled():
                try:
                    ...
                    if exception:
                        self.logger.error(
                            f"💊 HEALTH_INTERNAL: Monitoring task has exception but is_running=True - {health_info}"
                        )
                        self.logger.error(
                            f"💊 HEALTH_INTERNAL: Task exception: {exception}"
                        )
                        return False
                except:
        ...
        if hasattr(self, "_last_health_log_time"):
            if (
                time.time() - self._last_health_log_time > 60
            ):  # Log every minute when healthy
                self.logger.info(
                    f"💊 HEALTH_INTERNAL: ✅ Service healthy - failures: {self.consecutive_failures}/{self.max_consecutive_failures}, port: {self.dashboard_port}"
                )
