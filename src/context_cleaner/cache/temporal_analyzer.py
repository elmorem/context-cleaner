...
class SessionBoundary:
    ...
    def is_strong_boundary(self) -> bool:
        ...
        return (
            self.confidence_score > 0.8 or
            self.time_gap_minutes > 60 or
            self.activity_change_score > 0.7
        )
...
class TemporalContextAnalyzer:
    ...
    def __init__(self, config: Optional[CacheConfig] = None):
        ...
        self.topic_keywords = {
            'coding': ['function', 'class', 'variable', 'import', 'def', 'return', 'if'],
            'debugging': ['error', 'bug', 'fix', 'issue', 'problem', 'exception', 'traceback'],
            'testing': ['test', 'assert', 'mock', 'unittest', 'pytest', 'coverage'],
            'documentation': ['readme', 'docs', 'document', 'comment', 'explain', 'describe'],
            'refactoring': ['refactor', 'cleanup', 'reorganize', 'improve', 'optimize'],
            'learning': ['how', 'what', 'why', 'learn', 'understand', 'explain', 'tutorial'],
    ...
    def analyze_temporal_patterns(self, 
                                 ...
                                 time_window_days: Optional[int] = 30) -> TemporalInsights:
        """
        ...
        all_sessions = []
        cutoff_date = datetime.now() - timedelta(days=time_window_days) if time_window_days else None
            ...
            for session_file in location.session_files:
                try:
                    ...
                    if (analysis and analysis.start_time and 
                        (cutoff_date is None or analysis.start_time >= cutoff_date)):
        ...
        temporal_metrics = self._calculate_temporal_metrics(all_sessions, topic_transitions)
        ...
        logger.info(f"Temporal analysis complete. Overall health: {insights.overall_temporal_health}")
    ...
    def _detect_session_boundaries(self, sessions: List[SessionAnalysis]) -> List[SessionBoundary]:
        ...
        for i in range(len(sessions) - 1):
            ...
            activity_change = self._calculate_activity_change(current_session, next_session)
    ...
    def _calculate_activity_change(self, session1: SessionAnalysis, session2: SessionAnalysis) -> float:
        """Calculate the degree of activity change between sessions."""
        # Compare token usage patterns
        token_ratio = abs(session2.total_tokens - session1.total_tokens) / max(session1.total_tokens, 1)
        token_change = min(token_ratio, 1.0)
        
        # Compare operation counts
        ops_ratio = abs(len(session2.file_operations) - len(session1.file_operations)) / max(len(session1.file_operations), 1)
        ops_change = min(ops_ratio, 1.0)
        
        # Compare message counts
        msg_ratio = abs(session2.total_messages - session1.total_messages) / max(session1.total_messages, 1)
        msg_change = min(msg_ratio, 1.0)
        
        # Combined activity change score
        return (token_change + ops_change + msg_change) / 3
    
    def _classify_boundary(self, gap_minutes: float, context1: str, context2: str, activity_change: float) -> Tuple[str, float]:
    ...
    def _analyze_topic_transitions(self, sessions: List[SessionAnalysis]) -> List[TopicTransition]:
        ...
        for i in range(len(session_topics) - 1):
            ...
            if current_topic != next_topic and current_session.end_time and next_session.start_time:
                time_gap = (next_session.start_time - current_session.end_time).total_seconds() / 60
                ...
                context_similarity = self._calculate_session_similarity(current_session, next_session)
    ...
    def _classify_transition_type(self, session1: SessionAnalysis, session2: SessionAnalysis, time_gap: float) -> str:
    ...
    def _calculate_transition_confidence(self, session1: SessionAnalysis, session2: SessionAnalysis, 
                                       topic1: str, topic2: str) -> float:
    ...
    def _calculate_session_similarity(self, session1: SessionAnalysis, session2: SessionAnalysis) -> float:
        ...
        files1 = set(op.file_path.split('.')[-1] for op in session1.file_operations if op.file_path and '.' in op.file_path)
        files2 = set(op.file_path.split('.')[-1] for op in session2.file_operations if op.file_path and '.' in op.file_path)
        
        file_similarity = len(files1.intersection(files2)) / max(len(files1.union(files2)), 1)
        ...
        tool_similarity = len(tools1.intersection(tools2)) / max(len(tools1.union(tools2)), 1)
        
        return (file_similarity + tool_similarity) / 2
    
    def _analyze_context_evolution(self, sessions: List[SessionAnalysis]) -> List[ContextEvolutionPattern]:
    ...
    def _create_evolution_pattern(self, sessions: List[SessionAnalysis]) -> Optional[ContextEvolutionPattern]:
        """Create an evolution pattern from a window of sessions."""
        if not sessions:
            return None
        
        # Extract topics sequence
        topics_sequence = [self._estimate_session_topic(session) for session in sessions]
        ...
        evolution_type = self._classify_evolution_type(topics_sequence, complexity_progression)
        ...
        dominant_topics = [(topic, count / total_sessions) 
        ...
        productivity_correlation = self._calculate_productivity_correlation(sessions, complexity_progression)
    ...
    def _classify_evolution_type(self, topics: List[str], complexity: List[float]) -> str:
    ...
    def _is_branching_pattern(self, topics: List[str], complexity: List[float]) -> bool:
        ...
        increases = sum(1 for i in range(1, len(unique_counts)) 
                       if unique_counts[i] > unique_counts[i-1])
        
        return increases > len(unique_counts) * 0.6
    
    def _is_convergent_pattern(self, topics: List[str], complexity: List[float]) -> bool:
        """Check if pattern shows convergence (decreasing diversity)."""
        if len(complexity) < 3:
            return False
        
        # Check if complexity generally decreases
        decreases = sum(1 for i in range(1, len(complexity)) 
    ...
    def _calculate_productivity_correlation(self, sessions: List[SessionAnalysis], complexity: List[float]) -> float:
    ...
    def _calculate_session_metrics(self, sessions: List[SessionAnalysis]) -> Dict[str, float]:
        ...
        for i in range(len(sessions) - 1):
            if sessions[i].end_time and sessions[i+1].start_time:
                break_duration = (sessions[i+1].start_time - sessions[i].end_time).total_seconds() / 3600
        ...
        if productivity_scores:
            ...
            avg_productivity = {d: statistics.mean(scores) 
                             for d, scores in duration_groups.items() if len(scores) >= 2}
        ...
        recommended_breaks = optimal_length * 60 / 2  # Half the optimal session length in minutes
    ...
    def _calculate_temporal_metrics(self, sessions: List[SessionAnalysis], 
                                   transitions: List[TopicTransition]) -> Dict[str, float]:
        ...
        multitasking = statistics.mean(avg_concurrent_files) if avg_concurrent_files else 0.0
    ...
    def _analyze_productivity_patterns(self, sessions: List[SessionAnalysis]) -> Dict[str, Any]:
        """Analyze productivity patterns over time."""
        hourly_productivity = defaultdict(list)
        peak_periods = []
        
        for session in sessions:
            if session.start_time and session.duration_hours > 0 and session.total_messages > 0:
        ...
        if hourly_patterns:
            ...
            high_productivity_hours = [int(h) for h, p in hourly_patterns.items() 
                                     if p > avg_productivity * 1.2]
    ...
    def _calculate_analysis_confidence(self, sessions: List[SessionAnalysis]) -> float:
        ...
        # Data quality factor
        sessions_with_good_data = sum(1 for s in sessions 
                                    if s.total_messages > 5 and s.duration_hours > 0.1)
