...
from context_cleaner.telemetry import (
    ClickHouseClient,
...
def _wait_for_clickhouse(
    verbose: bool = False, retries: int = 12, delay: float = 5.0
) -> None:
...
@click.option(
    ...
    default=False,
    show_default=True,
    help="Pull container images before startup",
)
...
def init(ctx, pull: bool, force_recreate: bool):
    ...
    if pull:
        _run_compose_command(
            telemetry_dir,
            ["pull", "clickhouse", "otel-collector"],
            verbose=verbose,
            timeout=300,
        )
    ...
    async def _analyze():
        try:
            ...
            for error_type, details in stats.get("error_types", {}).items():
                ...
                click.echo(
                    f"    Models Affected: {', '.join(details['models_affected'])}"
                )
                ...
                if suggestions:
                    ...
                    for suggestion in suggestions:
                        ...
                        if suggestion.get("expected_savings"):
                            click.echo(
                                f"    Expected Savings: {suggestion['expected_savings']}"
                            )
...
@click.option(
    "--session-budget", type=float, default=2.0, help="Session budget limit in USD"
)
@click.option(
    "--daily-budget", type=float, default=5.0, help="Daily budget limit in USD"
)
def cost_optimize(
    ...
    async def _optimize():
        try:
            ...
            session_status = (
                "🟢" if session_pct < 50 else "🟡" if session_pct < 80 else "🔴"
            )
            daily_status = "🟢" if daily_pct < 50 else "🟡" if daily_pct < 80 else "🔴"

            click.echo(f"\n📊 Budget Status:")
            click.echo(
                f"Session Budget:   {session_status} {session_pct:.1f}% of ${session_budget:.2f}"
            )
            click.echo(
                f"Daily Budget:     {daily_status} {daily_pct:.1f}% of ${daily_budget:.2f}"
            )
            ...
            if suggestions:
                ...
                for suggestion in suggestions:
                    ...
                    if suggestion.expected_savings_percent:
                        click.echo(
                            f"    Expected Savings: {suggestion.expected_savings_percent:.0f}%"
                        )
            ...
            if analysis.vs_daily_avg != 0:
                ...
                click.echo(
                    f"📊 Daily cost is {abs(analysis.vs_daily_avg):.1f}% {trend} average"
                )

            if analysis.vs_session_avg != 0:
                trend = "above" if analysis.vs_session_avg > 0 else "below"
                click.echo(
                    f"📊 Session cost is {abs(analysis.vs_session_avg):.1f}% {trend} average"
                )
...
@click.option("--session-id", help="Session ID (defaults to current session)")
@click.option(
    "--budget-aware/--ignore-budget",
    default=True,
    help="Consider current budget in recommendation",
)
def model_recommend(
    task_description: str, session_id: Optional[str], budget_aware: bool
):
    ...
    async def _recommend():
        try:
            ...
            model_name = (
                recommendation.model.value.replace("claude-", "")
                .replace("-20250514", "")
                .replace("-20241022", "")
            )
            confidence_bar = "█" * int(recommendation.confidence * 10) + "░" * (
                10 - int(recommendation.confidence * 10)
            )

            click.echo(f"Recommended Model: {model_name.upper()}")
            click.echo(
                f"Confidence:        {confidence_bar} {recommendation.confidence:.0%}"
            )
            ...
            if recommendation.cost_savings:
                click.echo(
                    f"💰 Cost Savings:    ${recommendation.cost_savings:.4f} vs Sonnet"
                )
                savings_pct = (
                    recommendation.cost_savings
                    / (recommendation.expected_cost + recommendation.cost_savings)
                ) * 100
    ...
    async def _insights():
        try:
            ...
            if session_metrics.api_calls > 0:
                ...
                tokens_per_call = (
                    session_metrics.total_input_tokens / session_metrics.api_calls
                )
                click.echo(f"\n⚡ Efficiency:")
...
def health_check():
    ...
    async def _health_check():
        try:
            ...
            total_requests = sum(
                stats["request_count"] for stats in model_stats.values()
            )
            ...
            if model_stats:
                ...
                for model, stats in model_stats.items():
                    ...
                    click.echo(
                        f"   • {model_name}: {stats['request_count']} requests, ${stats['total_cost']:.3f}"
                    )
