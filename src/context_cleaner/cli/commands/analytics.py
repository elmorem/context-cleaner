...
def forecast(ctx, horizon, type, format):
    ...
    try:
        ...
        if not predictions:
            click.echo(
                "❌ No predictions could be generated. Please check your data and try again."
            )
            return
        ...
        if format == "json":
            ...
            for pred in predictions:
                ...
                click.echo(
                    f"   Confidence Interval: [{pred.confidence_interval[0]:.2f}, {pred.confidence_interval[1]:.2f}]"
                )
                click.echo(f"   Model Accuracy: {pred.model_accuracy:.1%}")
                click.echo(
                    f"   Generated: {pred.created_at.strftime('%Y-%m-%d %H:%M:%S')}"
                )
...
def content_analysis(ctx, conversation_id, content, format):
    ...
    if not conversation_id and not content:
        click.echo(
            "❌ Please provide either --conversation-id or --content for analysis"
        )
        sys.exit(1)
    ...
    try:
        ...
        engine = get_content_intelligence_engine(
            config.to_dict() if hasattr(config, "to_dict") else {}
        )
        ...
        async def run_analysis():
            if content:
                ...
                semantic_result = await engine.semantic_analyzer.analyze_content(
                    content
                )
                return {
                    "semantic_analysis": (
                        semantic_result.to_dict() if semantic_result else {}
                    ),
                    "summary": {
        ...
        if format == "json":
            ...
            if "semantic_analysis" in results:
                ...
                click.echo(
                    f"   Sentiment: {semantic.get('sentiment_label', 'N/A')} ({semantic.get('sentiment_score', 0):.2f})"
                )
                click.echo(
                    f"   Complexity Score: {semantic.get('complexity_score', 0):.2f}"
                )
            ...
            if "knowledge_extraction" in results:
                ...
                for concept in knowledge[:3]:  # Show top 3
                    click.echo(
                        f"   - {concept.get('concept', 'N/A')} (importance: {concept.get('importance_score', 0):.2f})"
                    )
        ...
        if format == "json":
            ...
            if full and "comprehensive_bi_report" in report:
                ...
                if "executive_dashboard" in bi_report:
                    exec_data = bi_report["executive_dashboard"].get(
                        "executive_summary", {}
                    )
                        ...
                        for category, category_metrics in metrics.items():
                            ...
                            for metric_name, metric_data in category_metrics.items():
                                ...
                                trend_icon = (
                                    "📈"
                                    if trend == "up"
                                    else "📉" if trend == "down" else "➡️"
                                )
                                click.echo(
                                    f"     {trend_icon} {metric_data.get('metric_name', metric_name)}: {value} {unit} ({trend_pct:+.1f}%)"
                                )
                    ...
                    if "insights" in exec_data:
                        ...
                        for insight in exec_data["insights"][:3]:
                            impact_icon = (
                                ...
                                else "🟡" if insight["impact"] == "medium" else "🟢"
                            )
                    ...
                    if "recommendations" in exec_data:
                        ...
                        for rec in exec_data["recommendations"][:3]:
                            ...
                            click.echo(
                                f"   {priority_icon} {rec['title']} ({rec['timeline']})"
                            )
                ...
                if "key_metrics" in exec_summary:
                    ...
                    for category, category_metrics in metrics.items():
                        click.echo(f"\n   {category.replace('_', ' ').title()}:")
                        for metric_name, metric_data in list(category_metrics.items())[
                            :2
                        ]:  # Limit to 2 per category
                            value = metric_data.get("value", 0)
                            unit = metric_data.get("unit", "")
                            click.echo(
                                f"     • {metric_data.get('metric_name', metric_name)}: {value} {unit}"
                            )
            ...
            if "benchmark_report" in report:
                ...
                if "metrics_comparison" in benchmark:
                    ...
                    for metric, data in comparisons.items():
                        ...
                        click.echo(
                            f"      Industry: {data.get('industry_benchmark', 'N/A')}"
                        )
                ...
                if "ranking" in benchmark:
                    ...
                    click.echo(
                        f"   Overall Percentile: {ranking.get('overall_percentile', 'N/A')}"
                    )
                    click.echo(
                        f"   Market Position: {ranking.get('market_position', 'N/A')}"
                    )
...
def warnings(ctx, format):
    ...
    try:
        ...
        async def check_warnings():
            ...
            warnings = await engine.check_early_warnings(
                0.65, current_metrics
            )  # Sample health score
            return warnings
        ...
        if format == "json":
            click.echo(
                json.dumps([warning.to_dict() for warning in warnings], indent=2)
            )
        else:
            # Table format
            if not warnings:
            ...
            for warning in warnings:
                ...
                click.echo(
                    f"   Time to Event: {warning.time_to_event.total_seconds() / 3600:.1f} hours"
                )
