...
def _list_processes_impl(ctx, format, service_type, status):
    ...
    try:
        ...
        if format == "json":
            ...
            click.echo("=" * 100)
            click.echo(
                f"{'PID':<8} {'Service':<15} {'Status':<10} {'Port':<6} {'Start Time':<20} {'Health':<8}"
            )
            click.echo("-" * 100)
            ...
            for process in processes:
                health_status = (
                    "✓"
                    if process.last_health_status
                    else "✗" if process.last_health_check else "?"
                )
                start_time_str = process.start_time.strftime("%m-%d %H:%M:%S")
                port_str = str(process.port) if process.port else "-"

                click.echo(
                    f"{process.pid:<8} {process.service_type:<15} {process.status:<10} {port_str:<6} {start_time_str:<20} {health_status:<8}"
                )
...
@click.option(
    "--register", is_flag=True, help="Register discovered processes in the registry"
)
@click.option(
    "--format",
    "-f",
...
def discover_services(ctx, register, format):
    ...
    try:
        ...
        if format == "json":
            ...
            click.echo("=" * 100)
            click.echo(
                f"{'PID':<8} {'Service':<15} {'Port':<6} {'Start Time':<20} {'Command':<45}"
            )
            click.echo("-" * 100)
            ...
            for process in discovered:
                ...
                port_str = str(process.port) if process.port else "-"
                command_short = (
                    process.command_line[:42] + "..."
                    if len(process.command_line) > 45
                    else process.command_line
                )

                click.echo(
                    f"{process.pid:<8} {process.service_type:<15} {port_str:<6} {start_time_str:<20} {command_short:<45}"
                )
    ...
    try:
        ...
        if dry_run:
            ...
            for entry in stale_entries:
                last_check = (
                    entry.last_health_check.strftime("%m-%d %H:%M:%S")
                    if entry.last_health_check
                    else "Never"
                )
                click.echo(
                    f"{entry.pid:<8} {entry.service_type:<15} {entry.status:<10} {last_check:<20}"
                )
...
@click.option(
    "--update-registry", is_flag=True, help="Update health status in registry"
)
@click.pass_context
def health_check(ctx, update_registry):
    ...
    try:
        ...
        click.echo("=" * 80)
        click.echo(
            f"{'PID':<8} {'Service':<15} {'Port':<6} {'Process':<8} {'Network':<8} {'Overall':<8}"
        )
        click.echo("-" * 80)
        ...
        for process in processes:
            ...
            click.echo(
                f"{process.pid:<8} {process.service_type:<15} {port_str:<6} {process_status:<8} {network_status:<8} {overall_status:<8}"
            )
...
@click.option(
    "--show-all", is_flag=True, help="Show all processes (not just Context Cleaner)"
)
@click.pass_context
def process_tree(ctx, show_all):
    ...
    try:
        ...
        def find_context_cleaner_processes():
            ...
            for proc in psutil.process_iter(
                ...
                try:
                    ...
                    if (
                        "context_cleaner" in cmdline
                        or "context-cleaner" in cmdline
                        or (
                            info["name"]
                            and "python" in info["name"]
        ...
        def print_process(proc, indent):
            ...
            cmdline_short = (
                proc["cmdline"][:60] + "..."
                if len(proc["cmdline"]) > 63
                else proc["cmdline"]
            )

            click.echo(
                f"{indent_str}├─ PID {proc['pid']} ({create_time_str}) {cmdline_short}"
            )
...
def test_registry(ctx):
    ...
    try:
        ...
        if retrieved:
            ...
            update_success = registry.register_process(
                retrieved
