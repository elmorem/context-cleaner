...
class AnalyticsCommandHandler:
    ...
    def __init__(self, config: Optional[ContextCleanerConfig] = None, verbose: bool = False):
    ...
    def handle_health_check_command(self, 
                                  detailed: bool = False,
    ...
    def handle_export_analytics_command(self,
        ...
        try:
            ...
            analytics_data = self._gather_comprehensive_analytics(days, include_sessions)
    ...
    def handle_effectiveness_stats_command(self,
                                         days: int = 30,
        ...
        """
        if self.verbose and format != "json":
            click.echo(f"📈 Analyzing optimization effectiveness for last {days} days...")
        
        try:
            effectiveness_data = self.effectiveness_tracker.get_effectiveness_summary(days)
    ...
    def handle_enhanced_dashboard_command(self,
    ...
    def _perform_health_check(self, detailed: bool = False) -> Dict[str, Any]:
        ...
        health_results["checks"]["data_directory"] = {
            ...
            "details": {
                ...
                "writable": data_dir.is_dir() and data_dir.exists() and data_dir.stat().st_mode
            }
        }
        ...
        health_results["checks"]["effectiveness_tracking"] = {
            ...
            "details": {
                ...
                "sessions_file_exists": (effectiveness_dir / "optimization_sessions.jsonl").exists()
            }
        }
        health_results["checks_performed"] += 1
    ...
    def _display_health_check_results(self, health_results: Dict[str, Any], detailed: bool) -> None:
        ...
        if detailed or health_results["issues_found"] > 0 or health_results["warnings_found"] > 0:
            click.echo("\n📋 DETAILED RESULTS")
            click.echo("-" * 20)
            
            for check_name, check_result in health_results["checks"].items():
                status_icon = {"pass": "✅", "warning": "⚠️", "fail": "❌"}
                click.echo(f"{status_icon.get(check_result['status'], '?')} {check_name}")
    ...
    def _gather_comprehensive_analytics(self, days: int, include_sessions: bool) -> Dict[str, Any]:
    ...
    def _display_effectiveness_stats(self, data: Dict[str, Any], 
                                   strategy: Optional[str], detailed: bool) -> None:
        ...
        click.echo(f"• Context size reduction: {metrics['size_reduction_percentage']:.1f}%")
        ...
        click.echo(f"• Estimated time saved: {impact['total_time_saved_estimate_hours']:.1f} hours")
        ...
        if detailed and data["strategy_effectiveness"]:
            ...
            for strategy_name, stats in data["strategy_effectiveness"].items():
                if not strategy or strategy_name == strategy:
                    ...
                    click.echo(f"  - Avg size reduction: {stats['avg_size_reduction']:.1f}%")
                    click.echo(f"  - Avg health improvement: {stats['avg_health_improvement']:.1f}")
    ...
    def _get_enhanced_dashboard_data(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "recent_effectiveness": self.effectiveness_tracker.get_effectiveness_summary(7),
    ...
    def _display_enhanced_dashboard(self, data: Dict[str, Any], 
                                  interactive: bool, operations: bool) -> None:
        ...
        click.echo(f"System Status: {status_emoji[health['overall_status']]} {health['overall_status'].title()}")
        
        # Recent effectiveness
        effectiveness = data["recent_effectiveness"]
        if effectiveness["total_sessions"] > 0:
            click.echo(f"Recent success rate: {effectiveness['success_rate_percentage']:.1f}%")
            click.echo(f"Avg size reduction: {effectiveness['average_metrics']['size_reduction_percentage']:.1f}%")
    ...
    def _handle_interactive_dashboard(self) -> None:
        ...
        click.echo("Coming in future version - will provide real-time operation triggers")
        ...
        for check_name, check_result in health_results["checks"].items():
            if check_result["status"] == "fail":
                if check_name == "data_directory":
                    ...
                    try:
                        effectiveness_dir = Path(self.config.data_directory) / "effectiveness"
                        effectiveness_dir.mkdir(parents=True, exist_ok=True)
                        click.echo(f"✅ Fixed: Created effectiveness tracking directory")
                        fixed_count += 1
                    except Exception as e:
    ...
    def _get_cross_session_insights(self) -> Dict[str, Any]:
        ...
        try:
            import asyncio
            insights = asyncio.run(self.cross_session_analyzer.analyze_cross_session_patterns([]))
            return {
                "status": "available",
                "total_sessions": len(insights.session_clusters) if insights else 0,
                "patterns_found": len(insights.cross_session_patterns) if insights else 0
            }
        except Exception as e:
            return {"status": "unavailable", "message": f"Cross-session insights error: {e}"}
    ...
    def _get_smart_recommendations(self) -> List[str]:
        ...
        if recent_data["total_sessions"] == 0:
            recommendations.append("Try running your first optimization to see effectiveness metrics")
        elif recent_data["success_rate_percentage"] < 80:
            recommendations.append("Consider using Conservative mode for more reliable optimizations")
        elif recent_data["average_metrics"]["size_reduction_percentage"] < 20:
            recommendations.append("Try Aggressive mode for greater context size reduction")
        
        # Cache-based recommendations would be added here
        recommendations.append("Enable cache integration for personalized optimization strategies")
    ...
    def _display_export_summary(self, analytics_data: Dict[str, Any]) -> None:
        ...
        click.echo(f"• Export timestamp: {analytics_data.get('export_metadata', {}).get('timestamp', 'unknown')}")