...
class OptimizationCommandHandler:
    ...
    def handle_quick_optimization(self, context_data: Optional[Dict[str, Any]] = None) -> None:
        """
        ...
        try:
            ...
            approval_system, approval_id = create_quick_approval(plan.operations, auto_approve_safe=True)
            ...
            if approved_ops:
                ...
                if result.operations_executed > 0:
                    ...
                    total_reduction = sum(op.estimated_token_impact for op in plan.operations if op.operation_id in approved_ops)
                    click.echo(f"   • Estimated token reduction: {abs(total_reduction):,}")
            else:
    ...
    def handle_preview_mode(self, 
        ...
        try:
            ...
            # Generate preview
            preview_format = PreviewFormat.JSON if format == "json" else PreviewFormat.TEXT
            ...
            if format == "json":
                preview_data = {
                    ...
                    'safety_level': preview.overall_risk.value if hasattr(preview, 'overall_risk') else 'unknown',
                    'operations': []
                }
                ...
                for op_preview in preview.operation_previews:
                    preview_data['operations'].append({
                        ...
                        'impact': op_preview.estimated_impact.get('token_reduction', 0),
                ...
                click.echo(f"Estimated Token Reduction: {abs(preview.total_size_reduction):,}")
                click.echo()
                
                if preview.operation_previews:
                    click.echo("Planned Changes:")
                    for i, op_preview in enumerate(preview.operation_previews, 1):
                        click.echo(f"  {i}. {op_preview.operation.operation_type.upper()}: {op_preview.operation.reasoning}")
                        impact = op_preview.estimated_impact.get('token_reduction', 0) if isinstance(op_preview.estimated_impact, dict) else 0
                        click.echo(f"     Impact: {impact:+d} tokens "
                                 f"(Confidence: {op_preview.operation.confidence_score:.1%})")
                else:
                    click.echo("No optimization changes needed - context is already well-structured")
    ...
    def handle_aggressive_optimization(self, context_data: Optional[Dict[str, Any]] = None) -> None:
        """
        ...
        try:
            ...
            plan = manager.generate_optimization_plan(session.session_id, StrategyType.AGGRESSIVE)
            
            if len(plan.operations) == 0:
                click.echo("✅ Context already optimized - no aggressive changes available")
                return
            ...
            click.echo(f"   • Estimated reduction: {abs(plan.estimated_total_reduction):,} tokens")
            click.echo()
    ...
    def handle_focus_mode(self, context_data: Optional[Dict[str, Any]] = None) -> None:
        ...
        try:
            ...
            plan = manager.generate_optimization_plan(session.session_id, StrategyType.FOCUS)
            ...
            if result.success:
                ...
                click.echo(f"   • {result.operations_executed} reordering operations applied")
    ...
    def handle_full_optimization(self, context_data: Optional[Dict[str, Any]] = None) -> None:
        """
        ...
        try:
            ...
            strategy_choices = [s.value for s in StrategyType if s != StrategyType.WORKFLOW_SPECIFIC]
            ...
            # Generate plan
            plan = manager.generate_optimization_plan(session.session_id, selected_strategy)
...
def create_optimization_handler(config: Optional[Dict[str, Any]] = None, 
                               verbose: bool = False) -> OptimizationCommandHandler:
    """Create an optimization command handler."""
    return OptimizationCommandHandler(config, verbose)


def execute_quick_optimization(context_data: Optional[Dict[str, Any]] = None,
...
def execute_preview_mode(context_data: Optional[Dict[str, Any]] = None,
...
def execute_aggressive_optimization(context_data: Optional[Dict[str, Any]] = None,
                                  verbose: bool = False) -> None:
...
def execute_focus_mode(context_data: Optional[Dict[str, Any]] = None,
                      verbose: bool = False) -> None:
...
def execute_full_optimization(context_data: Optional[Dict[str, Any]] = None,
                             verbose: bool = False) -> None:
