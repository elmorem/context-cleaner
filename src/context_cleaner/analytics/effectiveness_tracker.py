...
class EffectivenessTracker:
    ...
    def __init__(self, data_directory: Optional[Path] = None):
        ...
        self.data_directory = data_directory or Path.home() / ".context_cleaner" / "effectiveness"
    ...
    def start_optimization_tracking(self, 
        ...
        self._store_initial_metrics(session_id, context_data, strategy_type, context_source)
        
        return session_id
    
    def complete_optimization_tracking(self,
                                     session_id: str,
                                     ...
                                     user_feedback: Optional[str] = None) -> OptimizationSession:
    ...
    def get_effectiveness_summary(self, days: int = 30) -> Dict[str, Any]:
        ...
        total_size_reduction = sum(s.metrics.size_reduction_percentage for s in sessions)
        ...
        rated_sessions = [s for s in sessions if s.metrics.user_satisfaction_rating is not None]
        avg_satisfaction = (
            sum(s.metrics.user_satisfaction_rating for s in rated_sessions) / len(rated_sessions)
        ...
        for session in sessions:
            ...
            stats["success_rate"] += 1 if session.outcome == OptimizationOutcome.SUCCESS else 0
        ...
        return {
            ...
            "successful_sessions": len([s for s in sessions if s.outcome == OptimizationOutcome.SUCCESS]),
            "success_rate_percentage": (len([s for s in sessions if s.outcome == OptimizationOutcome.SUCCESS]) / len(sessions)) * 100,
            ...
            "total_impact": {
                ...
                "total_duplicates_removed": sum(s.metrics.duplicates_removed for s in sessions),
                "total_stale_items_removed": sum(s.metrics.stale_items_removed for s in sessions),
                "total_items_consolidated": sum(s.metrics.items_consolidated for s in sessions),
                "total_time_saved_estimate_hours": self._estimate_time_saved(sessions)
            },
            
            "strategy_effectiveness": strategy_stats,
            "sessions": [s.to_dict() for s in sessions[-10:]]  # Last 10 sessions for detail
        }
    ...
    def _store_initial_metrics(self, session_id: str, context_data: ContextData, 
                             strategy_type: str, context_source: str) -> None:
        """Store initial context metrics for later comparison."""
    ...
    def _calculate_effectiveness_metrics(self,
                                       ...
                                       user_feedback: Optional[str]) -> EffectivenessMetrics:
        ...
        optimized_health = self._analyze_context_health(optimized_context)['health_score']
    ...
    def _serialize_context_data(self, context_data: ContextData) -> Dict[str, Any]:
        ...
        return {
            "content_preview": str(context_data)[:1000] + "..." if len(str(context_data)) > 1000 else str(context_data),
    ...
    def _estimate_time_saved(self, sessions: List[OptimizationSession]) -> float:
        ...
        successful_sessions = [s for s in sessions if s.outcome == OptimizationOutcome.SUCCESS]
