...
class SemanticAnalyzer:
    ...
    async def analyze_content(
        ...
        try:
            # Generate unique ID
            insight_id = (
                content_id or f"semantic_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
            )
    ...
    async def _calculate_quality(self, content: str) -> float:
        ...
        try:
            ...
            structure_score = min(
                sentence_count / 10.0, 1.0
            )  # Favor structured content
            ...
            quality = (
                length_score * 0.4 + structure_score * 0.3 + engagement_score * 0.3
            )
        ...
        try:
            ...
            for chunk in entities:
                if hasattr(chunk, "label"):
                    ...
                    named_entities.append({"text": entity_name, "label": chunk.label()})
    ...
    async def _extract_key_phrases(self, content: str) -> List[str]:
        ...
        try:
            ...
            vectorizer = TfidfVectorizer(
                ngram_range=(2, 3), stop_words="english", max_features=20
            )
...
class ConversationFlowAnalyzer:
    ...
    async def analyze_flow(
        ...
        try:
            ...
            flow_id = (
                f"flow_{conversation_id}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
            )

            # Basic metrics
            total_turns = len(conversation_data)
            total_length = sum(
                len(turn.get("content", "")) for turn in conversation_data
            )
    ...
    async def _count_topic_switches(
        self, conversation_data: List[Dict[str, Any]]
    ) -> int:
        ...
        try:
            ...
            for i, turn in enumerate(conversation_data):
                ...
                if has_question and i + 1 < len(conversation_data):
                    ...
                    if (
                        len(next_content) > 20
                    ):  # Assume substantial responses are answers
                        qa_pairs += 1
    ...
    async def _calculate_efficiency(
        ...
        try:
            ...
            conciseness_score = max(
                0, 1.0 - (avg_turn_length / 1000)
            )  # Penalize very long turns
            productivity_score = min(
                total_turns / 20.0, 1.0
            )  # Reward active conversations
...
class KnowledgeExtractor:
    ...
    async def extract_knowledge(
        ...
        try:
            ...
            # Combine all content
            all_content = " ".join(
                [turn.get("content", "") for turn in conversation_data]
            )
            ...
            if semantic_result:
                ...
                for i, concept in enumerate(semantic_result.key_concepts[:10]):
                    ...
                    importance_score = await self._calculate_concept_importance(
                        concept, all_content
                    )
                    ...
                    knowledge_node = KnowledgeNode(
                        ...
                        source_conversations=[
                            conversation_data[0].get("conversation_id", "unknown")
                        ],
    ...
    async def _calculate_concept_importance(self, concept: str, content: str) -> float:
        ...
        try:
            ...
            normalized_frequency = (
                frequency / content_length if content_length > 0 else 0
            )
    ...
    async def _find_related_concepts(
        self, concept: str, all_concepts: List[str]
    ) -> List[str]:
...
class ContentIntelligenceEngine:
    ...
    async def analyze_conversation(
        ...
        try:
            ...
            # Semantic analysis
            semantic_result = await self.semantic_analyzer.analyze_content(
                all_content, conversation_id
            )
            if semantic_result:
            ...
            knowledge_nodes = await self.knowledge_extractor.extract_knowledge(
                conversation_data
            )
            results["knowledge_extraction"] = [
                node.to_dict() for node in knowledge_nodes
            ]
            ...
            results["summary"] = {
                ...
                "topics_identified": (
                    len(semantic_result.topics) if semantic_result else 0
                ),
...
def get_content_intelligence_engine(
