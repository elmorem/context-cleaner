...
class TestRedundancyDetector:
    ...
    async def test_detect_exact_duplicates(self):
        ...
        duplicate_items = [item for item in report.duplicate_items if item['type'] == 'exact_duplicate']
    ...
    async def test_detect_obsolete_todos(self):
        ...
        obsolete_items = [item for item in report.obsolete_items if item['type'] == 'obsolete_todo']
...
class TestRedundancyReport:
    ...
    def test_get_redundancy_summary(self):
        ...
        report = RedundancyReport(
            ...
            duplicate_items=[], obsolete_items=[], redundant_file_groups=[],
            stale_error_messages=[], total_items_analyzed=0,
            total_estimated_tokens=0, redundancy_analysis_duration=0,
            safe_to_remove=[], consolidation_candidates=[]
        )
