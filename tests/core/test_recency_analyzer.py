...
from src.context_cleaner.core.recency_analyzer import (
    RecencyAnalyzer, RecencyReport
)
...
class TestRecencyAnalyzer:
    ...
    def test_calculate_session_activity_high_activity(self):
        ...
        activity_score = self.analyzer._calculate_session_activity(timestamps, session_start)
    ...
    def test_calculate_session_activity_low_activity(self):
        ...
        timestamps = [
            session_start,
            session_start + timedelta(minutes=60),
            now
        ]
    ...
    def test_categorize_by_timestamp_with_session(self):
        ...
        category = self.analyzer._categorize_by_timestamp(timestamp_in_session, session_start)
        assert category == 'recent'
        
        # Timestamp before session should not be recent
        timestamp_before_session = now - timedelta(hours=5)
        category2 = self.analyzer._categorize_by_timestamp(timestamp_before_session, session_start)
    ...
    async def test_analyze_recency_no_timestamps(self):
        ...
        assert report.recent_context_percentage > 0 or report.aging_context_percentage > 0
    ...
    async def test_session_analysis_completeness(self):
        ...
        if report.estimated_session_start:
            ...
            session_start = datetime.fromisoformat(report.estimated_session_start.replace('Z', '+00:00'))
            now = datetime.now(pytz.UTC)
...
class TestRecencyReport:
    ...
    def test_get_recency_summary(self):
        ...
        report = RecencyReport(
            ...
            fresh_items=[], recent_items=[], aging_items=[], stale_items=[],
            estimated_session_start=None, session_duration_minutes=0,
            session_activity_score=0, total_items_categorized=0,
            items_with_timestamps=0, analysis_timestamp='',
