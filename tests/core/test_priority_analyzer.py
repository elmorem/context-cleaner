...
from src.context_cleaner.core.priority_analyzer import (
    PriorityAnalyzer, PriorityReport, PriorityItem
)
...
class TestPriorityAnalyzer:
    ...
    async def test_analyze_priorities_basic(self):
        ...
        assert report.priority_alignment_score >= 0 and report.priority_alignment_score <= 100
    ...
    async def test_detect_current_work(self):
        ...
        current_work_items = [item for item in report.critical_items + report.high_priority_items 
    ...
    def test_extract_deadlines_basic(self):
        ...
        assert any("day" in deadline.lower() or "friday" in deadline.lower() for deadline in deadlines)
    ...
    def test_extract_dependencies_basic(self):
        ...
        content = "This task depends on database migration and is blocked by security review"
    ...
    def test_extract_dependencies_various_patterns(self):
        ...
        test_cases = [
            ...
            ("Requires testing environment setup first", ["testing environment setup first"]),
    ...
    def test_calculate_priority_score_low_priority(self):
        ...
        score, urgency, impact = self.analyzer._calculate_priority_score(content, 8, 10)  # Near end
    ...
    def test_calculate_priority_score_position_bonus(self):
        ...
        score_middle, _, _ = self.analyzer._calculate_priority_score(content, 5, 10)  # Middle
        score_bottom, _, _ = self.analyzer._calculate_priority_score(content, 9, 10)  # Bottom
    ...
    def test_categorize_by_priority_current_work(self):
        ...
        category = self.analyzer._categorize_by_priority(75, 'high', 'Currently working on auth system')
        assert category == 'current_work'
    
    def test_categorize_by_priority_blocking(self):
        """Test content categorization for blocking items."""
        category = self.analyzer._categorize_by_priority(70, 'high', 'Blocked by security review')
        assert category == 'blocking'
    
    def test_categorize_by_priority_noise(self):
        """Test content categorization for noise items."""
        category = self.analyzer._categorize_by_priority(30, 'low', 'Completed old legacy system migration')
    ...
    def test_generate_reorder_recommendations(self):
        ...
        priority_items = [
            PriorityItem(
                path="item[0]", content="Low priority item", priority_score=30,
                urgency_level='low', impact_level='low', category='routine',
                deadline=None, dependencies=[]
            ),
            PriorityItem(
                path="item[1]", content="Another low priority", priority_score=25,
                urgency_level='low', impact_level='low', category='routine',
                deadline=None, dependencies=[]
            ),
            PriorityItem(
                path="item[2]", content="CRITICAL: High priority item", priority_score=95,
                urgency_level='critical', impact_level='high', category='critical',
                deadline=None, dependencies=[]
            ),
            PriorityItem(
                path="item[3]", content="Obsolete noise item", priority_score=10,
                urgency_level='low', impact_level='low', category='noise',
                deadline=None, dependencies=[]
            )
        ]
        
        recommendations = self.analyzer._generate_reorder_recommendations(priority_items)
    ...
    def test_generate_focus_improvements(self):
        ...
        priority_items = [
            ...
            PriorityItem("path4", "Some work", 60, 'medium', 'medium', 'routine', None, []),
            PriorityItem("path5", "Blocking item", 70, 'high', 'high', 'blocking', None, [])
        ]
    ...
    async def test_analyze_priorities_error_handling(self):
        ...
        with patch.object(self.analyzer, '_extract_content_items_with_positions') as mock:
...
class TestPriorityReport:
    ...
    def test_get_priority_summary(self):
        ...
        report = PriorityReport(
            ...
            high_priority_items=[], medium_priority_items=[], low_priority_items=[], noise_items=[],
            items_with_deadlines=[], blocking_dependencies=[], priority_conflicts=[],
            reorder_recommendations=[], focus_improvement_actions=[], priority_cleanup_opportunities=[],
            total_items_analyzed=0, items_with_priority_signals=0, priority_analysis_duration=0
        )
