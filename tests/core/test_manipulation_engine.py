...
class TestManipulationEngine:
    ...
    def sample_context_data(self):
        ...
        return {
            ...
            "conversation": "User: How do I fix this authentication bug? Assistant: You can fix it by updating the login function and adding proper null checks. "
            * 100,  # Very verbose/repetitive
    ...
    def test_engine_initialization(self):
        ...
        assert (
            engine.confidence_threshold == ManipulationEngine.MIN_CONFIDENCE_THRESHOLD
        )
    ...
    def test_generate_removal_operations_duplicates(
        self, manipulation_engine, sample_context_data, mock_analysis_result
    ):
        """Test removal operation generation for duplicates."""
        operations = manipulation_engine.generate_removal_operations(
            sample_context_data, mock_analysis_result.redundancy_report
        )
    ...
    def test_generate_removal_operations_obsolete(
        self, manipulation_engine, sample_context_data, mock_analysis_result
    ):
        ...
        for op in obsolete_ops:
            assert op.operation_type == "remove"
            assert (
                op.confidence_score >= 0.8
            )  # High confidence for obvious obsolete items

    def test_generate_removal_operations_resolved_errors(
        self, manipulation_engine, sample_context_data, mock_analysis_result
    ):
        ...
        error_ops = [
            op for op in operations if "resolved error" in op.reasoning.lower()
        ]
    ...
    def test_generate_consolidation_operations_files(
        self, manipulation_engine, sample_context_data, mock_analysis_result
    ):
    ...
    def test_generate_reorder_operations(
        self, manipulation_engine, sample_context_data, mock_analysis_result
    ):
        """Test reorder operation generation."""
        operations = manipulation_engine.generate_reorder_operations(
            sample_context_data, mock_analysis_result.priority_report
        )
        ...
        for op in operations:
            ...
            assert (
                op.estimated_token_impact == 0
            )  # Reordering doesn't change token count
            assert op.confidence_score >= 0.8  # Reordering is generally safe

    def test_generate_summarization_operations(
        self, manipulation_engine, sample_context_data, mock_analysis_result
    ):
        ...
        for op in summary_ops:
            ...
            )  # Summarization always needs confirmation

    def test_create_manipulation_plan_conservative(
        self, manipulation_engine, sample_context_data, mock_analysis_result
    ):
        """Test manipulation plan creation with conservative safety level."""
        plan = manipulation_engine.create_manipulation_plan(
            sample_context_data, mock_analysis_result, "conservative"
        )
    ...
    def test_create_manipulation_plan_balanced(
        self, manipulation_engine, sample_context_data, mock_analysis_result
    ):
        """Test manipulation plan creation with balanced safety level."""
        plan = manipulation_engine.create_manipulation_plan(
            sample_context_data, mock_analysis_result, "balanced"
        )
    ...
    def test_create_manipulation_plan_aggressive(
        self, manipulation_engine, sample_context_data, mock_analysis_result
    ):
        """Test manipulation plan creation with aggressive safety level."""
        plan = manipulation_engine.create_manipulation_plan(
            sample_context_data, mock_analysis_result, "aggressive"
        )
    ...
    def test_create_manipulation_plan_no_conflicts(
        self, manipulation_engine, sample_context_data, mock_analysis_result
    ):
    ...
    def test_execute_operation_consolidate(
        self, manipulation_engine, sample_context_data
    ):
        ...
        consolidated_keys = [
            k for k in modified_context.keys() if k.startswith("consolidated_")
        ]
    ...
    def test_execute_plan_success(
        ...
        assert len(result.modified_context) <= len(
            sample_context_data
        )  # Should reduce or maintain size

    def test_execute_plan_selective(
        self, manipulation_engine, sample_context_data, mock_analysis_result
    ):
    ...
    def test_convenience_functions(self, sample_context_data, mock_analysis_result):
        ...
        result = execute_manipulation_plan(plan, sample_context_data, execute_all=True)
    ...
    def test_operation_token_calculation(self, manipulation_engine):
        ...
        assert tokens > 0
        # Uses accurate token counting (not estimation), expect ~10 tokens for this phrase
        assert 8 <= tokens <= 12  # Allow some flexibility for different tokenizers
    ...
    def test_plan_metrics_calculation(
        self, manipulation_engine, sample_context_data, mock_analysis_result
    ):
