...
from src.context_cleaner.analysis.usage_analyzer import UsagePatternSummary, WorkflowPattern, FileUsageMetrics
...
class TestEnhancedContextAnalyzer:
    ...
    def setup_method(self):
        ...
        self.analyzer = EnhancedContextAnalyzer(context_analyzer=self.mock_context_analyzer)
        ...
        self.usage_patterns = UsagePatternSummary(
            ...
            workflow_patterns=[
                WorkflowPattern(
                    pattern_id="p1", name="Development", description="Code development",
                    file_sequence=["/test/app.py", "/test/utils.py"],
                    tool_sequence=["Read", "Edit", "Bash"],
                    frequency=5, confidence_score=0.8, average_duration=30.0,
                    common_transitions=[("app.py", "utils.py", 0.7)], session_ids=[]
                )
            ],
            file_usage_metrics={
                "/test/app.py": FileUsageMetrics(
                    file_path="/test/app.py", total_accesses=15, unique_sessions=8,
                    tool_types={"Read", "Edit"}, first_access=now - timedelta(days=7),
                    last_access=now - timedelta(hours=1), average_session_frequency=3.0,
                    peak_usage_hours=[9, 10, 14], common_contexts=["python", "main"]
                ),
                "/test/utils.py": FileUsageMetrics(
                    file_path="/test/utils.py", total_accesses=8, unique_sessions=5,
                    tool_types={"Read", "Edit"}, first_access=now - timedelta(days=5),
                    last_access=now - timedelta(hours=2), average_session_frequency=2.0,
                    peak_usage_hours=[10, 15], common_contexts=["utils", "helper"]
                )
            },
        ...
        self.token_efficiency = TokenAnalysisSummary(
            cache_efficiency=CacheEfficiencyMetrics(
                ...
                total_cache_creation_tokens=2000, total_cache_read_tokens=1500,
                ephemeral_5m_tokens=1200, ephemeral_1h_tokens=800,
                overall_cache_hit_ratio=0.75, average_cache_efficiency=0.7,
                cache_utilization_score=0.8, sessions_with_good_cache_usage=7,
                sessions_with_poor_cache_usage=2, sessions_with_no_cache_usage=1
            ),
            usage_insights=TokenUsageInsights(
                average_tokens_per_session=1500.0, average_tokens_per_message=150.0,
        ...
        self.temporal_insights = TemporalInsights(
            session_boundaries=[], topic_transitions=[],
            evolution_patterns=[], average_session_length=1.5,
            typical_break_duration=2.0, peak_activity_periods=[],
            context_stability_score=0.7, topic_drift_frequency=2.0,
            return_to_topic_rate=0.3, multitasking_intensity=0.4,
            optimal_session_length=2.0, recommended_break_frequency=60.0,
            productivity_time_patterns={"9": 0.8, "10": 0.9, "14": 0.7},
            context_switching_cost=0.3, analysis_period=(now - timedelta(days=30), now),
    ...
    def test_analyze_with_cache_intelligence(self):
        ...
        # Mock base analyzer
        self.mock_context_analyzer.analyze_context_sync.return_value = self.base_analysis
    ...
    def test_file_relates_to_topic(self):
        ...
        assert self.analyzer._file_relates_to_topic("/docs/README.md", {"documentation"})
        assert self.analyzer._file_relates_to_topic("/config/settings.yaml", {"configuration"})
    ...
    def test_generate_personalized_recommendations(self):
        ...
        recommendations = self.analyzer._generate_personalized_recommendations(
            self.base_analysis, self.usage_patterns, self.token_efficiency, self.temporal_insights
        )
    ...
    def test_cache_insights_caching(self):
        ...
        with patch.object(self.analyzer.discovery, 'discover_cache_locations') as mock_discover:
            with patch.object(self.analyzer.usage_analyzer, 'analyze_usage_patterns') as mock_usage:
                with patch.object(self.analyzer.token_analyzer, 'analyze_token_efficiency') as mock_token:
                    with patch.object(self.analyzer.temporal_analyzer, 'analyze_temporal_patterns') as mock_temporal:
...
class TestCacheEnhancedAnalysis:
    ...
    def test_cache_enhanced_analysis_properties(self):
        ...
        focus_score = UsageWeightedScore(
            base_score=0.6, usage_weight=0.2, final_score=0.7,
            usage_factors={}, confidence=0.8
        )
        priority_score = UsageWeightedScore(
            base_score=0.7, usage_weight=0.1, final_score=0.75,
            usage_factors={}, confidence=0.7
        )
        recency_score = UsageWeightedScore(
            base_score=0.8, usage_weight=0.0, final_score=0.8,
            usage_factors={}, confidence=0.6
        )
        redundancy_score = UsageWeightedScore(
            base_score=0.3, usage_weight=0.1, final_score=0.35,
            usage_factors={}, confidence=0.5
        )
        
        enhanced_analysis = CacheEnhancedAnalysis(
            base_analysis=base_analysis,
            usage_patterns=Mock(), token_efficiency=Mock(), temporal_insights=Mock(),
            usage_weighted_focus=focus_score, usage_weighted_priority=priority_score,
            usage_weighted_recency=recency_score, usage_weighted_redundancy=redundancy_score,
            personalized_recommendations=[], workflow_optimizations=[], context_health_insights=[],
            enhancement_confidence=0.8, cache_data_quality=0.9, personalization_strength=0.7
        )
