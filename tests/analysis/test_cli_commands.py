...
class TestComprehensiveCommand:
    ...
    def mock_analysis(self):
        ...
        return EnhancedTokenAnalysis(
            ...
            sessions=sessions,
            category_reported={"user_messages": 800, "claude_md": 300, "system_prompts": 400},
            api_calls_made=5,
    ...
    def test_comprehensive_command_basic_execution(self, runner, mock_analysis):
        ...
        with patch('src.context_cleaner.cli.commands.enhanced_token_analysis.EnhancedTokenCounterService') as mock_service_class:
            mock_service = AsyncMock()
            mock_service.analyze_comprehensive_token_usage = AsyncMock(return_value=mock_analysis)
            mock_service_class.return_value = mock_service
            
            result = runner.invoke(comprehensive, [])
            
            assert result.exit_code == 0
            assert "Enhanced Token Analysis - Addressing 90% Undercount Issue" in result.output
            assert "Analysis Complete" in result.output
            assert "Undercount detected: 90.0%" in result.output
    ...
    def test_comprehensive_command_undercount_detection_display(self, runner, mock_analysis):
...
class TestDashboardCommand:
    ...
    def test_dashboard_command_success(self, runner):
        ...
        with patch('src.context_cleaner.cli.commands.enhanced_token_analysis.DashboardTokenAnalyzer') as mock_analyzer_class:
            mock_analyzer = AsyncMock()
            mock_analyzer.get_enhanced_token_analysis = AsyncMock(return_value=mock_dashboard_data)
    ...
    def test_dashboard_command_fallback(self, runner):
        ...
        mock_fallback_data = {
            ...
            "recommendations": ["Enhanced token analysis failed. Check logs and API key configuration."]
        }
        
        with patch('src.context_cleaner.cli.commands.enhanced_token_analysis.DashboardTokenAnalyzer') as mock_analyzer_class:
            mock_analyzer = AsyncMock()
            mock_analyzer.get_enhanced_token_analysis = AsyncMock(return_value=mock_fallback_data)
            mock_analyzer_class.return_value = mock_analyzer
    ...
    def test_dashboard_command_error(self, runner):
        ...
        with patch('src.context_cleaner.cli.commands.enhanced_token_analysis.DashboardTokenAnalyzer') as mock_analyzer_class:
            ...
            assert result.exit_code == 1  # Should exit with error
            assert "❌ Dashboard integration failed: Dashboard integration failed" in result.output
...
class TestSessionCommand:
    ...
    def test_session_command_exact_match(self, runner, mock_analysis_with_sessions):
        ...
        with patch('src.context_cleaner.cli.commands.enhanced_token_analysis.EnhancedTokenCounterService') as mock_service_class:
            mock_service = AsyncMock()
            mock_service.analyze_comprehensive_token_usage = AsyncMock(return_value=mock_analysis_with_sessions)
            mock_service_class.return_value = mock_service
...
class TestTokenAnalysisGroup:
    ...
    def test_token_analysis_group_help(self, runner):
        ...
        assert result.exit_code == 0
        assert "Enhanced token analysis commands using Anthropic's count-tokens API" in result.output
    ...
    def test_comprehensive_command_help(self, runner):
        ...
        assert result.exit_code == 0
        assert "Run comprehensive token analysis addressing the 90% undercount issue" in result.output
    ...
    def test_dashboard_command_help(self, runner):
        ...
        assert result.exit_code == 0
        assert "Test the dashboard integration for enhanced token analysis" in result.output
