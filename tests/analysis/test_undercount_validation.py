...
class TestUndercountDetection:
    ...
    async def test_severe_undercount_detection(self, service, tmp_path):
        ...
        entries = [
            # Large user message - MISSED by current system
            {
                ...
                "message": {
                    "role": "user",
                    "content": "I need comprehensive help with creating a complex system that processes large amounts of data. "
            ...
            # Large system prompt - MISSED by current system
            {
                ...
                "message": {
                    "role": "system",
                    "content": "<system-reminder>You are Claude Code with extensive capabilities. "
                    * 30,  # Large system context
                },
            },
            ...
            *[
                {
                    ...
                    "message": {
                        "role": "assistant",
                        "content": f"Using tool {i} to process large amounts of data and analyze complex patterns. "
                        * 20,
                    },
                }
                for i in range(10)
        ...
        with patch.object(service, "cache_dir", tmp_path):
            ...
            # Tool usage is categorized as assistant messages, but the actual count
            # depends on how the enhanced counter processes tool_use entries
            assert (
                len(session.assistant_messages) >= 1
            )  # At least the assistant response
            assert session.content_categories["user_messages"] > 0
            # System prompts may be categorized differently (e.g., as system_tools)
            # The key is that total calculated tokens captures the content
            assert session.calculated_total_tokens > session.total_reported_tokens * 5
    ...
    async def test_moderate_undercount_detection(self, service, tmp_path):
        ...
        with patch.object(service, "cache_dir", tmp_path):
            ...
            # Calculated tokens may be lower based on actual heuristic calculations
            assert (
                session.calculated_total_tokens > 250
            )  # Should find more than reported
            assert session.undercount_percentage > 10.0  # Some undercount detected
    ...
    async def test_file_limit_impact_on_undercount(self, service, tmp_path):
                ...
                entries = [
                    {
                        ...
                        "message": {
                            "role": "user",
                            "content": f"Large user request with lots of content. "
                            * 30,  # Significant content
                        },
                    },
                    {
                        "type": "assistant",
        ...
        with patch.object(service, "cache_dir", tmp_path):
            ...
            assert (
                analysis_enhanced.total_calculated_tokens
                > analysis_limited.total_calculated_tokens
            )

            # Both systems should detect undercount, but exact percentages depend on content distribution
            # The key is that enhanced finds more tokens overall
            assert (
                analysis_enhanced.total_calculated_tokens
                > analysis_enhanced.total_reported_tokens
            )
            assert (
                analysis_limited.total_calculated_tokens
                > analysis_limited.total_reported_tokens
            )
    ...
    async def test_line_limit_impact_on_undercount(self, service, tmp_path):
            ...
            if i < 1000:
                ...
                entry = {
                    ...
                    "message": {
                        ...
                        "content": f"Additional conversation content {i} that would be missed by current system. "
                        * 10,
                    },
                }
        ...
        with patch.object(service, "cache_dir", tmp_path):
            ...
            analysis_limited = await service.analyze_comprehensive_token_usage(
                max_lines_per_file=1000, use_count_tokens_api=False
            )

            # Test enhanced system (all lines)
            analysis_enhanced = await service.analyze_comprehensive_token_usage(
                max_lines_per_file=None, use_count_tokens_api=False
            )
            ...
            assert (
                session_enhanced.calculated_total_tokens
                > session_limited.calculated_total_tokens
            )
            assert len(session_enhanced.user_messages) > len(
                session_limited.user_messages
            )
            assert len(session_enhanced.assistant_messages) > len(
                session_limited.assistant_messages
            )
    ...
    async def test_content_type_coverage_impact(self, service, tmp_path):
            ...
            *[
                {
                    ...
                    "message": {
                        "role": "user",
                        "content": f"User message {i} with substantial content explaining requirements. "
                        * 20,
                    },
                }
                for i in range(5)
            ...
            *[
                {
                    ...
                    "message": {
                        "role": "system",
                        "content": f"<system-reminder>System context {i} with detailed instructions. "
                        * 15
                        + "</system-reminder>",
                    },
                }
            ...
            *[
                {
                    ...
                    "message": {
                        "role": "assistant",
                        "content": f"Tool usage {i} with complex function calls and parameters. "
                        * 25,
                    },
                }
                for i in range(7)
        ...
        with patch.object(service, "cache_dir", tmp_path):
            ...
            # Tool usage categorization may vary based on implementation
            assert (
                len(session.assistant_messages) >= 2
            )  # At least the 2 assistant responses
            assert session.content_categories["user_messages"] > 0
            assert session.content_categories["system_prompts"] > 0

            # Should detect significant undercount due to missed content types
            assert session.undercount_percentage > 70.0  # Significant undercount
            assert session.calculated_total_tokens > 500  # Much more than 70
...
class TestUndercountComparison:
    ...
    async def test_current_vs_enhanced_system_comparison(self, tmp_path):
            ...
            entries.append(
                {
                    ...
                    "message": {
                        "role": "user",
                        "content": f"Complex user request {session_num} with detailed requirements and context. "
                        * 30,
                    },
                }
            )
            ...
            entries.append(
                {
                    ...
                    "message": {
                        "role": "system",
                        "content": "<system-reminder>Claude Code instructions with comprehensive guidelines. "
                        * 20
                        + "</system-reminder>",
                    },
                }
                ...
                entries.append(
                    {
                        ...
                        "message": {
                            "role": "assistant",
                            "content": f"Tool {tool_num} usage with detailed function call and extensive parameters. "
                            * 20,
                        },
                    }
                )
        ...
        with patch.object(service_enhanced, "cache_dir", tmp_path):
            enhanced_analysis = (
                await service_enhanced.analyze_comprehensive_token_usage(
        ...
        # Compare results
        assert (
            enhanced_analysis.total_files_processed
            > current_analysis.total_files_processed
        )
        assert (
            enhanced_analysis.total_lines_processed
            > current_analysis.total_lines_processed
        )
        assert (
            enhanced_analysis.total_sessions_analyzed
            > current_analysis.total_sessions_analyzed
        )

        # Enhanced system should find more tokens
        # Note: The improvement factor depends on actual content and heuristics
        improvement_factor = (
            enhanced_analysis.total_calculated_tokens
            / current_analysis.total_calculated_tokens
        )
        assert improvement_factor >= 1.1  # At least 10% more tokens found
        ...
        # Lines processed depends on actual file content
        assert improvement_summary["enhanced_coverage"]["lines_processed"] > 0
    ...
    async def test_dashboard_undercount_reporting(self):
        ...
        with patch.object(
            ...
            undercount_warning = any(
                "90.0%" in rec and "undercount" in rec.lower()
...
class TestUndercountEdgeCases:
    ...
    async def test_empty_files_handling(self, tmp_path):
        ...
        with patch.object(service, "cache_dir", tmp_path):
            ...
            assert (
                len(analysis.errors_encountered) == 0
            ...
            *[
                {
                    ...
                    "message": {
                        "role": "user",
                        "content": f"Extremely long user message {i} with enormous amounts of text content that represents significant token usage. "
                        * 100,
                    },
                }
                for i in range(50)
