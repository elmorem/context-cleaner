...
from src.context_cleaner.analysis.enhanced_token_counter import EnhancedTokenCounterService
...
class TestCompleteSystemIntegration:
    ...
    def realistic_test_environment(self, tmp_path):
        ...
        for i in range(18):
            file_path = cache_dir / f"conversation_{i:03d}_{datetime.now().strftime('%Y%m%d')}.jsonl"
            ...
            user_content = f"Complex user request {i} with detailed requirements: " + \
                          "Please help me implement a comprehensive solution that handles multiple edge cases. " * 20
            entries.append({
                "type": "user",
            ...
            system_content = f"<system-reminder>You are Claude Code with extensive capabilities for session {i}. " + \
                           "Provide comprehensive solutions following best practices. " * 15 + "</system-reminder>"
            entries.append({
                "type": "system",
                "timestamp": (datetime.now() - timedelta(hours=i)).isoformat(),
                "session_id": f"session_{i:03d}",
            ...
            for j in range(3):
                tool_content = f"Tool usage {j} for session {i}: Using Read tool to examine complex file structures. " * 25
                entries.append({
                    "type": "tool_use",
                    "timestamp": (datetime.now() - timedelta(hours=i)).isoformat(),
                    "session_id": f"session_{i:03d}",
    ...
    @pytest.mark.asyncio
    async def test_complete_undercount_detection_workflow(self, realistic_test_environment):
        ...
        with patch.object(service, 'cache_dir', env["cache_dir"]):
            ...
            assert analysis.total_lines_processed > 50   # Multiple entries per file
            
            # Validate session detection
            assert analysis.total_sessions_analyzed == 18  # One session per file
            
            # Validate undercount detection
            assert analysis.total_reported_tokens == env["total_reported_tokens"]  # Current system result
            assert analysis.total_calculated_tokens > env["total_reported_tokens"] * 5  # Much more content found
    ...
    @pytest.mark.asyncio
    async def test_dashboard_integration_complete_workflow(self, realistic_test_environment):
        ...
        with patch.object(analyzer.token_service, 'cache_dir', env["cache_dir"]):
            ...
            undercount_warning = any("undercount" in rec.lower() for rec in recommendations)
    ...
    async def test_api_integration_complete_workflow(self, realistic_test_environment):
        ...
        with patch.object(service, 'cache_dir', env["cache_dir"]):
            with patch('src.context_cleaner.analysis.enhanced_token_counter.AnthropicTokenCounter') as mock_counter_class:
                ...
                # Verify API was used
                assert analysis.api_calls_made > 0
                assert call_count > 0
    ...
    async def test_error_resilience_complete_workflow(self, tmp_path):
        ...
        files_data = [
            # Valid files
            ("valid_01.jsonl", JSONLFixtures.create_undercount_conversation("valid_01")),
            ("valid_02.jsonl", JSONLFixtures.create_undercount_conversation("valid_02")),
            ...
            ("partial_invalid.jsonl", [
                {"type": "user", "message": {"role": "user", "content": "Valid entry"}},
                "invalid json line",
                {"type": "assistant", "message": {"role": "assistant", "content": "Another valid entry"}}
            ]),
            
            # Large valid file
            ("large_valid.jsonl", JSONLFixtures.create_large_conversation("large_session", 500))
        ]
    ...
    async def test_development_session_scenario(self, tmp_path):
        ...
        entries = [
            # Initial user request
            {
                ...
                "message": {
                    "role": "user",
                    "content": "I need help creating comprehensive tests for the Enhanced Token Counting System. " * 10
                }
            },
...
You are Claude Code, Anthropic's official CLI for Claude. You are an expert test engineer specializing in Django/React applications with deep expertise in both frontend and backend testing methodologies.
            ...
            *[{
                ...
                "message": {
                    "role": "assistant",
                    "content": f"Tool usage {i}: Reading and analyzing file structure to understand the codebase architecture. " * 30
                }
            } for i in range(8)],
        ...
        with patch.object(service, 'cache_dir', cache_dir):
            ...
            assert session.content_categories["user_messages"] > 0
            assert session.content_categories["system_prompts"] > 0
            assert len(session.assistant_messages) > 8  # Tool usage captured
    ...
    async def test_mixed_content_types_scenario(self, tmp_path):
        ...
        for filename, category in patterns:
            ...
            if category == "claude_md":
                ...
                content = "Complex user request with detailed requirements and extensive context. " * 50
        ...
        with patch.object(service, 'cache_dir', cache_dir):
            ...
            assert analysis.category_reported["claude_md"] > 0
            assert analysis.category_reported["system_prompts"] > 0
            assert analysis.category_reported["system_tools"] > 0 
            assert analysis.category_reported["user_messages"] > 0
            
            # Should detect significant undercount across all categories
            assert analysis.global_undercount_percentage > 50.0
