...
class MockAPIResponses:
    ...
    def create_error_response(
        status: int = 429, message: str = "Rate limit exceeded"
    ) -> Dict[str, Any]:
...
class JSONLFixtures:
    ...
    @classmethod
    def create_undercount_conversation(
        cls, session_id: str = "session_001"
    ) -> List[Dict[str, Any]]:
        """
...
You are Claude Code, Anthropic's official CLI for Claude. You are an expert test engineer specializing in Django/React applications with deep expertise in both frontend and backend testing methodologies. You create robust, performant, and maintainable test suites that minimize flakiness and maximize reliability for modern web applications.
                    ...
                    "tool_calls": [
                        {
                            ...
                            "function": {
                                "name": "Read",
                                "arguments": {
                                    "file_path": "/path/to/context-cleaner/src/context_cleaner/analysis/enhanced_token_counter.py"
                                },
                            },
                        }
                    ],
    ...
    def create_large_conversation(
        cls, session_id: str = "large_session", num_entries: int = 2000
    ) -> List[Dict[str, Any]]:
        ...
        for i in range(num_entries):
            ...
            content = (
                f"Message {i + 1}: " + "This is test content " * 10
            )  # Varied length
            ...
            if (
                message_type == "assistant" and i % 10 == 1
            ):  # Only some assistant messages have usage
                entry["message"]["usage"] = {
                    "input_tokens": 50,
                    "output_tokens": 20,
                    "cache_creation_input_tokens": 0,
    ...
    def create_multiple_sessions_data(cls) -> Dict[str, List[Dict[str, Any]]]:
        ...
        sessions["high_undercount"] = cls.create_undercount_conversation(
            "session_high_undercount"
        )
    ...
    def create_realistic_conversation_entry(
        ...
        """Create a single realistic conversation entry."""
        if content is None:
            if message_type == "user":
                content = (
                    "Can you help me with this task? " + "Additional context " * 10
                )
            elif message_type == "assistant":
                content = "I'll help you with that. " + "Detailed response " * 10
            elif message_type == "system":
                content = "<system-reminder>You are Claude Code.</system-reminder>"
            else:
                content = "Tool output or other content"
        ...
        entry = {
            ...
            "message": {
                "role": (
                    message_type
                    if message_type in ["user", "assistant", "system"]
                    else "assistant"
                ),
                "content": content,
            },
        ...
        # Add usage stats if provided directly
        if usage is not None and message_type == "assistant":
            entry["message"]["usage"] = usage
        # Or add usage stats if requested (typical for assistant messages)
        elif has_usage and message_type == "assistant":
            entry["message"]["usage"] = {
                "input_tokens": len(content) // 4,  # Rough approximation
                "output_tokens": len(content) // 8,
                "cache_creation_input_tokens": 0,
                "cache_read_input_tokens": 0,
            }
    ...
    @classmethod
    def create_claude_md_content(cls) -> str:
...
## Architecture
This is a Claude Code project with enhanced token counting.
"""

    @classmethod
    def create_system_prompt_content(cls) -> str:
        """Create sample system prompt content for testing."""
        return """<system-reminder>
You are Claude Code, Anthropic's official CLI for Claude. You are an expert software engineer
with deep knowledge of Python, testing, and software architecture. You write clean, maintainable
code following best practices and industry standards.
</system-reminder>"""

    @classmethod
    def create_tool_usage_content(cls) -> str:
        """Create sample tool usage content for testing."""
        return """Tool execution result: Successfully read file '/path/to/file.py'
Content includes multiple classes, functions, and comprehensive documentation.
Total lines: 500, Functions: 25, Classes: 8
This represents typical tool output that contributes to token counts."""
...
class UndercountTestCases:
    ...
    def all_scenarios(cls) -> List[UndercountScenario]:
        return [
            cls.SEVERE_UNDERCOUNT,
            cls.MODERATE_UNDERCOUNT,
            cls.MINIMAL_UNDERCOUNT,
            cls.NO_UNDERCOUNT,
        ]
    ...
    def create_test_file_structure(self, base_path: str = "/test/claude/projects"):
        ...
        for i in range(15):
            ...
            if i < 5:
                ...
                content = JSONLFixtures.create_undercount_conversation(
                    f"session_{i:03d}"
                )
            elif i < 10:
                # Older files that current system would miss
                content = JSONLFixtures.create_large_conversation(
                    f"session_{i:03d}", 500
                )
            else:
                # Even older files - completely missed by current system
                content = JSONLFixtures.create_large_conversation(
                    f"session_{i:03d}", 1500
                )
