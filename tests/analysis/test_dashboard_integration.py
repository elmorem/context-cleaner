...
class TestDashboardTokenAnalyzer:
    ...
    async def test_dashboard_compatible_format(self, analyzer, mock_enhanced_analysis):
        ...
        with patch.object(analyzer.token_service, 'analyze_comprehensive_token_usage', 
            ...
            # Verify total_tokens structure
            total_tokens = result["total_tokens"]
            required_fields = ["input", "cache_creation", "cache_read", "output", "total"]
            for field in required_fields:
                assert field in total_tokens
                assert isinstance(total_tokens[field], int)
            ...
            for category in categories:
                ...
                assert "total_tokens" in category
    ...
    async def test_category_breakdown_accuracy(self, analyzer, mock_enhanced_analysis):
        ...
        with patch.object(analyzer.token_service, 'analyze_comprehensive_token_usage', 
            ...
            assert top_category["total_tokens"] == 1100  # From mock data
    ...
    async def test_session_breakdown(self, analyzer, mock_enhanced_analysis):
        ...
        with patch.object(analyzer.token_service, 'analyze_comprehensive_token_usage', 
            ...
            session_001 = next(s for s in top_sessions if s["session_id"] == "session_001")
    ...
    async def test_optimization_recommendations(self, analyzer, mock_enhanced_analysis):
        ...
        with patch.object(analyzer.token_service, 'analyze_comprehensive_token_usage', 
            ...
            undercount_warning = any("undercount detected" in rec.lower() for rec in recommendations)
            assert undercount_warning
            
            # Should mention file coverage improvement
            coverage_improvement = any("conversation files" in rec.lower() for rec in recommendations)
            assert coverage_improvement
            
            # Should mention API usage if available
            api_usage = any("count-tokens api" in rec.lower() for rec in recommendations)
    ...
    async def test_caching_mechanism(self, analyzer, mock_enhanced_analysis):
        ...
        with patch.object(analyzer.token_service, 'analyze_comprehensive_token_usage', 
                         return_value=mock_enhanced_analysis) as mock_analyze:
    ...
    @pytest.mark.asyncio
    async def test_api_key_auto_detection(self, analyzer):
        """Test automatic API key detection for API validation."""
        # Test with API key available
        with patch.dict('os.environ', {'ANTHROPIC_API_KEY': 'test_key'}):
            analyzer_with_key = DashboardTokenAnalyzer()
            assert analyzer_with_key.anthropic_api_key == 'test_key'
        
        # Test without API key
        with patch.dict('os.environ', {}, clear=True):
            analyzer_no_key = DashboardTokenAnalyzer()
            assert analyzer_no_key.anthropic_api_key is None
    
    @pytest.mark.asyncio
    async def test_api_validation_parameter_handling(self, analyzer, mock_enhanced_analysis):
        ...
        with patch.object(analyzer.token_service, 'analyze_comprehensive_token_usage', 
            ...
            await analyzer.get_enhanced_token_analysis(use_api_validation=False)
            
            call_args = mock_analyze.call_args
            assert call_args[1]["use_count_tokens_api"] is False
    
    @pytest.mark.asyncio
    async def test_comprehensive_vs_limited_processing(self, analyzer, mock_enhanced_analysis):
...
class TestDashboardIntegrationFunctions:
    ...
    async def test_get_enhanced_token_analysis_for_dashboard(self):
        ...
        with patch('src.context_cleaner.analysis.dashboard_integration.DashboardTokenAnalyzer') as mock_analyzer_class:
            mock_analyzer = AsyncMock()
            mock_analyzer.get_enhanced_token_analysis = AsyncMock(return_value={"test": "result"})
            mock_analyzer_class.return_value = mock_analyzer
            
            result = await get_enhanced_token_analysis_for_dashboard()
            
            assert result == {"test": "result"}
            mock_analyzer.get_enhanced_token_analysis.assert_called_once_with(force_refresh=False)
    
    def test_get_enhanced_token_analysis_sync(self):
        """Test synchronous dashboard integration wrapper."""
        with patch('src.context_cleaner.analysis.dashboard_integration.get_enhanced_token_analysis_for_dashboard') as mock_async:
            ...
            with patch('asyncio.new_event_loop') as mock_new_loop, \
                ...
                mock_loop = MagicMock()
                mock_loop.run_until_complete = MagicMock(return_value={"sync": "result"})
    ...
    def test_sync_wrapper_error_handling(self):
        ...
        with patch('asyncio.new_event_loop', side_effect=Exception("Async error")):
            result = get_enhanced_token_analysis_sync()
    ...
    async def test_drop_in_replacement_compatibility(self):
        ...
        def current_analyze_token_usage():
            ...
            return {
                ...
                "categories": [{"name": "Assistant Messages", "tokens": {"total": 150}}]
            }
        ...
        with patch('src.context_cleaner.analysis.dashboard_integration.DashboardTokenAnalyzer') as mock_analyzer_class:
            mock_analyzer = AsyncMock()
            mock_analyzer.get_enhanced_token_analysis = AsyncMock(return_value={
                "total_tokens": {"input": 1000, "cache_creation": 200, "cache_read": 100, "output": 300, "total": 1600},
                "categories": [
            ...
            assert enhanced_result["total_tokens"]["total"] > current_result["total_tokens"]["total"]
            assert len(enhanced_result["categories"]) >= len(current_result["categories"])
