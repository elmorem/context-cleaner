...
class TestAnthropicTokenCounter:
    ...
    async def test_successful_token_counting(self, token_counter):
        ...
        # Mock aiohttp response - needs to be an async context manager
        mock_response = AsyncMock()
        mock_response.status = 200
        mock_response.json = AsyncMock(return_value={"input_tokens": 150})

        # Create a proper async context manager
        async def mock_response_context(*args, **kwargs):
            return mock_response

        # Mock aiohttp session
        mock_session = AsyncMock()
        mock_session.post = MagicMock(return_value=mock_response)
        mock_response.__aenter__ = mock_response_context
...
class TestEnhancedTokenCounterService:
    ...
    def test_content_categorization(self, service):
        ...
        # Test regular user message (avoid words that match system_tools patterns)
        user_content = "Please help me with this problem"
    ...
    @pytest.mark.asyncio
    async def test_complete_file_processing_vs_line_limit(
        self, service, mock_cache_dir
    ):
        ...
        with open(large_file, "w") as f:
            for i in range(2000):
                entry = JSONLFixtures.create_realistic_conversation_entry(
                    session_id="large_session", content=f"Line {i + 1} content"
                )
                f.write(json.dumps(entry) + "\n")
    ...
    async def test_all_message_types_processing(self, service, mock_cache_dir):
        ...
        with patch.object(service, "cache_dir", mock_cache_dir):
            ...
            assert (
                session.calculated_total_tokens > session.total_reported_tokens
            )  # Enhanced counting
    ...
    async def test_undercount_detection(self, service, mock_cache_dir):
        ...
        with patch.object(service, "cache_dir", mock_cache_dir):
            ...
            assert (
                analysis.global_undercount_percentage > 50.0
            )  # Significant undercount detected
            assert (
                analysis.global_accuracy_ratio > 2.0
            )  # Much more content than reported
    ...
    async def test_api_integration(self, service, mock_cache_dir):
        ...
        with patch.object(service, "cache_dir", mock_cache_dir):
            with patch(
                ...
                # Make the instance work as an async context manager
    ...
    async def test_fallback_processing(self, service, mock_cache_dir):
        ...
        with patch.object(service, "cache_dir", mock_cache_dir):
            ...
            assert (
                analysis.sessions["fallback_session"].calculated_total_tokens > 0
            )  # Estimated tokens
...
class TestSessionTokenTracker:
    ...
    async def test_session_token_updates(self, tracker):
        ...
        await tracker.update_session_tokens(
            session_id="test_session", input_tokens=100, output_tokens=50
        )
    ...
    async def test_concurrent_access(self, tracker):
        ...
        async def update_session(tokens):
            await tracker.update_session_tokens(
                session_id=session_id, input_tokens=tokens
            )
